var location_data = {
   "Ohio, United States" : {
      "KTOL: Toledo Express Airport" : {
         "city" : "Toledo",
         "name" : "Toledo Express Airport",
         "state" : "Ohio",
         "code" : "KTOL"
      },
      "KCAK: Akron-Canton Regional Airport" : {
         "city" : "Akron",
         "name" : "Akron-Canton Regional Airport",
         "state" : "Ohio",
         "code" : "KCAK"
      },
      "KAKR: Akron Fulton International Airport" : {
         "city" : "Akron",
         "name" : "Akron Fulton International Airport",
         "state" : "Ohio",
         "code" : "KAKR"
      },
      "KOXD: Miami University Airport" : {
         "city" : "Oxford",
         "name" : "Miami University Airport",
         "state" : "Ohio",
         "code" : "KOXD"
      },
      "KCQA: Lakefield Airport" : {
         "city" : "Celina",
         "name" : "Lakefield Airport",
         "state" : "Ohio",
         "code" : "KCQA"
      },
      "KLCK: Rickenbacker International Airport" : {
         "city" : "Columbus",
         "name" : "Rickenbacker International Airport",
         "state" : "Ohio",
         "code" : "KLCK"
      },
      "KDLZ: Delaware Municipal Airport" : {
         "city" : "Delaware",
         "name" : "Delaware Municipal Airport",
         "state" : "Ohio",
         "code" : "KDLZ"
      },
      "KCYO: Pickaway County Memorial Airport" : {
         "city" : "Circleville",
         "name" : "Pickaway County Memorial Airport",
         "state" : "Ohio",
         "code" : "KCYO"
      },
      "KVES: Darke County Airport" : {
         "city" : "Versailles",
         "name" : "Darke County Airport",
         "state" : "Ohio",
         "code" : "KVES"
      },
      "KLHQ: Fairfield County Airport (Ohio)" : {
         "city" : "Lancaster",
         "name" : "Fairfield County Airport (Ohio)",
         "state" : "Ohio",
         "code" : "KLHQ"
      },
      "KFFO: Wright-Patterson Air Force Base" : {
         "city" : "Dayton",
         "name" : "Wright-Patterson Air Force Base",
         "state" : "Ohio",
         "code" : "KFFO"
      },
      "KDFI: Defiance Memorial Airport" : {
         "city" : "Defiance",
         "name" : "Defiance Memorial Airport",
         "state" : "Ohio",
         "code" : "KDFI"
      },
      "KRZT: Ross County Regional Airport" : {
         "city" : "Chillicothe",
         "name" : "Ross County Regional Airport",
         "state" : "Ohio",
         "code" : "KRZT"
      },
      "KTZR: Bolton Field" : {
         "city" : "Columbus",
         "name" : "Bolton Field",
         "state" : "Ohio",
         "code" : "KTZR"
      },
      "KCGF: Cuyahoga County Airport" : {
         "city" : "Cleveland",
         "name" : "Cuyahoga County Airport",
         "state" : "Ohio",
         "code" : "KCGF"
      },
      "KDAY: James M. Cox International Airport" : {
         "city" : "Dayton",
         "name" : "James M. Cox International Airport",
         "state" : "Ohio",
         "code" : "KDAY"
      },
      "KAXV: Neil Armstrong Airport" : {
         "city" : "Wapakoneta",
         "name" : "Neil Armstrong Airport",
         "state" : "Ohio",
         "code" : "KAXV"
      },
      "KCLE: Cleveland Hopkins International Airport" : {
         "city" : "Cleveland",
         "name" : "Cleveland Hopkins International Airport",
         "state" : "Ohio",
         "code" : "KCLE"
      },
      "KEOP: Pike County Airport (Ohio)" : {
         "city" : "Waverly",
         "name" : "Pike County Airport (Ohio)",
         "state" : "Ohio",
         "code" : "KEOP"
      },
      "KFDY: Findlay Airport" : {
         "city" : "Findlay",
         "name" : "Findlay Airport",
         "state" : "Ohio",
         "code" : "KFDY"
      },
      "KCMH: Port Columbus International Airport" : {
         "city" : "Columbus",
         "name" : "Port Columbus International Airport",
         "state" : "Ohio",
         "code" : "KCMH"
      },
      "KLPR: Lorain County Regional Airport" : {
         "city" : "Lorain",
         "name" : "Lorain County Regional Airport",
         "state" : "Ohio",
         "code" : "KLPR"
      },
      "KLNN: Willoughby Lost Nation Municipal Airport" : {
         "city" : "Willoughby",
         "name" : "Willoughby Lost Nation Municipal Airport",
         "state" : "Ohio",
         "code" : "KLNN"
      },
      "KCDI: Cambridge Municipal Airport (Ohio)" : {
         "city" : "Cambridge",
         "name" : "Cambridge Municipal Airport (Ohio)",
         "state" : "Ohio",
         "code" : "KCDI"
      },
      "KFZI: Fostoria Metropolitan Airport" : {
         "city" : "Fostoria",
         "name" : "Fostoria Metropolitan Airport",
         "state" : "Ohio",
         "code" : "KFZI"
      },
      "KAOH: Lima Allen County Airport" : {
         "city" : "Lima",
         "name" : "Lima Allen County Airport",
         "state" : "Ohio",
         "code" : "KAOH"
      },
      "KMGY: Dayton Wright Brothers Airport" : {
         "city" : "Dayton",
         "name" : "Dayton Wright Brothers Airport",
         "state" : "Ohio",
         "code" : "KMGY"
      },
      "KOSU: Ohio State University Airport" : {
         "city" : "Columbus",
         "name" : "Ohio State University Airport",
         "state" : "Ohio",
         "code" : "KOSU"
      },
      "KZZV: Zanesville Municipal Airport" : {
         "city" : "Zanesville",
         "name" : "Zanesville Municipal Airport",
         "state" : "Ohio",
         "code" : "KZZV"
      },
      "KAMT: Alexander Salamon Airport" : {
         "city" : "West Union",
         "name" : "Alexander Salamon Airport",
         "state" : "Ohio",
         "code" : "KAMT"
      },
      "KEDJ: Bellefontaine Regional Airport" : {
         "city" : "Bellefontaine",
         "name" : "Bellefontaine Regional Airport",
         "state" : "Ohio",
         "code" : "KEDJ"
      },
      "KUNI: Gordon K. Bush Airport" : {
         "city" : "Athens",
         "name" : "Gordon K. Bush Airport",
         "state" : "Ohio",
         "code" : "KUNI"
      },
      "KHAO: Butler County Regional Airport" : {
         "city" : "Hamilton",
         "name" : "Butler County Regional Airport",
         "state" : "Ohio",
         "code" : "KHAO"
      },
      "KVTA: Newark-Heath Airport" : {
         "city" : "Newark",
         "name" : "Newark-Heath Airport",
         "state" : "Ohio",
         "code" : "KVTA"
      },
      "KYNG: Youngstown-Warren Regional Airport" : {
         "city" : "Youngstown",
         "name" : "Youngstown-Warren Regional Airport",
         "state" : "Ohio",
         "code" : "KYNG"
      },
      "KBJJ: Wayne County Airport (Ohio)" : {
         "city" : "Wooster",
         "name" : "Wayne County Airport (Ohio)",
         "state" : "Ohio",
         "code" : "KBJJ"
      },
      "KILN: Airborne Airpark" : {
         "city" : "Wilmington",
         "name" : "Airborne Airpark",
         "state" : "Ohio",
         "code" : "KILN"
      },
      "KBKL: Cleveland Burke Lakefront Airport" : {
         "city" : "Cleveland",
         "name" : "Cleveland Burke Lakefront Airport",
         "state" : "Ohio",
         "code" : "KBKL"
      }
   },
   "Oklahoma, United States" : {
      "KRVS: Richard Lloyd Jones Jr. Airport" : {
         "city" : "Tulsa County",
         "name" : "Richard Lloyd Jones Jr. Airport",
         "state" : "Oklahoma",
         "code" : "KRVS"
      },
      "KTUL: Tulsa International Airport" : {
         "city" : "Tulsa",
         "name" : "Tulsa International Airport",
         "state" : "Oklahoma",
         "code" : "KTUL"
      },
      "KBFK: Buffalo Municipal Airport (Oklahoma)" : {
         "city" : "Buffalo",
         "name" : "Buffalo Municipal Airport (Oklahoma)",
         "state" : "Oklahoma",
         "code" : "KBFK"
      },
      "KAVK: Alva Regional Airport" : {
         "city" : "Alva",
         "name" : "Alva Regional Airport",
         "state" : "Oklahoma",
         "code" : "KAVK"
      },
      "KCLK: Clinton Regional Airport" : {
         "city" : "Clinton",
         "name" : "Clinton Regional Airport",
         "state" : "Oklahoma",
         "code" : "KCLK"
      },
      "KEND: Vance Air Force Base" : {
         "city" : "Enid",
         "name" : "Vance Air Force Base",
         "state" : "Oklahoma",
         "code" : "KEND"
      },
      "KCUH: Cushing Municipal Airport" : {
         "city" : "Cushing",
         "name" : "Cushing Municipal Airport",
         "state" : "Oklahoma",
         "code" : "KCUH"
      },
      "KCHK: Chickasha Municipal Airport" : {
         "city" : "Chickasha",
         "name" : "Chickasha Municipal Airport",
         "state" : "Oklahoma",
         "code" : "KCHK"
      },
      "KAQR: Atoka Municipal Airport" : {
         "city" : "Atoka",
         "name" : "Atoka Municipal Airport",
         "state" : "Oklahoma",
         "code" : "KAQR"
      },
      "KSNL: Shawnee Regional Airport" : {
         "city" : "Shawnee",
         "name" : "Shawnee Regional Airport",
         "state" : "Oklahoma",
         "code" : "KSNL"
      },
      "KADM: Ardmore Municipal Airport" : {
         "city" : "Ardmore",
         "name" : "Ardmore Municipal Airport",
         "state" : "Oklahoma",
         "code" : "KADM"
      },
      "KFSI: Henry Post Army Airfield" : {
         "city" : "Lawton",
         "name" : "Henry Post Army Airfield",
         "state" : "Oklahoma",
         "code" : "KFSI"
      },
      "KFDR: Frederick Municipal Airport (Oklahoma)" : {
         "city" : "Frederick",
         "name" : "Frederick Municipal Airport (Oklahoma)",
         "state" : "Oklahoma",
         "code" : "KFDR"
      },
      "KCKA: Kegelman Air Force Auxiliary Field" : {
         "city" : "Cherokee",
         "name" : "Kegelman Air Force Auxiliary Field",
         "state" : "Oklahoma",
         "code" : "KCKA"
      },
      "KADH: Ada Municipal Airport" : {
         "city" : "Ada",
         "name" : "Ada Municipal Airport",
         "state" : "Oklahoma",
         "code" : "KADH"
      },
      "KAXS: Altus/Quartz Mountain Regional Airport" : {
         "city" : "Altus",
         "name" : "Altus/Quartz Mountain Regional Airport",
         "state" : "Oklahoma",
         "code" : "KAXS"
      },
      "KDUA: Eaker Field" : {
         "city" : "Durant",
         "name" : "Eaker Field",
         "state" : "Oklahoma",
         "code" : "KDUA"
      },
      "KBVO: Bartlesville Municipal Airport" : {
         "city" : "Bartlesville",
         "name" : "Bartlesville Municipal Airport",
         "state" : "Oklahoma",
         "code" : "KBVO"
      },
      "KOUN: University of Oklahoma Westheimer Airport" : {
         "city" : "Norman",
         "name" : "University of Oklahoma Westheimer Airport",
         "state" : "Oklahoma",
         "code" : "KOUN"
      },
      "KELK: Elk City Municipal Airport" : {
         "city" : "Elk City",
         "name" : "Elk City Municipal Airport",
         "state" : "Oklahoma",
         "code" : "KELK"
      },
      "KOKC: Will Rogers World Airport" : {
         "city" : "Oklahoma City",
         "name" : "Will Rogers World Airport",
         "state" : "Oklahoma",
         "code" : "KOKC"
      },
      "KDUC: Halliburton Field (airport)" : {
         "city" : "Duncan",
         "name" : "Halliburton Field (airport)",
         "state" : "Oklahoma",
         "code" : "KDUC"
      },
      "KCSM: Clinton-Sherman Airport" : {
         "city" : "Clinton",
         "name" : "Clinton-Sherman Airport",
         "state" : "Oklahoma",
         "code" : "KCSM"
      },
      "KTIK: Tinker Air Force Base" : {
         "city" : "Oklahoma City",
         "name" : "Tinker Air Force Base",
         "state" : "Oklahoma",
         "code" : "KTIK"
      },
      "KCQB: Chandler Regional Airport" : {
         "city" : "Chandler",
         "name" : "Chandler Regional Airport",
         "state" : "Oklahoma",
         "code" : "KCQB"
      }
   },
   "Utah, United States" : {
      "KVEL: Vernal-Uintah County Airport" : {
         "city" : "Vernal",
         "name" : "Vernal-Uintah County Airport",
         "state" : "Utah",
         "code" : "KVEL"
      },
      "KKNB: Kanab Municipal Airport" : {
         "city" : "Kanab",
         "name" : "Kanab Municipal Airport",
         "state" : "Utah",
         "code" : "KKNB"
      },
      "KBDG: Blanding Municipal Airport" : {
         "city" : "Blanding",
         "name" : "Blanding Municipal Airport",
         "state" : "Utah",
         "code" : "KBDG"
      },
      "KPVU: Provo Municipal Airport" : {
         "city" : "Provo",
         "name" : "Provo Municipal Airport",
         "state" : "Utah",
         "code" : "KPVU"
      },
      "KPUC: Carbon County Airport" : {
         "city" : "Price",
         "name" : "Carbon County Airport",
         "state" : "Utah",
         "code" : "KPUC"
      },
      "KBTF: Skypark Airport" : {
         "city" : "Bountiful",
         "name" : "Skypark Airport",
         "state" : "Utah",
         "code" : "KBTF"
      },
      "KMLF: Milford Municipal Airport" : {
         "city" : "Milford",
         "name" : "Milford Municipal Airport",
         "state" : "Utah",
         "code" : "KMLF"
      },
      "KBMC: Brigham City Airport" : {
         "city" : "Brigham City",
         "name" : "Brigham City Airport",
         "state" : "Utah",
         "code" : "KBMC"
      },
      "KTVY: Bolinder Field-Tooele Valley Airport" : {
         "city" : "Tooele",
         "name" : "Bolinder Field-Tooele Valley Airport",
         "state" : "Utah",
         "code" : "KTVY"
      },
      "KOGD: Ogden-Hinckley Airport" : {
         "city" : "Ogden",
         "name" : "Ogden-Hinckley Airport",
         "state" : "Utah",
         "code" : "KOGD"
      },
      "KDTA: Delta Municipal Airport" : {
         "city" : "Delta",
         "name" : "Delta Municipal Airport",
         "state" : "Utah",
         "code" : "KDTA"
      },
      "KSLC: Salt Lake City International Airport" : {
         "city" : "Salt Lake City",
         "name" : "Salt Lake City International Airport",
         "state" : "Utah",
         "code" : "KSLC"
      },
      "KSGU: St. George Municipal Airport" : {
         "city" : "St. George",
         "name" : "St. George Municipal Airport",
         "state" : "Utah",
         "code" : "KSGU"
      },
      "KBCE: Bryce Canyon Airport" : {
         "city" : "Bryce Canyon",
         "name" : "Bryce Canyon Airport",
         "state" : "Utah",
         "code" : "KBCE"
      },
      "KRIF: Richfield Municipal Airport" : {
         "city" : "Richfield",
         "name" : "Richfield Municipal Airport",
         "state" : "Utah",
         "code" : "KRIF"
      },
      "KENV: Wendover Airport" : {
         "city" : "Wendover",
         "name" : "Wendover Airport",
         "state" : "Utah",
         "code" : "KENV"
      },
      "KCNY: Canyonlands Field" : {
         "city" : "Moab",
         "name" : "Canyonlands Field",
         "state" : "Utah",
         "code" : "KCNY"
      },
      "KFOM: Fillmore Municipal Airport" : {
         "city" : "Fillmore",
         "name" : "Fillmore Municipal Airport",
         "state" : "Utah",
         "code" : "KFOM"
      },
      "KDPG: Michael Army Airfield" : {
         "city" : "Michael Army Airfield – Dugway Proving Ground",
         "name" : "Michael Army Airfield",
         "state" : "Utah",
         "code" : "KDPG"
      },
      "KHIF: Hill Air Force Base" : {
         "city" : "Ogden",
         "name" : "Hill Air Force Base",
         "state" : "Utah",
         "code" : "KHIF"
      },
      "KLGU: Logan-Cache Airport" : {
         "city" : "Logan",
         "name" : "Logan-Cache Airport",
         "state" : "Utah",
         "code" : "KLGU"
      },
      "KHVE: Hanksville Airport" : {
         "city" : "Hanksville",
         "name" : "Hanksville Airport",
         "state" : "Utah",
         "code" : "KHVE"
      },
      "KCDC: Cedar City Regional Airport" : {
         "city" : "Cedar City",
         "name" : "Cedar City Regional Airport",
         "state" : "Utah",
         "code" : "KCDC"
      }
   },
   "Alabama, United States" : {
      "KMGM: Montgomery Regional Airport" : {
         "city" : "Montgomery",
         "name" : "Montgomery Regional Airport",
         "state" : "Alabama",
         "code" : "KMGM"
      },
      "KGZH: Middleton Airport" : {
         "city" : "Evergreen",
         "name" : "Middleton Airport",
         "state" : "Alabama",
         "code" : "KGZH"
      },
      "KMSL: Northwest Alabama Regional Airport" : {
         "city" : "Muscle Shoals",
         "name" : "Northwest Alabama Regional Airport",
         "state" : "Alabama",
         "code" : "KMSL"
      },
      "KHUA: Redstone Army Airfield" : {
         "city" : "Huntsville",
         "name" : "Redstone Army Airfield",
         "state" : "Alabama",
         "code" : "KHUA"
      },
      "KEKY: Bessemer Airport" : {
         "city" : "Bessemer",
         "name" : "Bessemer Airport",
         "state" : "Alabama",
         "code" : "KEKY"
      },
      "KASN: Talladega Municipal Airport" : {
         "city" : "Talladega",
         "name" : "Talladega Municipal Airport",
         "state" : "Alabama",
         "code" : "KASN"
      },
      "KJFX: Walker County Airport" : {
         "city" : "Jasper",
         "name" : "Walker County Airport",
         "state" : "Alabama",
         "code" : "KJFX"
      },
      "KAIV: George Downer Airport" : {
         "city" : "Aliceville",
         "name" : "George Downer Airport",
         "state" : "Alabama",
         "code" : "KAIV"
      },
      "KMOB: Mobile Regional Airport" : {
         "city" : "Mobile",
         "name" : "Mobile Regional Airport",
         "state" : "Alabama",
         "code" : "KMOB"
      },
      "KEDN: Enterprise Municipal Airport (Alabama)" : {
         "city" : "Enterprise",
         "name" : "Enterprise Municipal Airport (Alabama)",
         "state" : "Alabama",
         "code" : "KEDN"
      },
      "KGAD: Northeast Alabama Regional Airport" : {
         "city" : "Gadsden",
         "name" : "Northeast Alabama Regional Airport",
         "state" : "Alabama",
         "code" : "KGAD"
      },
      "KBHM: Birmingham International Airport (US)" : {
         "city" : "Birmingham",
         "name" : "Birmingham International Airport (US)",
         "state" : "Alabama",
         "code" : "KBHM"
      },
      "KMVC: Monroe County Airport (Alabama)" : {
         "city" : "Monroeville",
         "name" : "Monroe County Airport (Alabama)",
         "state" : "Alabama",
         "code" : "KMVC"
      },
      "KHSV: Huntsville International Airport" : {
         "city" : "Huntsville",
         "name" : "Huntsville International Airport",
         "state" : "Alabama",
         "code" : "KHSV"
      },
      "KEUF: Weedon Field" : {
         "city" : "Eufaula",
         "name" : "Weedon Field",
         "state" : "Alabama",
         "code" : "KEUF"
      },
      "KSCD: Sylacauga Municipal Airport" : {
         "city" : "Sylacauga",
         "name" : "Sylacauga Municipal Airport",
         "state" : "Alabama",
         "code" : "KSCD"
      },
      "KPRN: Mac Crenshaw Memorial Airport" : {
         "city" : "Greenville",
         "name" : "Mac Crenshaw Memorial Airport",
         "state" : "Alabama",
         "code" : "KPRN"
      },
      "KTOI: Troy Municipal Airport" : {
         "city" : "Troy",
         "name" : "Troy Municipal Airport",
         "state" : "Alabama",
         "code" : "KTOI"
      },
      "KEET: Shelby County Airport (Alabama)" : {
         "city" : "Alabaster",
         "name" : "Shelby County Airport (Alabama)",
         "state" : "Alabama",
         "code" : "KEET"
      },
      "KTCL: Tuscaloosa Regional Airport" : {
         "city" : "Tuscaloosa",
         "name" : "Tuscaloosa Regional Airport",
         "state" : "Alabama",
         "code" : "KTCL"
      },
      "KDCU: Pryor Field Regional Airport" : {
         "city" : "Decatur",
         "name" : "Pryor Field Regional Airport",
         "state" : "Alabama",
         "code" : "KDCU"
      },
      "KMDQ: Madison County Executive Airport" : {
         "city" : "Huntsville",
         "name" : "Madison County Executive Airport",
         "state" : "Alabama",
         "code" : "KMDQ"
      },
      "KMXF: Maxwell Air Force Base" : {
         "city" : "Montgomery",
         "name" : "Maxwell Air Force Base",
         "state" : "Alabama",
         "code" : "KMXF"
      },
      "KHEY: Hanchey Army Heliport" : {
         "city" : "Dale County",
         "name" : "Hanchey Army Heliport",
         "state" : "Alabama",
         "code" : "KHEY"
      },
      "KALX: Thomas C. Russell Field" : {
         "city" : "Alexander City",
         "name" : "Thomas C. Russell Field",
         "state" : "Alabama",
         "code" : "KALX"
      },
      "KANB: Anniston Metropolitan Airport" : {
         "city" : "Anniston",
         "name" : "Anniston Metropolitan Airport",
         "state" : "Alabama",
         "code" : "KANB"
      },
      "KOZR: Cairns Army Airfield" : {
         "city" : "Cairns Army Airfield – Fort Rucker",
         "name" : "Cairns Army Airfield",
         "state" : "Alabama",
         "code" : "KOZR"
      },
      "KPLR: St. Clair County Airport" : {
         "city" : "Pell City",
         "name" : "St. Clair County Airport",
         "state" : "Alabama",
         "code" : "KPLR"
      },
      "KAUO: Auburn-Opelika Robert G. Pitts Airport" : {
         "city" : "Auburn",
         "name" : "Auburn-Opelika Robert G. Pitts Airport",
         "state" : "Alabama",
         "code" : "KAUO"
      },
      "KHAB: Marion County - Rankin Fite Airport" : {
         "city" : "Hamilton",
         "name" : "Marion County - Rankin Fite Airport",
         "state" : "Alabama",
         "code" : "KHAB"
      },
      "KSEM: Craig Field (Alabama)" : {
         "city" : "Selma",
         "name" : "Craig Field (Alabama)",
         "state" : "Alabama",
         "code" : "KSEM"
      },
      "KJKA: Jack Edwards Airport" : {
         "city" : "Gulf Shores",
         "name" : "Jack Edwards Airport",
         "state" : "Alabama",
         "code" : "KJKA"
      },
      "KDHN: Dothan Regional Airport" : {
         "city" : "Dothan",
         "name" : "Dothan Regional Airport",
         "state" : "Alabama",
         "code" : "KDHN"
      },
      "KBFM: Mobile Downtown Airport" : {
         "city" : "Mobile",
         "name" : "Mobile Downtown Airport",
         "state" : "Alabama",
         "code" : "KBFM"
      }
   },
   "South Dakota, United States" : {
      "KEIN: Pine Ridge Airport" : {
         "city" : "Pine Ridge",
         "name" : "Pine Ridge Airport",
         "state" : "South Dakota",
         "code" : "KEIN"
      },
      "KSFD: Wiley Airport" : {
         "city" : "Winner",
         "name" : "Wiley Airport",
         "state" : "South Dakota",
         "code" : "KSFD"
      },
      "KHON: Huron Regional Airport" : {
         "city" : "Huron",
         "name" : "Huron Regional Airport",
         "state" : "South Dakota",
         "code" : "KHON"
      },
      "KICR: Winner Regional Airport" : {
         "city" : "Winner",
         "name" : "Winner Regional Airport",
         "state" : "South Dakota",
         "code" : "KICR"
      },
      "KPHP: Philip Airport" : {
         "city" : "Philip",
         "name" : "Philip Airport",
         "state" : "South Dakota",
         "code" : "KPHP"
      },
      "KRCA: Ellsworth Air Force Base" : {
         "city" : "Rapid City",
         "name" : "Ellsworth Air Force Base",
         "state" : "South Dakota",
         "code" : "KRCA"
      },
      "KAGZ: Wagner Municipal Airport" : {
         "city" : "Wagner",
         "name" : "Wagner Municipal Airport",
         "state" : "South Dakota",
         "code" : "KAGZ"
      },
      "KSPF: Black Hills Airport" : {
         "city" : "Spearfish",
         "name" : "Black Hills Airport",
         "state" : "South Dakota",
         "code" : "KSPF"
      },
      "KBTN: Britton Municipal Airport" : {
         "city" : "Britton",
         "name" : "Britton Municipal Airport",
         "state" : "South Dakota",
         "code" : "KBTN"
      },
      "KABR: Aberdeen Regional Airport" : {
         "city" : "Aberdeen",
         "name" : "Aberdeen Regional Airport",
         "state" : "South Dakota",
         "code" : "KABR"
      },
      "KBKX: Brookings Municipal Airport" : {
         "city" : "Brookings",
         "name" : "Brookings Municipal Airport",
         "state" : "South Dakota",
         "code" : "KBKX"
      },
      "KVMR: Davidson Airport" : {
         "city" : "Vermillion",
         "name" : "Davidson Airport",
         "state" : "South Dakota",
         "code" : "KVMR"
      },
      "KHSR: Hot Springs Municipal Airport" : {
         "city" : "Hot Springs",
         "name" : "Hot Springs Municipal Airport",
         "state" : "South Dakota",
         "code" : "KHSR"
      },
      "KRAP: Rapid City Regional Airport" : {
         "city" : "Rapid City",
         "name" : "Rapid City Regional Airport",
         "state" : "South Dakota",
         "code" : "KRAP"
      },
      "KYKN: Chan Gurney Municipal Airport" : {
         "city" : "Yankton",
         "name" : "Chan Gurney Municipal Airport",
         "state" : "South Dakota",
         "code" : "KYKN"
      },
      "KMHE: Mitchell Municipal Airport" : {
         "city" : "Mitchell",
         "name" : "Mitchell Municipal Airport",
         "state" : "South Dakota",
         "code" : "KMHE"
      },
      "KLEM: Lemmon Municipal Airport" : {
         "city" : "Lemmon",
         "name" : "Lemmon Municipal Airport",
         "state" : "South Dakota",
         "code" : "KLEM"
      },
      "KMKA: Miller Municipal Airport" : {
         "city" : "Miller",
         "name" : "Miller Municipal Airport",
         "state" : "South Dakota",
         "code" : "KMKA"
      },
      "KMDS: Madison Municipal Airport (South Dakota)" : {
         "city" : "Madison",
         "name" : "Madison Municipal Airport (South Dakota)",
         "state" : "South Dakota",
         "code" : "KMDS"
      },
      "KPIR: Pierre Regional Airport" : {
         "city" : "Pierre",
         "name" : "Pierre Regional Airport",
         "state" : "South Dakota",
         "code" : "KPIR"
      },
      "KCUT: Custer County Airport" : {
         "city" : "Custer",
         "name" : "Custer County Airport",
         "state" : "South Dakota",
         "code" : "KCUT"
      },
      "KATY: Watertown Regional Airport" : {
         "city" : "Watertown",
         "name" : "Watertown Regional Airport",
         "state" : "South Dakota",
         "code" : "KATY"
      },
      "KEFC: Belle Fourche Municipal Airport" : {
         "city" : "Belle Fourche",
         "name" : "Belle Fourche Municipal Airport",
         "state" : "South Dakota",
         "code" : "KEFC"
      },
      "KFSD: Sioux Falls Regional Airport" : {
         "city" : "Sioux Falls",
         "name" : "Sioux Falls Regional Airport",
         "state" : "South Dakota",
         "code" : "KFSD"
      },
      "KMBG: Mobridge Municipal Airport" : {
         "city" : "Mobridge",
         "name" : "Mobridge Municipal Airport",
         "state" : "South Dakota",
         "code" : "KMBG"
      }
   },
   "Nevada, United States" : {
      "KMEV: Minden-Tahoe Airport" : {
         "city" : "Minden",
         "name" : "Minden-Tahoe Airport",
         "state" : "Nevada",
         "code" : "KMEV"
      },
      "KRNO: Reno-Tahoe International Airport" : {
         "city" : "Reno",
         "name" : "Reno-Tahoe International Airport",
         "state" : "Nevada",
         "code" : "KRNO"
      },
      "KVGT: North Las Vegas Airport" : {
         "city" : "Las Vegas",
         "name" : "North Las Vegas Airport",
         "state" : "Nevada",
         "code" : "KVGT"
      },
      "KFLX: Fallon Municipal Airport" : {
         "city" : "Fallon",
         "name" : "Fallon Municipal Airport",
         "state" : "Nevada",
         "code" : "KFLX"
      },
      "KLSV: Nellis Air Force Base" : {
         "city" : "Las Vegas",
         "name" : "Nellis Air Force Base",
         "state" : "Nevada",
         "code" : "KLSV"
      },
      "KLOL: Derby Airport" : {
         "city" : "Lovelock",
         "name" : "Derby Airport",
         "state" : "Nevada",
         "code" : "KLOL"
      },
      "KELY: Ely Airport" : {
         "city" : "Ely",
         "name" : "Ely Airport",
         "state" : "Nevada",
         "code" : "KELY"
      },
      "KGAB: Gabbs Airport" : {
         "city" : "Gabbs",
         "name" : "Gabbs Airport",
         "state" : "Nevada",
         "code" : "KGAB"
      },
      "KDRA: Desert Rock Airport" : {
         "city" : "Mercury",
         "name" : "Desert Rock Airport",
         "state" : "Nevada",
         "code" : "KDRA"
      },
      "KLWL: Wells Municipal Airport (Nevada)" : {
         "city" : "Wells",
         "name" : "Wells Municipal Airport (Nevada)",
         "state" : "Nevada",
         "code" : "KLWL"
      },
      "KTNX: Tonopah Test Range Airport" : {
         "city" : "Tonopah",
         "name" : "Tonopah Test Range Airport",
         "state" : "Nevada",
         "code" : "KTNX"
      },
      "KINS: Creech Air Force Base" : {
         "city" : "Indian Springs",
         "name" : "Creech Air Force Base",
         "state" : "Nevada",
         "code" : "KINS"
      },
      "KHND: Henderson Executive Airport" : {
         "city" : "Henderson",
         "name" : "Henderson Executive Airport",
         "state" : "Nevada",
         "code" : "KHND"
      },
      "KLAS: McCarran International Airport" : {
         "city" : "Las Vegas",
         "name" : "McCarran International Airport",
         "state" : "Nevada",
         "code" : "KLAS"
      },
      "KBTY: Beatty Airport" : {
         "city" : "Beatty",
         "name" : "Beatty Airport",
         "state" : "Nevada",
         "code" : "KBTY"
      },
      "KWMC: Winnemucca Municipal Airport" : {
         "city" : "Winnemucca",
         "name" : "Winnemucca Municipal Airport",
         "state" : "Nevada",
         "code" : "KWMC"
      },
      "KBAM: Battle Mountain Airport" : {
         "city" : "Battle Mountain",
         "name" : "Battle Mountain Airport",
         "state" : "Nevada",
         "code" : "KBAM"
      },
      "KCXP: Carson City Airport" : {
         "city" : "Carson City",
         "name" : "Carson City Airport",
         "state" : "Nevada",
         "code" : "KCXP"
      },
      "KHTH: Hawthorne Industrial Airport" : {
         "city" : "Hawthorne",
         "name" : "Hawthorne Industrial Airport",
         "state" : "Nevada",
         "code" : "KHTH"
      },
      "KXTA: Area 51" : {
         "city" : "Groom Lake",
         "name" : "Area 51",
         "state" : "Nevada",
         "code" : "KXTA"
      },
      "KEKO: Elko Regional Airport" : {
         "city" : "Elko",
         "name" : "Elko Regional Airport",
         "state" : "Nevada",
         "code" : "KEKO"
      },
      "KTPH: Tonopah Airport" : {
         "city" : "Tonopah",
         "name" : "Tonopah Airport",
         "state" : "Nevada",
         "code" : "KTPH"
      },
      "KNFL: Naval Air Station Fallon" : {
         "city" : "Fallon",
         "name" : "Naval Air Station Fallon",
         "state" : "Nevada",
         "code" : "KNFL"
      }
   },
   "Oregon, United States" : {
      "KLGD: La Grande/Union County Airport" : {
         "city" : "La Grande",
         "name" : "La Grande/Union County Airport",
         "state" : "Oregon",
         "code" : "KLGD"
      },
      "KCZK: Cascade Locks State Airport" : {
         "city" : "Cascade Locks",
         "name" : "Cascade Locks State Airport",
         "state" : "Oregon",
         "code" : "KCZK"
      },
      "KDLS: Columbia Gorge Regional Airport" : {
         "city" : "The Dalles",
         "name" : "Columbia Gorge Regional Airport",
         "state" : "Oregon",
         "code" : "KDLS"
      },
      "KRBG: Roseburg Regional Airport" : {
         "city" : "Roseburg",
         "name" : "Roseburg Regional Airport",
         "state" : "Oregon",
         "code" : "KRBG"
      },
      "KPFC: Pacific City State Airport" : {
         "city" : "Pacific City",
         "name" : "Pacific City State Airport",
         "state" : "Oregon",
         "code" : "KPFC"
      },
      "KHIO: Hillsboro Airport" : {
         "city" : "Portland",
         "name" : "Hillsboro Airport",
         "state" : "Oregon",
         "code" : "KHIO"
      },
      "KRDM: Roberts Field" : {
         "city" : "Redmond",
         "name" : "Roberts Field",
         "state" : "Oregon",
         "code" : "KRDM"
      },
      "KSPB: Scappoose Industrial Airpark" : {
         "city" : "Scappoose",
         "name" : "Scappoose Industrial Airpark",
         "state" : "Oregon",
         "code" : "KSPB"
      },
      "KMFR: Rogue Valley International-Medford Airport" : {
         "city" : "Medford",
         "name" : "Rogue Valley International-Medford Airport",
         "state" : "Oregon",
         "code" : "KMFR"
      },
      "KCVO: Corvallis Municipal Airport" : {
         "city" : "Corvallis",
         "name" : "Corvallis Municipal Airport",
         "state" : "Oregon",
         "code" : "KCVO"
      },
      "KBKE: Baker City Municipal Airport" : {
         "city" : "Baker City",
         "name" : "Baker City Municipal Airport",
         "state" : "Oregon",
         "code" : "KBKE"
      },
      "KONO: Ontario Municipal Airport" : {
         "city" : "Ontario",
         "name" : "Ontario Municipal Airport",
         "state" : "Oregon",
         "code" : "KONO"
      },
      "KOTH: North Bend Municipal Airport" : {
         "city" : "North Bend",
         "name" : "North Bend Municipal Airport",
         "state" : "Oregon",
         "code" : "KOTH"
      },
      "KONP: Newport Municipal Airport (Oregon)" : {
         "city" : "Newport",
         "name" : "Newport Municipal Airport (Oregon)",
         "state" : "Oregon",
         "code" : "KONP"
      },
      "KBNO: Burns Municipal Airport" : {
         "city" : "Burns",
         "name" : "Burns Municipal Airport",
         "state" : "Oregon",
         "code" : "KBNO"
      },
      "KTTD: Troutdale Airport" : {
         "city" : "Portland",
         "name" : "Troutdale Airport",
         "state" : "Oregon",
         "code" : "KTTD"
      },
      "KUAO: Aurora State Airport" : {
         "city" : "Aurora",
         "name" : "Aurora State Airport",
         "state" : "Oregon",
         "code" : "KUAO"
      },
      "KLKV: Lake County Airport (Oregon)" : {
         "city" : "Lakeview",
         "name" : "Lake County Airport (Oregon)",
         "state" : "Oregon",
         "code" : "KLKV"
      },
      "KREO: Rome State Airport" : {
         "city" : "Rome",
         "name" : "Rome State Airport",
         "state" : "Oregon",
         "code" : "KREO"
      },
      "KEUG: Eugene Airport" : {
         "city" : "Eugene",
         "name" : "Eugene Airport",
         "state" : "Oregon",
         "code" : "KEUG"
      },
      "KBOK: Brookings State Airport" : {
         "city" : "Brookings",
         "name" : "Brookings State Airport",
         "state" : "Oregon",
         "code" : "KBOK"
      },
      "KMMV: McMinnville Municipal Airport" : {
         "city" : "McMinnville",
         "name" : "McMinnville Municipal Airport",
         "state" : "Oregon",
         "code" : "KMMV"
      },
      "KLMT: Klamath Falls Airport" : {
         "city" : "Klamath Falls",
         "name" : "Klamath Falls Airport",
         "state" : "Oregon",
         "code" : "KLMT"
      },
      "KSLE: McNary Field" : {
         "city" : "Salem",
         "name" : "McNary Field",
         "state" : "Oregon",
         "code" : "KSLE"
      },
      "KHRI: Hermiston Municipal Airport" : {
         "city" : "Hermiston",
         "name" : "Hermiston Municipal Airport",
         "state" : "Oregon",
         "code" : "KHRI"
      },
      "KAST: Astoria Regional Airport" : {
         "city" : "Astoria",
         "name" : "Astoria Regional Airport",
         "state" : "Oregon",
         "code" : "KAST"
      },
      "KPDX: Portland International Airport" : {
         "city" : "Portland",
         "name" : "Portland International Airport",
         "state" : "Oregon",
         "code" : "KPDX"
      },
      "KPDT: Eastern Oregon Regional Airport" : {
         "city" : "Pendleton",
         "name" : "Eastern Oregon Regional Airport",
         "state" : "Oregon",
         "code" : "KPDT"
      }
   },
   "Wyoming, United States" : {
      "KTHP: Hot Springs County Municipal Airport" : {
         "city" : "Thermopolis",
         "name" : "Hot Springs County Municipal Airport",
         "state" : "Wyoming",
         "code" : "KTHP"
      },
      "KJAC: Jackson Hole Airport" : {
         "city" : "Jackson Hole",
         "name" : "Jackson Hole Airport",
         "state" : "Wyoming",
         "code" : "KJAC"
      },
      "KBYG: Johnson County Airport (Wyoming)" : {
         "city" : "Buffalo",
         "name" : "Johnson County Airport (Wyoming)",
         "state" : "Wyoming",
         "code" : "KBYG"
      },
      "KEVW: Evanston-Uinta County Airport" : {
         "city" : "Evanston",
         "name" : "Evanston-Uinta County Airport",
         "state" : "Wyoming",
         "code" : "KEVW"
      },
      "KGEY: South Big Horn County Airport" : {
         "city" : "Greybull",
         "name" : "South Big Horn County Airport",
         "state" : "Wyoming",
         "code" : "KGEY"
      },
      "KWRL: Worland Municipal Airport" : {
         "city" : "Worland",
         "name" : "Worland Municipal Airport",
         "state" : "Wyoming",
         "code" : "KWRL"
      },
      "KLND: Hunt Field" : {
         "city" : "Lander",
         "name" : "Hunt Field",
         "state" : "Wyoming",
         "code" : "KLND"
      },
      "KEMM: Kemmerer Municipal Airport" : {
         "city" : "Kemmerer",
         "name" : "Kemmerer Municipal Airport",
         "state" : "Wyoming",
         "code" : "KEMM"
      },
      "KCPR: Natrona County International Airport" : {
         "city" : "Casper",
         "name" : "Natrona County International Airport",
         "state" : "Wyoming",
         "code" : "KCPR"
      },
      "KRWL: Rawlins Municipal Airport" : {
         "city" : "Rawlins",
         "name" : "Rawlins Municipal Airport",
         "state" : "Wyoming",
         "code" : "KRWL"
      },
      "KPNA: Wenz Airport" : {
         "city" : "Pinedale",
         "name" : "Wenz Airport",
         "state" : "Wyoming",
         "code" : "KPNA"
      },
      "KDGW: Converse County Airport" : {
         "city" : "Douglas",
         "name" : "Converse County Airport",
         "state" : "Wyoming",
         "code" : "KDGW"
      },
      "KEAN: Phifer Airfield" : {
         "city" : "Wheatland",
         "name" : "Phifer Airfield",
         "state" : "Wyoming",
         "code" : "KEAN"
      },
      "KECS: Mondell Field" : {
         "city" : "Newcastle",
         "name" : "Mondell Field",
         "state" : "Wyoming",
         "code" : "KECS"
      },
      "KSHR: Sheridan County Airport" : {
         "city" : "Sheridan",
         "name" : "Sheridan County Airport",
         "state" : "Wyoming",
         "code" : "KSHR"
      },
      "KLSK: Lusk Municipal Airport" : {
         "city" : "Lusk",
         "name" : "Lusk Municipal Airport",
         "state" : "Wyoming",
         "code" : "KLSK"
      },
      "KHAR: Harford Airport" : {
         "city" : "Casper",
         "name" : "Harford Airport",
         "state" : "Wyoming",
         "code" : "KHAR"
      },
      "KCYS: Cheyenne Regional Airport" : {
         "city" : "Cheyenne",
         "name" : "Cheyenne Regional Airport",
         "state" : "Wyoming",
         "code" : "KCYS"
      },
      "KRIW: Riverton Regional Airport" : {
         "city" : "Riverton",
         "name" : "Riverton Regional Airport",
         "state" : "Wyoming",
         "code" : "KRIW"
      },
      "KAFO: Afton Municipal Airport" : {
         "city" : "Afton",
         "name" : "Afton Municipal Airport",
         "state" : "Wyoming",
         "code" : "KAFO"
      },
      "KBPI: Big Piney-Marbleton Airport" : {
         "city" : "Big Piney",
         "name" : "Big Piney-Marbleton Airport",
         "state" : "Wyoming",
         "code" : "KBPI"
      },
      "KCOD: Yellowstone Regional Airport" : {
         "city" : "Cody",
         "name" : "Yellowstone Regional Airport",
         "state" : "Wyoming",
         "code" : "KCOD"
      },
      "KLAR: Laramie Regional Airport" : {
         "city" : "Laramie",
         "name" : "Laramie Regional Airport",
         "state" : "Wyoming",
         "code" : "KLAR"
      },
      "KFBR: Fort Bridger Airport" : {
         "city" : "Fort Bridger",
         "name" : "Fort Bridger Airport",
         "state" : "Wyoming",
         "code" : "KFBR"
      },
      "KTOR: Torrington Municipal Airport" : {
         "city" : "Torrington",
         "name" : "Torrington Municipal Airport",
         "state" : "Wyoming",
         "code" : "KTOR"
      },
      "KSAA: Shively Airport" : {
         "city" : "Saratoga",
         "name" : "Shively Airport",
         "state" : "Wyoming",
         "code" : "KSAA"
      },
      "KPOY: Powell Municipal Airport" : {
         "city" : "Powell",
         "name" : "Powell Municipal Airport",
         "state" : "Wyoming",
         "code" : "KPOY"
      },
      "KGCC: Gillette-Campbell County Airport" : {
         "city" : "Gillette",
         "name" : "Gillette-Campbell County Airport",
         "state" : "Wyoming",
         "code" : "KGCC"
      },
      "KRKS: Rock Springs - Sweetwater County Airport" : {
         "city" : "Rock Springs",
         "name" : "Rock Springs - Sweetwater County Airport",
         "state" : "Wyoming",
         "code" : "KRKS"
      }
   },
   "New Hampshire, United States" : {
      "KEEN: Dillant-Hopkins Airport" : {
         "city" : "Keene",
         "name" : "Dillant-Hopkins Airport",
         "state" : "New Hampshire",
         "code" : "KEEN"
      },
      "KPSM: Portsmouth International Airport at Pease" : {
         "city" : "Portsmouth",
         "name" : "Portsmouth International Airport at Pease",
         "state" : "New Hampshire",
         "code" : "KPSM"
      },
      "KCON: Concord Municipal Airport" : {
         "city" : "Concord",
         "name" : "Concord Municipal Airport",
         "state" : "New Hampshire",
         "code" : "KCON"
      },
      "KHIE: Mount Washington Regional Airport" : {
         "city" : "Whitefield",
         "name" : "Mount Washington Regional Airport",
         "state" : "New Hampshire",
         "code" : "KHIE"
      },
      "KCNH: Claremont Municipal Airport" : {
         "city" : "Claremont",
         "name" : "Claremont Municipal Airport",
         "state" : "New Hampshire",
         "code" : "KCNH"
      },
      "KLCI: Laconia Municipal Airport" : {
         "city" : "Laconia",
         "name" : "Laconia Municipal Airport",
         "state" : "New Hampshire",
         "code" : "KLCI"
      },
      "KAFN: Jaffrey Airport - Silver Ranch Airpark" : {
         "city" : "Jaffrey",
         "name" : "Jaffrey Airport - Silver Ranch Airpark",
         "state" : "New Hampshire",
         "code" : "KAFN"
      },
      "KERR: Errol Airport" : {
         "city" : "Errol",
         "name" : "Errol Airport",
         "state" : "New Hampshire",
         "code" : "KERR"
      },
      "KLEB: Lebanon Municipal Airport (New Hampshire)" : {
         "city" : "West Lebanon",
         "name" : "Lebanon Municipal Airport (New Hampshire)",
         "state" : "New Hampshire",
         "code" : "KLEB"
      },
      "KASH: Boire Field" : {
         "city" : "Nashua",
         "name" : "Boire Field",
         "state" : "New Hampshire",
         "code" : "KASH"
      },
      "KMHT: Manchester-Boston Regional Airport" : {
         "city" : "Manchester",
         "name" : "Manchester-Boston Regional Airport",
         "state" : "New Hampshire",
         "code" : "KMHT"
      },
      "KBML: Berlin Municipal Airport" : {
         "city" : "Berlin",
         "name" : "Berlin Municipal Airport",
         "state" : "New Hampshire",
         "code" : "KBML"
      },
      "KDAW: Skyhaven Airport (New Hampshire)" : {
         "city" : "Rochester",
         "name" : "Skyhaven Airport (New Hampshire)",
         "state" : "New Hampshire",
         "code" : "KDAW"
      }
   },
   "Iraq" : {
      "KQSL: Camp Speicher" : {
         "city" : "Al Sahra Airfield",
         "name" : "Camp Speicher",
         "state" : "Iraq",
         "code" : "KQSL"
      },
      "KQMA: Camp Fallujah" : {
         "city" : "Camp Fallujah",
         "name" : "Camp Fallujah",
         "state" : "Iraq",
         "code" : "KQMA"
      },
      "KQAJ: Al Asad Airbase" : {
         "city" : "Al Asad AB",
         "name" : "Al Asad Airbase",
         "state" : "Iraq",
         "code" : "KQAJ"
      },
      "KQAE: Musayyib" : {
         "city" : "al Musayyib FOB",
         "name" : "Musayyib",
         "state" : "Iraq",
         "code" : "KQAE"
      },
      "KQAQ: Camp Taji" : {
         "city" : "Camp Taji",
         "name" : "Camp Taji",
         "state" : "Iraq",
         "code" : "KQAQ"
      },
      "KQTZ: Baghdad International Airport" : {
         "city" : "Baghdad International Airport",
         "name" : "Baghdad International Airport",
         "state" : "Iraq",
         "code" : "KQTZ"
      },
      "KQEZ: Al Taqaddum Airbase" : {
         "city" : "Al Taqaddum AB",
         "name" : "Al Taqaddum Airbase",
         "state" : "Iraq",
         "code" : "KQEZ"
      },
      "KQNY: Ramadi" : {
         "city" : "Camp Ramadi",
         "name" : "Ramadi",
         "state" : "Iraq",
         "code" : "KQNY"
      }
   },
   "Illinois, United States" : {
      "KBLV: MidAmerica St. Louis Airport" : {
         "city" : "Belleville",
         "name" : "MidAmerica St. Louis Airport",
         "state" : "Illinois",
         "code" : "KBLV"
      },
      "KFWC: Fairfield Municipal Airport (Illinois)" : {
         "city" : "Fairfield",
         "name" : "Fairfield Municipal Airport (Illinois)",
         "state" : "Illinois",
         "code" : "KFWC"
      },
      "KDKB: DeKalb Taylor Municipal Airport" : {
         "city" : "DeKalb",
         "name" : "DeKalb Taylor Municipal Airport",
         "state" : "Illinois",
         "code" : "KDKB"
      },
      "KENL: Centralia Municipal Airport" : {
         "city" : "Centralia",
         "name" : "Centralia Municipal Airport",
         "state" : "Illinois",
         "code" : "KENL"
      },
      "KEZI: Kewanee Municipal Airport" : {
         "city" : "Kewanee",
         "name" : "Kewanee Municipal Airport",
         "state" : "Illinois",
         "code" : "KEZI"
      },
      "KALN: St. Louis Regional Airport" : {
         "city" : "Alton",
         "name" : "St. Louis Regional Airport",
         "state" : "Illinois",
         "code" : "KALN"
      },
      "KFOA: Flora Municipal Airport" : {
         "city" : "Flora",
         "name" : "Flora Municipal Airport",
         "state" : "Illinois",
         "code" : "KFOA"
      },
      "KVYS: Illinois Valley Regional Airport" : {
         "city" : "Peru",
         "name" : "Illinois Valley Regional Airport",
         "state" : "Illinois",
         "code" : "KVYS"
      },
      "KDPA: Dupage Airport" : {
         "city" : "West Chicago",
         "name" : "Dupage Airport",
         "state" : "Illinois",
         "code" : "KDPA"
      },
      "KDNV: Vermilion County Airport" : {
         "city" : "Danville",
         "name" : "Vermilion County Airport",
         "state" : "Illinois",
         "code" : "KDNV"
      },
      "KORD: O'Hare International Airport" : {
         "city" : "Chicago",
         "name" : "O'Hare International Airport",
         "state" : "Illinois",
         "code" : "KORD"
      },
      "KMDW: Chicago Midway International Airport" : {
         "city" : "Chicago",
         "name" : "Chicago Midway International Airport",
         "state" : "Illinois",
         "code" : "KMDW"
      },
      "KCUL: Carmi Municipal Airport" : {
         "city" : "Carmi",
         "name" : "Carmi Municipal Airport",
         "state" : "Illinois",
         "code" : "KCUL"
      },
      "KCTK: Ingersoll Airport" : {
         "city" : "Canton",
         "name" : "Ingersoll Airport",
         "state" : "Illinois",
         "code" : "KCTK"
      },
      "KAAA: Logan County Airport (Illinois)" : {
         "city" : "Lincoln",
         "name" : "Logan County Airport (Illinois)",
         "state" : "Illinois",
         "code" : "KAAA"
      },
      "KAJG: Mount Carmel Municipal Airport" : {
         "city" : "Mount Carmel",
         "name" : "Mount Carmel Municipal Airport",
         "state" : "Illinois",
         "code" : "KAJG"
      },
      "KFEP: Albertus Airport" : {
         "city" : "Freeport",
         "name" : "Albertus Airport",
         "state" : "Illinois",
         "code" : "KFEP"
      },
      "KARR: Aurora Municipal Airport (Illinois)" : {
         "city" : "Aurora",
         "name" : "Aurora Municipal Airport (Illinois)",
         "state" : "Illinois",
         "code" : "KARR"
      },
      "KUGN: Waukegan Regional Airport" : {
         "city" : "Waukegan",
         "name" : "Waukegan Regional Airport",
         "state" : "Illinois",
         "code" : "KUGN"
      },
      "KDTG: Dwight Airport" : {
         "city" : "Dwight",
         "name" : "Dwight Airport",
         "state" : "Illinois",
         "code" : "KDTG"
      },
      "KCIR: Cairo Regional Airport" : {
         "city" : "Cairo",
         "name" : "Cairo Regional Airport",
         "state" : "Illinois",
         "code" : "KCIR"
      },
      "KDEC: Decatur Airport" : {
         "city" : "Decatur",
         "name" : "Decatur Airport",
         "state" : "Illinois",
         "code" : "KDEC"
      },
      "KCPS: St. Louis Downtown Airport" : {
         "city" : "Cahokia",
         "name" : "St. Louis Downtown Airport",
         "state" : "Illinois",
         "code" : "KCPS"
      },
      "KLWV: Lawrenceville-Vincennes International Airport" : {
         "city" : "Lawrenceville",
         "name" : "Lawrenceville-Vincennes International Airport",
         "state" : "Illinois",
         "code" : "KLWV"
      },
      "KBMI: Central Illinois Regional Airport" : {
         "city" : "Bloomington",
         "name" : "Central Illinois Regional Airport",
         "state" : "Illinois",
         "code" : "KBMI"
      },
      "KCMI: University of Illinois Willard Airport" : {
         "city" : "Savoy",
         "name" : "University of Illinois Willard Airport",
         "state" : "Illinois",
         "code" : "KCMI"
      },
      "KPIA: Greater Peoria Regional Airport" : {
         "city" : "Peoria",
         "name" : "Greater Peoria Regional Airport",
         "state" : "Illinois",
         "code" : "KPIA"
      }
   },
   "Hawaii, United States" : {
      "PHHN: Hana Airport" : {
         "city" : "Hana",
         "name" : "Hana Airport",
         "state" : "Hawaii",
         "code" : "PHHN"
      },
      "PHHI: Wheeler Army Airfield" : {
         "city" : "Wahiawa",
         "name" : "Wheeler Army Airfield",
         "state" : "Hawaii",
         "code" : "PHHI"
      },
      "PHMK: Molokai Airport" : {
         "city" : "Kaunakakai",
         "name" : "Molokai Airport",
         "state" : "Hawaii",
         "code" : "PHMK"
      },
      "PHJH: Kapalua Airport" : {
         "city" : "Lahaina",
         "name" : "Kapalua Airport",
         "state" : "Hawaii",
         "code" : "PHJH"
      },
      "PHKO: Kona International Airport" : {
         "city" : "Kailua-Kona",
         "name" : "Kona International Airport",
         "state" : "Hawaii",
         "code" : "PHKO"
      },
      "PHLI: Lihu'e Airport" : {
         "city" : "Lihue",
         "name" : "Lihu'e Airport",
         "state" : "Hawaii",
         "code" : "PHLI"
      },
      "PHDH: Dillingham Airfield" : {
         "city" : "Waialua",
         "name" : "Dillingham Airfield",
         "state" : "Hawaii",
         "code" : "PHDH"
      },
      "PHIK: Hickam AFB" : {
         "city" : "Honolulu",
         "name" : "Hickam AFB",
         "state" : "Hawaii",
         "code" : "PHIK"
      },
      "PHUP: Upolu Airport" : {
         "city" : "Hawi",
         "name" : "Upolu Airport",
         "state" : "Hawaii",
         "code" : "PHUP"
      },
      "PHOG: Kahului Airport" : {
         "city" : "Kahului",
         "name" : "Kahului Airport",
         "state" : "Hawaii",
         "code" : "PHOG"
      },
      "PHTO: Hilo International Airport" : {
         "city" : "Hilo",
         "name" : "Hilo International Airport",
         "state" : "Hawaii",
         "code" : "PHTO"
      },
      "PHLU: Kalaupapa Airport" : {
         "city" : "Kalaupapa",
         "name" : "Kalaupapa Airport",
         "state" : "Hawaii",
         "code" : "PHLU"
      },
      "PHMU: Waimea-Kohala Airport" : {
         "city" : "Kamuela",
         "name" : "Waimea-Kohala Airport",
         "state" : "Hawaii",
         "code" : "PHMU"
      },
      "PHNL: Honolulu International Airport" : {
         "city" : "Honolulu",
         "name" : "Honolulu International Airport",
         "state" : "Hawaii",
         "code" : "PHNL"
      },
      "PHJR: Kalaeloa Airport" : {
         "city" : "Kapolei",
         "name" : "Kalaeloa Airport",
         "state" : "Hawaii",
         "code" : "PHJR"
      },
      "PHHF: French Frigate Shoals Airport" : {
         "city" : "French Frigate Shoals",
         "name" : "French Frigate Shoals Airport",
         "state" : "Hawaii",
         "code" : "PHHF"
      },
      "PHNY: Lanai Airport" : {
         "city" : "Lanai City",
         "name" : "Lanai Airport",
         "state" : "Hawaii",
         "code" : "PHNY"
      }
   },
   "United Arab Emirates" : {
      "KQGX: Al Dhafra Air Base" : {
         "city" : "Al Dhafra AB",
         "name" : "Al Dhafra Air Base",
         "state" : "United Arab Emirates",
         "code" : "KQGX"
      }
   },
   "Connecticut, United States" : {
      "KDXR: Danbury Municipal Airport" : {
         "city" : "Danbury",
         "name" : "Danbury Municipal Airport",
         "state" : "Connecticut",
         "code" : "KDXR"
      },
      "KIJD: Windham Airport" : {
         "city" : "Willimantic",
         "name" : "Windham Airport",
         "state" : "Connecticut",
         "code" : "KIJD"
      },
      "KBDR: Igor I. Sikorsky Memorial Airport" : {
         "city" : "Bridgeport",
         "name" : "Igor I. Sikorsky Memorial Airport",
         "state" : "Connecticut",
         "code" : "KBDR"
      },
      "KBDL: Bradley International Airport" : {
         "city" : "Windsor Locks",
         "name" : "Bradley International Airport",
         "state" : "Connecticut",
         "code" : "KBDL"
      },
      "KHVN: Tweed-New Haven Airport" : {
         "city" : "New Haven",
         "name" : "Tweed-New Haven Airport",
         "state" : "Connecticut",
         "code" : "KHVN"
      },
      "KSNC: Chester Airport" : {
         "city" : "Chester",
         "name" : "Chester Airport",
         "state" : "Connecticut",
         "code" : "KSNC"
      },
      "KHFD: Hartford-Brainard Airport" : {
         "city" : "Hartford",
         "name" : "Hartford-Brainard Airport",
         "state" : "Connecticut",
         "code" : "KHFD"
      },
      "KGON: Groton-New London Airport" : {
         "city" : "Groton",
         "name" : "Groton-New London Airport",
         "state" : "Connecticut",
         "code" : "KGON"
      },
      "KMMK: Meriden Markham Municipal Airport" : {
         "city" : "Meriden",
         "name" : "Meriden Markham Municipal Airport",
         "state" : "Connecticut",
         "code" : "KMMK"
      },
      "KOXC: Waterbury-Oxford Airport" : {
         "city" : "Oxford",
         "name" : "Waterbury-Oxford Airport",
         "state" : "Connecticut",
         "code" : "KOXC"
      }
   },
   "Indiana, United States" : {
      "KIND: Indianapolis International Airport" : {
         "city" : "Indianapolis",
         "name" : "Indianapolis International Airport",
         "state" : "Indiana",
         "code" : "KIND"
      },
      "KFKR: Frankfort Municipal Airport" : {
         "city" : "Frankfort",
         "name" : "Frankfort Municipal Airport",
         "state" : "Indiana",
         "code" : "KFKR"
      },
      "KBAK: Columbus Municipal Airport (Indiana)" : {
         "city" : "Columbus",
         "name" : "Columbus Municipal Airport (Indiana)",
         "state" : "Indiana",
         "code" : "KBAK"
      },
      "KIMS: Madison Municipal Airport (Indiana)" : {
         "city" : "Madison",
         "name" : "Madison Municipal Airport (Indiana)",
         "state" : "Indiana",
         "code" : "KIMS"
      },
      "KFRH: French Lick Municipal Airport" : {
         "city" : "French Lick",
         "name" : "French Lick Municipal Airport",
         "state" : "Indiana",
         "code" : "KFRH"
      },
      "KRID: Richmond Municipal Airport" : {
         "city" : "Richmond",
         "name" : "Richmond Municipal Airport",
         "state" : "Indiana",
         "code" : "KRID"
      },
      "KGYY: Gary/Chicago International Airport" : {
         "city" : "Gary",
         "name" : "Gary/Chicago International Airport",
         "state" : "Indiana",
         "code" : "KGYY"
      },
      "KOKK: Kokomo Municipal Airport" : {
         "city" : "Kokomo",
         "name" : "Kokomo Municipal Airport",
         "state" : "Indiana",
         "code" : "KOKK"
      },
      "KSMD: Smith Field (Indiana)" : {
         "city" : "Fort Wayne",
         "name" : "Smith Field (Indiana)",
         "state" : "Indiana",
         "code" : "KSMD"
      },
      "KEYE: Eagle Creek Airpark" : {
         "city" : "Indianapolis",
         "name" : "Eagle Creek Airpark",
         "state" : "Indiana",
         "code" : "KEYE"
      },
      "KCFJ: Crawfordsville Municipal Airport" : {
         "city" : "Crawfordsville",
         "name" : "Crawfordsville Municipal Airport",
         "state" : "Indiana",
         "code" : "KCFJ"
      },
      "KTYQ: Indianapolis Executive Airport" : {
         "city" : "Indianapolis",
         "name" : "Indianapolis Executive Airport",
         "state" : "Indiana",
         "code" : "KTYQ"
      },
      "KTEL: Perry County Municipal Airport" : {
         "city" : "Tell City",
         "name" : "Perry County Municipal Airport",
         "state" : "Indiana",
         "code" : "KTEL"
      },
      "KLAF: Purdue University Airport" : {
         "city" : "West Lafayette",
         "name" : "Purdue University Airport",
         "state" : "Indiana",
         "code" : "KLAF"
      },
      "KBFR: Virgil I. Grissom Municipal Airport" : {
         "city" : "Bedford",
         "name" : "Virgil I. Grissom Municipal Airport",
         "state" : "Indiana",
         "code" : "KBFR"
      },
      "KBMG: Monroe County Airport (Indiana)" : {
         "city" : "Bloomington",
         "name" : "Monroe County Airport (Indiana)",
         "state" : "Indiana",
         "code" : "KBMG"
      },
      "KRZL: Jasper County Airport (Indiana)" : {
         "city" : "Rensselaer",
         "name" : "Jasper County Airport (Indiana)",
         "state" : "Indiana",
         "code" : "KRZL"
      },
      "KGUS: Grissom Air Reserve Base" : {
         "city" : "Peru",
         "name" : "Grissom Air Reserve Base",
         "state" : "Indiana",
         "code" : "KGUS"
      },
      "KFWA: Fort Wayne International Airport" : {
         "city" : "Fort Wayne",
         "name" : "Fort Wayne International Airport",
         "state" : "Indiana",
         "code" : "KFWA"
      },
      "KANQ: Tri-State Steuben County Airport" : {
         "city" : "Angola",
         "name" : "Tri-State Steuben County Airport",
         "state" : "Indiana",
         "code" : "KANQ"
      },
      "KAID: Anderson Municipal Airport" : {
         "city" : "Anderson",
         "name" : "Anderson Municipal Airport",
         "state" : "Indiana",
         "code" : "KAID"
      },
      "KEVV: Evansville Regional Airport" : {
         "city" : "Evansville",
         "name" : "Evansville Regional Airport",
         "state" : "Indiana",
         "code" : "KEVV"
      },
      "KASW: Warsaw Municipal Airport (Indiana)" : {
         "city" : "Warsaw",
         "name" : "Warsaw Municipal Airport (Indiana)",
         "state" : "Indiana",
         "code" : "KASW"
      },
      "KDCY: Daviess County Airport" : {
         "city" : "Washington",
         "name" : "Daviess County Airport",
         "state" : "Indiana",
         "code" : "KDCY"
      },
      "KVPZ: Porter County Airport" : {
         "city" : "Valparaiso",
         "name" : "Porter County Airport",
         "state" : "Indiana",
         "code" : "KVPZ"
      },
      "KEKM: Elkhart Municipal Airport" : {
         "city" : "Elkhart",
         "name" : "Elkhart Municipal Airport",
         "state" : "Indiana",
         "code" : "KEKM"
      },
      "KCEV: Mettel Field" : {
         "city" : "Connersville",
         "name" : "Mettel Field",
         "state" : "Indiana",
         "code" : "KCEV"
      }
   },
   "Louisiana, United States" : {
      "KGNI: Grand Isle Seaplane Base" : {
         "city" : "Grand Isle",
         "name" : "Grand Isle Seaplane Base",
         "state" : "Louisiana",
         "code" : "KGNI"
      },
      "KOPL: St. Landry Parish Airport" : {
         "city" : "Opelousas",
         "name" : "St. Landry Parish Airport",
         "state" : "Louisiana",
         "code" : "KOPL"
      },
      "KPTN: Harry P. Williams Memorial Airport" : {
         "city" : "Patterson",
         "name" : "Harry P. Williams Memorial Airport",
         "state" : "Louisiana",
         "code" : "KPTN"
      },
      "KMSY: Louis Armstrong International Airport" : {
         "city" : "New Orleans",
         "name" : "Louis Armstrong International Airport",
         "state" : "Louisiana",
         "code" : "KMSY"
      },
      "KDTN: Shreveport Downtown Airport" : {
         "city" : "Shreveport",
         "name" : "Shreveport Downtown Airport",
         "state" : "Louisiana",
         "code" : "KDTN"
      },
      "KCWF: Chennault International Airport" : {
         "city" : "Lake Charles",
         "name" : "Chennault International Airport",
         "state" : "Louisiana",
         "code" : "KCWF"
      },
      "KBXA: George R. Carr Memorial Airfield" : {
         "city" : "Bogalusa",
         "name" : "George R. Carr Memorial Airfield",
         "state" : "Louisiana",
         "code" : "KBXA"
      },
      "KHUM: Houma-Terrebonne Airport" : {
         "city" : "Houma",
         "name" : "Houma-Terrebonne Airport",
         "state" : "Louisiana",
         "code" : "KHUM"
      },
      "KACP: Allen Parish Airport" : {
         "city" : "Oakdale",
         "name" : "Allen Parish Airport",
         "state" : "Louisiana",
         "code" : "KACP"
      },
      "KAEX: Alexandria International Airport" : {
         "city" : "Alexandria",
         "name" : "Alexandria International Airport",
         "state" : "Louisiana",
         "code" : "KAEX"
      },
      "KBTR: Baton Rouge Metropolitan Airport" : {
         "city" : "Baton Rouge",
         "name" : "Baton Rouge Metropolitan Airport",
         "state" : "Louisiana",
         "code" : "KBTR"
      },
      "KDRI: Beauregard Regional Airport" : {
         "city" : "De Ridder",
         "name" : "Beauregard Regional Airport",
         "state" : "Louisiana",
         "code" : "KDRI"
      },
      "KLCH: Lake Charles Regional Airport" : {
         "city" : "Lake Charles",
         "name" : "Lake Charles Regional Airport",
         "state" : "Louisiana",
         "code" : "KLCH"
      },
      "KMLU: Monroe Regional Airport (Louisiana)" : {
         "city" : "Monroe",
         "name" : "Monroe Regional Airport (Louisiana)",
         "state" : "Louisiana",
         "code" : "KMLU"
      },
      "KESF: Esler Regional Airport" : {
         "city" : "Alexandria",
         "name" : "Esler Regional Airport",
         "state" : "Louisiana",
         "code" : "KESF"
      },
      "KASD: Slidell Airport" : {
         "city" : "Slidell",
         "name" : "Slidell Airport",
         "state" : "Louisiana",
         "code" : "KASD"
      },
      "KBAD: Barksdale Air Force Base" : {
         "city" : "Bossier City",
         "name" : "Barksdale Air Force Base",
         "state" : "Louisiana",
         "code" : "KBAD"
      },
      "KARA: Acadiana Regional Airport" : {
         "city" : "New Iberia",
         "name" : "Acadiana Regional Airport",
         "state" : "Louisiana",
         "code" : "KARA"
      }
   },
   "FL, United States" : {
      "KPFN: Panama City-Bay County International Airport" : {
         "city" : "Panama City",
         "name" : "Panama City-Bay County International Airport",
         "state" : "FL",
         "code" : "KPFN"
      },
      "KPAM: Tyndall AFB" : {
         "city" : "Panama City",
         "name" : "Tyndall AFB",
         "state" : "FL",
         "code" : "KPAM"
      }
   },
   "Georgia, United States" : {
      "KCXU: Camilla-Mitchell County Airport" : {
         "city" : "Camilla",
         "name" : "Camilla-Mitchell County Airport",
         "state" : "Georgia",
         "code" : "KCXU"
      },
      "KFFC: Falcon Field (Peachtree City, Georgia)" : {
         "city" : "Peachtree City",
         "name" : "Falcon Field (Peachtree City, Georgia)",
         "state" : "Georgia",
         "code" : "KFFC"
      },
      "KCSG: Columbus Metropolitan Airport" : {
         "city" : "Columbus",
         "name" : "Columbus Metropolitan Airport",
         "state" : "Georgia",
         "code" : "KCSG"
      },
      "KAMG: Bacon County Airport" : {
         "city" : "Alma",
         "name" : "Bacon County Airport",
         "state" : "Georgia",
         "code" : "KAMG"
      },
      "KLSF: Lawson Army Airfield" : {
         "city" : "Chattahoochee County",
         "name" : "Lawson Army Airfield",
         "state" : "Georgia",
         "code" : "KLSF"
      },
      "KDNN: Dalton Municipal Airport" : {
         "city" : "Dalton",
         "name" : "Dalton Municipal Airport",
         "state" : "Georgia",
         "code" : "KDNN"
      },
      "KPUJ: Paulding County Regional Airport" : {
         "city" : "Dallas",
         "name" : "Paulding County Regional Airport",
         "state" : "Georgia",
         "code" : "KPUJ"
      },
      "KHQU: Thomson-McDuffie County Airport" : {
         "city" : "Thomson",
         "name" : "Thomson-McDuffie County Airport",
         "state" : "Georgia",
         "code" : "KHQU"
      },
      "KBQK: Brunswick Golden Isles Airport" : {
         "city" : "Brunswick",
         "name" : "Brunswick Golden Isles Airport",
         "state" : "Georgia",
         "code" : "KBQK"
      },
      "KCCO: Newnan Coweta County Airport" : {
         "city" : "Newnan",
         "name" : "Newnan Coweta County Airport",
         "state" : "Georgia",
         "code" : "KCCO"
      },
      "KPDK: DeKalb-Peachtree Airport" : {
         "city" : "Atlanta",
         "name" : "DeKalb-Peachtree Airport",
         "state" : "Georgia",
         "code" : "KPDK"
      },
      "KAJR: Habersham County Airport" : {
         "city" : "Cornelia",
         "name" : "Habersham County Airport",
         "state" : "Georgia",
         "code" : "KAJR"
      },
      "KDBN: W. H. 'Bud' Barron Airport" : {
         "city" : "Dublin",
         "name" : "W. H. 'Bud' Barron Airport",
         "state" : "Georgia",
         "code" : "KDBN"
      },
      "KDZJ: Blairsville Airport" : {
         "city" : "Blairsville",
         "name" : "Blairsville Airport",
         "state" : "Georgia",
         "code" : "KDZJ"
      },
      "KCZL: Tom B. David Field" : {
         "city" : "Calhoun",
         "name" : "Tom B. David Field",
         "state" : "Georgia",
         "code" : "KCZL"
      },
      "KDQH: Douglas Municipal Airport (Georgia)" : {
         "city" : "Douglas",
         "name" : "Douglas Municipal Airport (Georgia)",
         "state" : "Georgia",
         "code" : "KDQH"
      },
      "KABY: Southwest Georgia Regional Airport" : {
         "city" : "Albany",
         "name" : "Southwest Georgia Regional Airport",
         "state" : "Georgia",
         "code" : "KABY"
      },
      "KBHC: Baxley Municipal Airport" : {
         "city" : "Baxley",
         "name" : "Baxley Municipal Airport",
         "state" : "Georgia",
         "code" : "KBHC"
      },
      "KNAB: Albany Naval Air Station" : {
         "city" : "Albany",
         "name" : "Albany Naval Air Station",
         "state" : "Georgia",
         "code" : "KNAB"
      },
      "KMAC: Macon Downtown Airport" : {
         "city" : "Macon",
         "name" : "Macon Downtown Airport",
         "state" : "Georgia",
         "code" : "KMAC"
      },
      "KMCN: Middle Georgia Regional Airport" : {
         "city" : "Macon",
         "name" : "Middle Georgia Regional Airport",
         "state" : "Georgia",
         "code" : "KMCN"
      },
      "KFTY: Fulton County Airport (Georgia)" : {
         "city" : "Atlanta",
         "name" : "Fulton County Airport (Georgia)",
         "state" : "Georgia",
         "code" : "KFTY"
      },
      "KBGE: Decatur County Industrial Air Park" : {
         "city" : "Bainbridge",
         "name" : "Decatur County Industrial Air Park",
         "state" : "Georgia",
         "code" : "KBGE"
      },
      "KATL: Hartsfield-Jackson Atlanta International Airport" : {
         "city" : "Atlanta",
         "name" : "Hartsfield-Jackson Atlanta International Airport",
         "state" : "Georgia",
         "code" : "KATL"
      },
      "KEZM: Heart of Georgia Regional Airport" : {
         "city" : "Eastman",
         "name" : "Heart of Georgia Regional Airport",
         "state" : "Georgia",
         "code" : "KEZM"
      },
      "KAHN: Athens-Ben Epps Airport" : {
         "city" : "Athens",
         "name" : "Athens-Ben Epps Airport",
         "state" : "Georgia",
         "code" : "KAHN"
      },
      "KAYS: Waycross-Ware County Airport" : {
         "city" : "Waycross",
         "name" : "Waycross-Ware County Airport",
         "state" : "Georgia",
         "code" : "KAYS"
      },
      "KHOE: Homerville Airport" : {
         "city" : "Homerville",
         "name" : "Homerville Airport",
         "state" : "Georgia",
         "code" : "KHOE"
      },
      "KAZE: Hazlehurst Airport" : {
         "city" : "Hazlehurst",
         "name" : "Hazlehurst Airport",
         "state" : "Georgia",
         "code" : "KAZE"
      },
      "KRYY: Cobb County McCollum Field" : {
         "city" : "Kennesaw",
         "name" : "Cobb County McCollum Field",
         "state" : "Georgia",
         "code" : "KRYY"
      },
      "KCWV: Claxton-Evans County Airport" : {
         "city" : "Claxton",
         "name" : "Claxton-Evans County Airport",
         "state" : "Georgia",
         "code" : "KCWV"
      },
      "KIIY: Washington-Wilkes County Airport" : {
         "city" : "Washington",
         "name" : "Washington-Wilkes County Airport",
         "state" : "Georgia",
         "code" : "KIIY"
      },
      "KJZP: Pickens County Airport (Georgia)" : {
         "city" : "Jasper",
         "name" : "Pickens County Airport (Georgia)",
         "state" : "Georgia",
         "code" : "KJZP"
      },
      "KBXG: Burke County Airport" : {
         "city" : "Waynesboro",
         "name" : "Burke County Airport",
         "state" : "Georgia",
         "code" : "KBXG"
      },
      "KVLD: Valdosta Regional Airport" : {
         "city" : "Valdosta",
         "name" : "Valdosta Regional Airport",
         "state" : "Georgia",
         "code" : "KVLD"
      },
      "KDNL: Daniel Field" : {
         "city" : "Augusta",
         "name" : "Daniel Field",
         "state" : "Georgia",
         "code" : "KDNL"
      },
      "KAGS: Augusta Regional Airport" : {
         "city" : "Augusta",
         "name" : "Augusta Regional Airport",
         "state" : "Georgia",
         "code" : "KAGS"
      },
      "KCTJ: West Georgia Regional Airport" : {
         "city" : "Carrollton",
         "name" : "West Georgia Regional Airport",
         "state" : "Georgia",
         "code" : "KCTJ"
      },
      "KFZG: Fitzgerald Municipal Airport" : {
         "city" : "Fitzgerald",
         "name" : "Fitzgerald Municipal Airport",
         "state" : "Georgia",
         "code" : "KFZG"
      },
      "KACJ: Souther Field" : {
         "city" : "Americus",
         "name" : "Souther Field",
         "state" : "Georgia",
         "code" : "KACJ"
      },
      "KCKF: Crisp County-Cordele Airport" : {
         "city" : "Cordele",
         "name" : "Crisp County-Cordele Airport",
         "state" : "Georgia",
         "code" : "KCKF"
      }
   },
   "Minnesota, United States" : {
      "KRGK: Red Wing Regional Airport" : {
         "city" : "Red Wing",
         "name" : "Red Wing Regional Airport",
         "state" : "Minnesota",
         "code" : "KRGK"
      },
      "KCFE: Buffalo Municipal Airport (Minnesota)" : {
         "city" : "Buffalo",
         "name" : "Buffalo Municipal Airport (Minnesota)",
         "state" : "Minnesota",
         "code" : "KCFE"
      },
      "KCOQ: Cloquet Carlton County Airport" : {
         "city" : "Cloquet",
         "name" : "Cloquet Carlton County Airport",
         "state" : "Minnesota",
         "code" : "KCOQ"
      },
      "KMIC: Crystal Airport (Minnesota)" : {
         "city" : "Crystal",
         "name" : "Crystal Airport (Minnesota)",
         "state" : "Minnesota",
         "code" : "KMIC"
      },
      "KAQP: Appleton Municipal Airport" : {
         "city" : "Appleton",
         "name" : "Appleton Municipal Airport",
         "state" : "Minnesota",
         "code" : "KAQP"
      },
      "KFKA: Fillmore County Airport" : {
         "city" : "Preston",
         "name" : "Fillmore County Airport",
         "state" : "Minnesota",
         "code" : "KFKA"
      },
      "KBRD: Brainerd Lakes Regional Airport" : {
         "city" : "Brainerd",
         "name" : "Brainerd Lakes Regional Airport",
         "state" : "Minnesota",
         "code" : "KBRD"
      },
      "KSTC: St. Cloud Regional Airport" : {
         "city" : "St. Cloud",
         "name" : "St. Cloud Regional Airport",
         "state" : "Minnesota",
         "code" : "KSTC"
      },
      "KDYT: Sky Harbor Airport (KDYT)" : {
         "city" : "Duluth",
         "name" : "Sky Harbor Airport (KDYT)",
         "state" : "Minnesota",
         "code" : "KDYT"
      },
      "KMSP: Minneapolis-Saint Paul International Airport" : {
         "city" : "Bloomington",
         "name" : "Minneapolis-Saint Paul International Airport",
         "state" : "Minnesota",
         "code" : "KMSP"
      },
      "KLVN: Airlake Airport" : {
         "city" : "Lakeville",
         "name" : "Airlake Airport",
         "state" : "Minnesota",
         "code" : "KLVN"
      },
      "KFBL: Faribault Municipal Airport" : {
         "city" : "Faribault",
         "name" : "Faribault Municipal Airport",
         "state" : "Minnesota",
         "code" : "KFBL"
      },
      "KDVP: Slayton Municipal Airport" : {
         "city" : "Slayton",
         "name" : "Slayton Municipal Airport",
         "state" : "Minnesota",
         "code" : "KDVP"
      },
      "KPNM: Princeton Municipal Airport (Minnesota)" : {
         "city" : "Princeton",
         "name" : "Princeton Municipal Airport (Minnesota)",
         "state" : "Minnesota",
         "code" : "KPNM"
      },
      "KSYN: Stanton Airfield" : {
         "city" : "Stanton",
         "name" : "Stanton Airfield",
         "state" : "Minnesota",
         "code" : "KSYN"
      },
      "KFFM: Fergus Falls Municipal Airport" : {
         "city" : "Fergus Falls",
         "name" : "Fergus Falls Municipal Airport",
         "state" : "Minnesota",
         "code" : "KFFM"
      },
      "KDLH: Duluth International Airport" : {
         "city" : "Duluth",
         "name" : "Duluth International Airport",
         "state" : "Minnesota",
         "code" : "KDLH"
      },
      "KXVG: Longville Municipal Airport" : {
         "city" : "Longville",
         "name" : "Longville Municipal Airport",
         "state" : "Minnesota",
         "code" : "KXVG"
      },
      "KAEL: Albert Lea Municipal Airport" : {
         "city" : "Albert Lea",
         "name" : "Albert Lea Municipal Airport",
         "state" : "Minnesota",
         "code" : "KAEL"
      },
      "KBDE: Baudette International Airport" : {
         "city" : "Baudette",
         "name" : "Baudette International Airport",
         "state" : "Minnesota",
         "code" : "KBDE"
      },
      "KFSE: Fosston Municipal Airport" : {
         "city" : "Fosston",
         "name" : "Fosston Municipal Airport",
         "state" : "Minnesota",
         "code" : "KFSE"
      },
      "KAXN: Chandler Field" : {
         "city" : "Alexandria",
         "name" : "Chandler Field",
         "state" : "Minnesota",
         "code" : "KAXN"
      },
      "KBFW: Silver Bay Municipal Airport" : {
         "city" : "Silver Bay",
         "name" : "Silver Bay Municipal Airport",
         "state" : "Minnesota",
         "code" : "KBFW"
      },
      "KORB: Orr Regional Airport" : {
         "city" : "Orr",
         "name" : "Orr Regional Airport",
         "state" : "Minnesota",
         "code" : "KORB"
      },
      "KCKN: Crookston Municipal Airport" : {
         "city" : "Crookston",
         "name" : "Crookston Municipal Airport",
         "state" : "Minnesota",
         "code" : "KCKN"
      },
      "KONA: Winona Municipal Airport" : {
         "city" : "Winona",
         "name" : "Winona Municipal Airport",
         "state" : "Minnesota",
         "code" : "KONA"
      },
      "KRRT: Warroad International Airport" : {
         "city" : "Warroad",
         "name" : "Warroad International Airport",
         "state" : "Minnesota",
         "code" : "KRRT"
      },
      "KANE: Anoka County-Blaine Airport" : {
         "city" : "Minneapolis",
         "name" : "Anoka County-Blaine Airport",
         "state" : "Minnesota",
         "code" : "KANE"
      },
      "KBJI: Bemidji Regional Airport" : {
         "city" : "Bemidji",
         "name" : "Bemidji Regional Airport",
         "state" : "Minnesota",
         "code" : "KBJI"
      },
      "KADC: Wadena Municipal Airport" : {
         "city" : "Wadena",
         "name" : "Wadena Municipal Airport",
         "state" : "Minnesota",
         "code" : "KADC"
      },
      "KRST: Rochester International Airport" : {
         "city" : "Rochester",
         "name" : "Rochester International Airport",
         "state" : "Minnesota",
         "code" : "KRST"
      },
      "KMGG: Maple Lake Municipal Airport" : {
         "city" : "Maple Lake",
         "name" : "Maple Lake Municipal Airport",
         "state" : "Minnesota",
         "code" : "KMGG"
      },
      "KCDD: Scotts Seaplane Base" : {
         "city" : "Crane Lake",
         "name" : "Scotts Seaplane Base",
         "state" : "Minnesota",
         "code" : "KCDD"
      },
      "KAUM: Austin Municipal Airport" : {
         "city" : "Austin",
         "name" : "Austin Municipal Airport",
         "state" : "Minnesota",
         "code" : "KAUM"
      },
      "KLXL: Little Falls/Morrison County Airport" : {
         "city" : "Little Falls",
         "name" : "Little Falls/Morrison County Airport",
         "state" : "Minnesota",
         "code" : "KLXL"
      },
      "KBBB: Benson Municipal Airport (Minnesota)" : {
         "city" : "Benson",
         "name" : "Benson Municipal Airport (Minnesota)",
         "state" : "Minnesota",
         "code" : "KBBB"
      },
      "KFRM: Fairmont Municipal Airport (Minnesota)" : {
         "city" : "Fairmont",
         "name" : "Fairmont Municipal Airport (Minnesota)",
         "state" : "Minnesota",
         "code" : "KFRM"
      },
      "KSGS: South St. Paul Municipal, Fleming Field" : {
         "city" : "South St. Paul",
         "name" : "South St. Paul Municipal, Fleming Field",
         "code" : "KSGS",
         "state" : "Minnesota"
      },
      "KFCM: Flying Cloud Airport" : {
         "city" : "Minneapolis",
         "name" : "Flying Cloud Airport",
         "state" : "Minnesota",
         "code" : "KFCM"
      },
      "KAIT: Aitkin Municipal Airport" : {
         "city" : "Aitkin",
         "name" : "Aitkin Municipal Airport",
         "state" : "Minnesota",
         "code" : "KAIT"
      },
      "KCQM: Cook Municipal Airport" : {
         "city" : "Cook",
         "name" : "Cook Municipal Airport",
         "state" : "Minnesota",
         "code" : "KCQM"
      },
      "KDTL: Detroit Lakes Airport" : {
         "city" : "Detroit Lakes",
         "name" : "Detroit Lakes Airport",
         "state" : "Minnesota",
         "code" : "KDTL"
      },
      "KELO: Ely Municipal Airport" : {
         "city" : "Ely",
         "name" : "Ely Municipal Airport",
         "state" : "Minnesota",
         "code" : "KELO"
      },
      "KMKT: Mankato Regional Airport" : {
         "city" : "Mankato",
         "name" : "Mankato Regional Airport",
         "state" : "Minnesota",
         "code" : "KMKT"
      },
      "KACQ: Waseca Municipal Airport" : {
         "city" : "Waseca",
         "name" : "Waseca Municipal Airport",
         "state" : "Minnesota",
         "code" : "KACQ"
      },
      "KEVM: Eveleth-Virginia Municipal Airport" : {
         "city" : "Eveleth",
         "name" : "Eveleth-Virginia Municipal Airport",
         "state" : "Minnesota",
         "code" : "KEVM"
      },
      "KFOZ: Bigfork Municipal Airport" : {
         "city" : "Bigfork",
         "name" : "Bigfork Municipal Airport",
         "state" : "Minnesota",
         "code" : "KFOZ"
      },
      "KCBG: Cambridge Municipal Airport (Minnesota)" : {
         "city" : "Cambridge",
         "name" : "Cambridge Municipal Airport (Minnesota)",
         "state" : "Minnesota",
         "code" : "KCBG"
      },
      "KCHU: Houston County Airport (Minnesota)" : {
         "city" : "Caledonia",
         "name" : "Houston County Airport (Minnesota)",
         "state" : "Minnesota",
         "code" : "KCHU"
      },
      "KSTP: St. Paul Downtown Airport" : {
         "city" : "St. Paul",
         "name" : "St. Paul Downtown Airport",
         "state" : "Minnesota",
         "code" : "KSTP"
      },
      "KHCD: Hutchinson Municipal Airport (Minnesota)" : {
         "city" : "Hutchinson",
         "name" : "Hutchinson Municipal Airport (Minnesota)",
         "state" : "Minnesota",
         "code" : "KHCD"
      },
      "KDXX: Lac qui Parle County Airport" : {
         "city" : "Madison",
         "name" : "Lac qui Parle County Airport",
         "state" : "Minnesota",
         "code" : "KDXX"
      },
      "KCKC: Grand Marais/Cook County Airport" : {
         "city" : "Grand Marais",
         "name" : "Grand Marais/Cook County Airport",
         "state" : "Minnesota",
         "code" : "KCKC"
      }
   },
   "California, United States" : {
      "KMIT: Shafter Airport" : {
         "city" : "Shafter",
         "name" : "Shafter Airport",
         "state" : "California",
         "code" : "KMIT"
      },
      "KMCE: Merced Municipal Airport" : {
         "city" : "Merced",
         "name" : "Merced Municipal Airport",
         "state" : "California",
         "code" : "KMCE"
      },
      "KACV: Arcata-Eureka Airport" : {
         "city" : "Arcata",
         "name" : "Arcata-Eureka Airport",
         "state" : "California",
         "code" : "KACV"
      },
      "KNTK: Marine Corps Air Station Tustin" : {
         "city" : "Santa Ana",
         "name" : "Marine Corps Air Station Tustin",
         "state" : "California",
         "code" : "KNTK"
      },
      "KSFO: San Francisco International Airport" : {
         "city" : "San Mateo County",
         "name" : "San Francisco International Airport",
         "state" : "California",
         "code" : "KSFO"
      },
      "KBNG: Banning Municipal Airport" : {
         "city" : "Banning",
         "name" : "Banning Municipal Airport",
         "state" : "California",
         "code" : "KBNG"
      },
      "KCXL: Calexico International Airport" : {
         "city" : "Calexico",
         "name" : "Calexico International Airport",
         "state" : "California",
         "code" : "KCXL"
      },
      "KBLH: Blythe Airport" : {
         "city" : "Blythe",
         "name" : "Blythe Airport",
         "state" : "California",
         "code" : "KBLH"
      },
      "KBIH: Eastern Sierra Regional Airport" : {
         "city" : "Bishop",
         "name" : "Eastern Sierra Regional Airport",
         "state" : "California",
         "code" : "KBIH"
      },
      "KRIR: Flabob Airport" : {
         "city" : "Riverside",
         "name" : "Flabob Airport",
         "state" : "California",
         "code" : "KRIR"
      },
      "KAPC: Napa County Airport" : {
         "city" : "Napa",
         "name" : "Napa County Airport",
         "state" : "California",
         "code" : "KAPC"
      },
      "KSCK: Stockton Metro Airport" : {
         "city" : "Stockton",
         "name" : "Stockton Metro Airport",
         "state" : "California",
         "code" : "KSCK"
      },
      "KAAT: Alturas Municipal Airport" : {
         "city" : "Alturas",
         "name" : "Alturas Municipal Airport",
         "state" : "California",
         "code" : "KAAT"
      },
      "KRAL: Riverside Municipal Airport" : {
         "city" : "Riverside",
         "name" : "Riverside Municipal Airport",
         "state" : "California",
         "code" : "KRAL"
      },
      "KIYK: Inyokern Airport" : {
         "city" : "Inyokern",
         "name" : "Inyokern Airport",
         "state" : "California",
         "code" : "KIYK"
      },
      "KSQL: San Carlos Airport" : {
         "city" : "San Carlos",
         "name" : "San Carlos Airport",
         "state" : "California",
         "code" : "KSQL"
      },
      "KMCC: McClellan Airfield" : {
         "city" : "Sacramento",
         "name" : "McClellan Airfield",
         "state" : "California",
         "code" : "KMCC"
      },
      "KTOA: Zamperini Airport" : {
         "city" : "Torrance",
         "name" : "Zamperini Airport",
         "state" : "California",
         "code" : "KTOA"
      },
      "KBAB: Beale Air Force Base" : {
         "city" : "Marysville",
         "name" : "Beale Air Force Base",
         "state" : "California",
         "code" : "KBAB"
      },
      "KCRQ: McClellan-Palomar Airport" : {
         "city" : "Carlsbad",
         "name" : "McClellan-Palomar Airport",
         "state" : "California",
         "code" : "KCRQ"
      },
      "KSZP: Santa Paula Airport" : {
         "city" : "Santa Paula",
         "name" : "Santa Paula Airport",
         "state" : "California",
         "code" : "KSZP"
      },
      "KPVF: Placerville Airport" : {
         "city" : "Placerville",
         "name" : "Placerville Airport",
         "state" : "California",
         "code" : "KPVF"
      },
      "KOAK: Oakland International Airport" : {
         "city" : "Oakland",
         "name" : "Oakland International Airport",
         "state" : "California",
         "code" : "KOAK"
      },
      "KUDD: Bermuda Dunes Airport" : {
         "city" : "Palm Springs",
         "name" : "Bermuda Dunes Airport",
         "state" : "California",
         "code" : "KUDD"
      },
      "KTLR: Mefford Airport" : {
         "city" : "Tulare",
         "name" : "Mefford Airport",
         "state" : "California",
         "code" : "KTLR"
      },
      "KLPC: Lompoc Airport" : {
         "city" : "Lompoc",
         "name" : "Lompoc Airport",
         "state" : "California",
         "code" : "KLPC"
      },
      "KSVE: Susanville Airport" : {
         "city" : "Susanville",
         "name" : "Susanville Airport",
         "state" : "California",
         "code" : "KSVE"
      },
      "KTNP: Twentynine Palms Airport" : {
         "city" : "Twentynine Palms",
         "name" : "Twentynine Palms Airport",
         "state" : "California",
         "code" : "KTNP"
      },
      "KWVI: Watsonville Municipal Airport" : {
         "city" : "Watsonville",
         "name" : "Watsonville Municipal Airport",
         "state" : "California",
         "code" : "KWVI"
      },
      "KNJK: Naval Air Facility El Centro" : {
         "city" : "El Centro",
         "name" : "Naval Air Facility El Centro",
         "state" : "California",
         "code" : "KNJK"
      },
      "KSNA: John Wayne Airport" : {
         "city" : "Santa Ana",
         "name" : "John Wayne Airport",
         "state" : "California",
         "code" : "KSNA"
      },
      "KMAE: Madera Municipal Airport" : {
         "city" : "Madera",
         "name" : "Madera Municipal Airport",
         "state" : "California",
         "code" : "KMAE"
      },
      "KBFL: Meadows Field Airport" : {
         "city" : "Bakersfield",
         "name" : "Meadows Field Airport",
         "state" : "California",
         "code" : "KBFL"
      },
      "KCKL: Cliff Hatfield Memorial Airport" : {
         "city" : "Calipatria",
         "name" : "Cliff Hatfield Memorial Airport",
         "state" : "California",
         "code" : "KCKL"
      },
      "KSAN: San Diego International Airport" : {
         "city" : "San Diego",
         "name" : "San Diego International Airport",
         "state" : "California",
         "code" : "KSAN"
      },
      "KNZY: Naval Air Station North Island" : {
         "city" : "San Diego",
         "name" : "Naval Air Station North Island",
         "state" : "California",
         "code" : "KNZY"
      },
      "KSNS: Salinas Airport" : {
         "city" : "Salinas",
         "name" : "Salinas Airport",
         "state" : "California",
         "code" : "KSNS"
      },
      "KLGB: Long Beach Airport" : {
         "city" : "Long Beach",
         "name" : "Long Beach Airport",
         "state" : "California",
         "code" : "KLGB"
      },
      "KLAX: Los Angeles International Airport" : {
         "city" : "Los Angeles",
         "name" : "Los Angeles International Airport",
         "state" : "California",
         "code" : "KLAX"
      },
      "KRNM: Ramona Airport" : {
         "city" : "Ramona",
         "name" : "Ramona Airport",
         "state" : "California",
         "code" : "KRNM"
      },
      "KPOC: Brackett Airport" : {
         "city" : "La Verne",
         "name" : "Brackett Airport",
         "state" : "California",
         "code" : "KPOC"
      },
      "KOKB: Oceanside Municipal Airport" : {
         "city" : "Oceanside",
         "name" : "Oceanside Municipal Airport",
         "state" : "California",
         "code" : "KOKB"
      },
      "KNFG: Marine Corps Air Station Camp Pendleton" : {
         "city" : "Oceanside",
         "name" : "Marine Corps Air Station Camp Pendleton",
         "state" : "California",
         "code" : "KNFG"
      },
      "KSBD: San Bernardino International Airport" : {
         "city" : "San Bernardino",
         "name" : "San Bernardino International Airport",
         "state" : "California",
         "code" : "KSBD"
      },
      "KHAF: Half Moon Bay Airport" : {
         "city" : "Half Moon Bay",
         "name" : "Half Moon Bay Airport",
         "state" : "California",
         "code" : "KHAF"
      },
      "KRBL: Red Bluff Municipal Airport" : {
         "city" : "Red Bluff",
         "name" : "Red Bluff Municipal Airport",
         "state" : "California",
         "code" : "KRBL"
      },
      "KTRM: Desert Resorts Regional Airport" : {
         "city" : "Palm Springs",
         "name" : "Desert Resorts Regional Airport",
         "state" : "California",
         "code" : "KTRM"
      },
      "KRIV: March Air Reserve Base" : {
         "city" : "Riverside",
         "name" : "March Air Reserve Base",
         "state" : "California",
         "code" : "KRIV"
      },
      "KMYV: Yuba County Airport" : {
         "city" : "Marysville",
         "name" : "Yuba County Airport",
         "state" : "California",
         "code" : "KMYV"
      },
      "KSBP: San Luis Obispo County Regional Airport" : {
         "city" : "San Luis Obispo",
         "name" : "San Luis Obispo County Regional Airport",
         "state" : "California",
         "code" : "KSBP"
      },
      "KCRO: Corcoran Airport" : {
         "city" : "Corcoran",
         "name" : "Corcoran Airport",
         "state" : "California",
         "code" : "KCRO"
      },
      "KOAR: Marina Airport" : {
         "city" : "Marina",
         "name" : "Marina Airport",
         "state" : "California",
         "code" : "KOAR"
      },
      "KOXR: Oxnard Airport" : {
         "city" : "Oxnard",
         "name" : "Oxnard Airport",
         "state" : "California",
         "code" : "KOXR"
      },
      "KPMD: Palmdale Regional Airport" : {
         "city" : "Palmdale",
         "name" : "Palmdale Regional Airport",
         "state" : "California",
         "code" : "KPMD"
      },
      "KDVO: Gnoss Field" : {
         "city" : "Novato",
         "name" : "Gnoss Field",
         "state" : "California",
         "code" : "KDVO"
      },
      "KFAT: Fresno Yosemite International Airport" : {
         "city" : "Fresno",
         "name" : "Fresno Yosemite International Airport",
         "state" : "California",
         "code" : "KFAT"
      },
      "KMHV: Mojave Spaceport" : {
         "city" : "Mojave",
         "name" : "Mojave Spaceport",
         "state" : "California",
         "code" : "KMHV"
      },
      "KNUC: San Clemente Island" : {
         "city" : "San Clemente",
         "name" : "San Clemente Island",
         "state" : "California",
         "code" : "KNUC"
      },
      "KHHR: Hawthorne Municipal Airport (California)" : {
         "city" : "Hawthorne",
         "name" : "Hawthorne Municipal Airport (California)",
         "state" : "California",
         "code" : "KHHR"
      },
      "KMOD: Modesto City-County Airport" : {
         "city" : "Modesto",
         "name" : "Modesto City-County Airport",
         "state" : "California",
         "code" : "KMOD"
      },
      "KWHP: Whiteman Airport" : {
         "city" : "Los Angeles",
         "name" : "Whiteman Airport",
         "state" : "California",
         "code" : "KWHP"
      },
      "KNZJ: Marine Corps Air Station El Toro" : {
         "city" : "Santa Ana",
         "name" : "Marine Corps Air Station El Toro",
         "state" : "California",
         "code" : "KNZJ"
      },
      "KAJO: Corona Municipal Airport" : {
         "city" : "Corona",
         "name" : "Corona Municipal Airport",
         "state" : "California",
         "code" : "KAJO"
      },
      "KDAG: Barstow-Daggett Airport" : {
         "city" : "Daggett",
         "name" : "Barstow-Daggett Airport",
         "state" : "California",
         "code" : "KDAG"
      },
      "KVNY: Van Nuys Airport" : {
         "city" : "Van Nuys",
         "name" : "Van Nuys Airport",
         "state" : "California",
         "code" : "KVNY"
      },
      "KIPL: Imperial County Airport" : {
         "city" : "Imperial",
         "name" : "Imperial County Airport",
         "state" : "California",
         "code" : "KIPL"
      },
      "KNGZ: Alameda Naval Air Station" : {
         "city" : "Alameda",
         "name" : "Alameda Naval Air Station",
         "state" : "California",
         "code" : "KNGZ"
      },
      "KMER: Castle Airport" : {
         "city" : "Atwater",
         "name" : "Castle Airport",
         "state" : "California",
         "code" : "KMER"
      },
      "KCIC: Chico Municipal Airport" : {
         "city" : "Chico",
         "name" : "Chico Municipal Airport",
         "state" : "California",
         "code" : "KCIC"
      },
      "KBYS: Bicycle Lake Army Airfield" : {
         "city" : "Barstow",
         "name" : "Bicycle Lake Army Airfield",
         "state" : "California",
         "code" : "KBYS"
      },
      "KIZA: Santa Ynez Airport" : {
         "city" : "Santa Ynez",
         "name" : "Santa Ynez Airport",
         "state" : "California",
         "code" : "KIZA"
      },
      "KHWD: Hayward Executive Airport" : {
         "city" : "Hayward",
         "name" : "Hayward Executive Airport",
         "state" : "California",
         "code" : "KHWD"
      },
      "KSIY: Siskiyou County Airport" : {
         "city" : "Montague",
         "name" : "Siskiyou County Airport",
         "state" : "California",
         "code" : "KSIY"
      },
      "KNXP: Marine Corps Air Ground Combat Center Twentynine Palms" : {
         "city" : "Twentynine Palms",
         "name" : "Marine Corps Air Ground Combat Center Twentynine Palms",
         "state" : "California",
         "code" : "KNXP"
      },
      "KNLC: Naval Air Station Lemoore" : {
         "city" : "Lemoore",
         "name" : "Naval Air Station Lemoore",
         "state" : "California",
         "code" : "KNLC"
      },
      "KHJO: Hanford Municipal Airport" : {
         "city" : "Hanford",
         "name" : "Hanford Municipal Airport",
         "state" : "California",
         "code" : "KHJO"
      },
      "KFOT: Rohnerville Airport" : {
         "city" : "Fortuna",
         "name" : "Rohnerville Airport",
         "state" : "California",
         "code" : "KFOT"
      },
      "KRDD: Redding Municipal Airport" : {
         "city" : "Redding",
         "name" : "Redding Municipal Airport",
         "state" : "California",
         "code" : "KRDD"
      },
      "KMMH: Mammoth Yosemite Airport" : {
         "city" : "Mammoth Lakes",
         "name" : "Mammoth Yosemite Airport",
         "state" : "California",
         "code" : "KMMH"
      },
      "KNKX: Marine Corps Air Station Miramar" : {
         "city" : "San Diego",
         "name" : "Marine Corps Air Station Miramar",
         "state" : "California",
         "code" : "KNKX"
      },
      "KRIU: Rancho Murieta Airport" : {
         "city" : "Rancho Murieta",
         "name" : "Rancho Murieta Airport",
         "state" : "California",
         "code" : "KRIU"
      },
      "KRHV: Reid-Hillview Airport" : {
         "city" : "San Jose",
         "name" : "Reid-Hillview Airport",
         "state" : "California",
         "code" : "KRHV"
      },
      "KSTS: Sonoma County Airport" : {
         "city" : "Santa Rosa",
         "name" : "Sonoma County Airport",
         "state" : "California",
         "code" : "KSTS"
      },
      "KVIS: Visalia Municipal Airport" : {
         "city" : "Visalia",
         "name" : "Visalia Municipal Airport",
         "state" : "California",
         "code" : "KVIS"
      },
      "KAPV: Apple Valley Airport (California)" : {
         "city" : "Apple Valley",
         "name" : "Apple Valley Airport (California)",
         "state" : "California",
         "code" : "KAPV"
      },
      "KTRK: Truckee-Tahoe Airport" : {
         "city" : "Truckee",
         "name" : "Truckee-Tahoe Airport",
         "state" : "California",
         "code" : "KTRK"
      },
      "KNUQ: Moffett Federal Airfield" : {
         "city" : "Mountain View",
         "name" : "Moffett Federal Airfield",
         "state" : "California",
         "code" : "KNUQ"
      },
      "KNTD: Naval Air Station Point Mugu" : {
         "city" : "Oxnard",
         "name" : "Naval Air Station Point Mugu",
         "state" : "California",
         "code" : "KNTD"
      },
      "KSAS: Salton Sea Airport" : {
         "city" : "Salton City",
         "name" : "Salton Sea Airport",
         "state" : "California",
         "code" : "KSAS"
      },
      "KAVX: Catalina Airport" : {
         "city" : "Avalon",
         "name" : "Catalina Airport",
         "state" : "California",
         "code" : "KAVX"
      },
      "KCLR: Cliff Hatfield Memorial Airport" : {
         "city" : "Calipatria",
         "name" : "Cliff Hatfield Memorial Airport",
         "state" : "California",
         "code" : "KCLR"
      },
      "KEDW: Edwards Air Force Base" : {
         "city" : "Rosamond",
         "name" : "Edwards Air Force Base",
         "state" : "California",
         "code" : "KEDW"
      },
      "KPTV: Porterville Municipal Airport" : {
         "city" : "Porterville",
         "name" : "Porterville Municipal Airport",
         "state" : "California",
         "code" : "KPTV"
      },
      "KDLO: Delano Municipal Airport" : {
         "city" : "Delano",
         "name" : "Delano Municipal Airport",
         "state" : "California",
         "code" : "KDLO"
      },
      "KSLI: Los Alamitos Army Airfield" : {
         "city" : "Los Alamitos",
         "name" : "Los Alamitos Army Airfield",
         "state" : "California",
         "code" : "KSLI"
      },
      "KWLW: Willows-Glenn County Airport" : {
         "city" : "Willows",
         "name" : "Willows-Glenn County Airport",
         "state" : "California",
         "code" : "KWLW"
      },
      "KCMA: Camarillo Airport" : {
         "city" : "Camarillo",
         "name" : "Camarillo Airport",
         "state" : "California",
         "code" : "KCMA"
      },
      "KCPU: Calaveras County Airport" : {
         "city" : "San Andreas",
         "name" : "Calaveras County Airport",
         "state" : "California",
         "code" : "KCPU"
      },
      "KAUN: Auburn Municipal Airport (California)" : {
         "city" : "Auburn",
         "name" : "Auburn Municipal Airport (California)",
         "state" : "California",
         "code" : "KAUN"
      },
      "KEKA: Murray Field" : {
         "city" : "Eureka",
         "name" : "Murray Field",
         "state" : "California",
         "code" : "KEKA"
      },
      "KHMT: Hemet-Ryan Airport" : {
         "city" : "Hemet",
         "name" : "Hemet-Ryan Airport",
         "state" : "California",
         "code" : "KHMT"
      },
      "KNSI: Naval Outlying Field San Nicolas Island" : {
         "city" : "San Nicolas Island",
         "name" : "Naval Outlying Field San Nicolas Island",
         "state" : "California",
         "code" : "KNSI"
      },
      "KFUL: Fullerton Municipal Airport" : {
         "city" : "Fullerton",
         "name" : "Fullerton Municipal Airport",
         "state" : "California",
         "code" : "KFUL"
      },
      "KLVK: Livermore Municipal Airport" : {
         "city" : "Livermore",
         "name" : "Livermore Municipal Airport",
         "state" : "California",
         "code" : "KLVK"
      },
      "KCCB: Cable Airport" : {
         "city" : "Upland",
         "name" : "Cable Airport",
         "state" : "California",
         "code" : "KCCB"
      },
      "KEED: Needles Airport" : {
         "city" : "Needles",
         "name" : "Needles Airport",
         "state" : "California",
         "code" : "KEED"
      },
      "KCPM: Compton/Woodley Airport" : {
         "city" : "Compton",
         "name" : "Compton/Woodley Airport",
         "state" : "California",
         "code" : "KCPM"
      },
      "KEMT: El Monte Airport" : {
         "city" : "El Monte",
         "name" : "El Monte Airport",
         "state" : "California",
         "code" : "KEMT"
      },
      "KBUR: Bob Hope Airport" : {
         "city" : "Burbank",
         "name" : "Bob Hope Airport",
         "state" : "California",
         "code" : "KBUR"
      },
      "KSAC: Sacramento Executive Airport" : {
         "city" : "Sacramento",
         "name" : "Sacramento Executive Airport",
         "state" : "California",
         "code" : "KSAC"
      },
      "KCNO: Chino Airport" : {
         "city" : "Chino",
         "name" : "Chino Airport",
         "state" : "California",
         "code" : "KCNO"
      },
      "KFCH: Fresno Chandler Executive Airport" : {
         "city" : "Fresno",
         "name" : "Fresno Chandler Executive Airport",
         "state" : "California",
         "code" : "KFCH"
      },
      "KTSP: Tehachapi Municipal Airport" : {
         "city" : "Tehachapi",
         "name" : "Tehachapi Municipal Airport",
         "state" : "California",
         "code" : "KTSP"
      },
      "KBLU: Blue Canyon-Nyack Airport" : {
         "city" : "Emigrant Gap",
         "name" : "Blue Canyon-Nyack Airport",
         "state" : "California",
         "code" : "KBLU"
      },
      "KNRS: Naval Outlying Landing Field Imperial Beach" : {
         "city" : "Imperial Beach",
         "name" : "Naval Outlying Landing Field Imperial Beach",
         "state" : "California",
         "code" : "KNRS"
      },
      "KSDM: Brown Municipal Airport" : {
         "city" : "San Diego",
         "name" : "Brown Municipal Airport",
         "state" : "California",
         "code" : "KSDM"
      },
      "KLHM: Lincoln Regional Airport (California)" : {
         "city" : "Lincoln",
         "name" : "Lincoln Regional Airport (California)",
         "state" : "California",
         "code" : "KLHM"
      },
      "KVCV: Southern California Logistics Airport" : {
         "city" : "Victorville",
         "name" : "Southern California Logistics Airport",
         "state" : "California",
         "code" : "KVCV"
      },
      "KSMO: Santa Monica Airport" : {
         "city" : "Santa Monica",
         "name" : "Santa Monica Airport",
         "state" : "California",
         "code" : "KSMO"
      },
      "KPAO: Palo Alto Airport of Santa Clara County" : {
         "city" : "Palo Alto",
         "name" : "Palo Alto Airport of Santa Clara County",
         "state" : "California",
         "code" : "KPAO"
      },
      "KBWC: Brawley Municipal Airport" : {
         "city" : "Brawley",
         "name" : "Brawley Municipal Airport",
         "state" : "California",
         "code" : "KBWC"
      },
      "KSJC: Norman Y. Mineta San José International Airport" : {
         "city" : "San Jose",
         "name" : "Norman Y. Mineta San José International Airport",
         "state" : "California",
         "code" : "KSJC"
      },
      "KSMF: Sacramento International Airport" : {
         "city" : "Sacramento",
         "name" : "Sacramento International Airport",
         "state" : "California",
         "code" : "KSMF"
      },
      "KSMX: Santa Maria Public Airport" : {
         "city" : "Santa Maria",
         "name" : "Santa Maria Public Airport",
         "state" : "California",
         "code" : "KSMX"
      },
      "KMPI: Mariposa-Yosemite Airport" : {
         "city" : "Mariposa",
         "name" : "Mariposa-Yosemite Airport",
         "state" : "California",
         "code" : "KMPI"
      },
      "KSBA: Santa Barbara Municipal Airport" : {
         "city" : "Santa Barbara",
         "name" : "Santa Barbara Municipal Airport",
         "state" : "California",
         "code" : "KSBA"
      },
      "KCEC: Jack McNamara Field" : {
         "city" : "Crescent City",
         "name" : "Jack McNamara Field",
         "state" : "California",
         "code" : "KCEC"
      },
      "KUKI: Ukiah Municipal Airport" : {
         "city" : "Ukiah",
         "name" : "Ukiah Municipal Airport",
         "state" : "California",
         "code" : "KUKI"
      },
      "KMHR: Mather Airport" : {
         "city" : "Sacramento",
         "name" : "Mather Airport",
         "state" : "California",
         "code" : "KMHR"
      },
      "KKIC: Mesa Del Rey Airport" : {
         "city" : "King City",
         "name" : "Mesa Del Rey Airport",
         "state" : "California",
         "code" : "KKIC"
      },
      "KONT: Ontario International Airport" : {
         "city" : "Ontario",
         "name" : "Ontario International Airport",
         "state" : "California",
         "code" : "KONT"
      },
      "KREI: Redlands Municipal Airport" : {
         "city" : "Redlands",
         "name" : "Redlands Municipal Airport",
         "state" : "California",
         "code" : "KREI"
      },
      "KSEE: Gillespie Airport" : {
         "city" : "San Diego",
         "name" : "Gillespie Airport",
         "state" : "California",
         "code" : "KSEE"
      },
      "KNID: Naval Air Weapons Station China Lake" : {
         "city" : "Ridgecrest",
         "name" : "Naval Air Weapons Station China Lake",
         "state" : "California",
         "code" : "KNID"
      },
      "KTCY: Tracy Municipal Airport (California)" : {
         "city" : "Tracy",
         "name" : "Tracy Municipal Airport (California)",
         "state" : "California",
         "code" : "KTCY"
      },
      "KOVE: Oroville Municipal Airport" : {
         "city" : "Oroville",
         "name" : "Oroville Municipal Airport",
         "state" : "California",
         "code" : "KOVE"
      },
      "KPSP: Palm Springs International Airport" : {
         "city" : "Palm Springs",
         "name" : "Palm Springs International Airport",
         "state" : "California",
         "code" : "KPSP"
      },
      "KVCB: Nut Tree Airport" : {
         "city" : "Vacaville",
         "name" : "Nut Tree Airport",
         "state" : "California",
         "code" : "KVCB"
      },
      "KPRB: Paso Robles Municipal Airport" : {
         "city" : "Paso Robles",
         "name" : "Paso Robles Municipal Airport",
         "state" : "California",
         "code" : "KPRB"
      },
      "KSYL: Roberts Army Heliport" : {
         "city" : "San Luis Obispo County",
         "name" : "Roberts Army Heliport",
         "state" : "California",
         "code" : "KSYL"
      },
      "KLSN: Los Banos Municipal Airport" : {
         "city" : "Los Banos",
         "name" : "Los Banos Municipal Airport",
         "state" : "California",
         "code" : "KLSN"
      },
      "KTVL: Lake Tahoe Airport" : {
         "city" : "South Lake Tahoe",
         "name" : "Lake Tahoe Airport",
         "state" : "California",
         "code" : "KTVL"
      },
      "KMRY: Monterey Peninsula Airport" : {
         "city" : "Monterey",
         "name" : "Monterey Peninsula Airport",
         "state" : "California",
         "code" : "KMRY"
      },
      "KWJF: General William J. Fox Airfield" : {
         "city" : "Lancaster",
         "name" : "General William J. Fox Airfield",
         "state" : "California",
         "code" : "KWJF"
      },
      "KMYF: Montgomery Field" : {
         "city" : "San Diego",
         "name" : "Montgomery Field",
         "state" : "California",
         "code" : "KMYF"
      },
      "KAHC: Amedee Army Airfield" : {
         "city" : "Herlong",
         "name" : "Amedee Army Airfield",
         "state" : "California",
         "code" : "KAHC"
      },
      "KSUU: Travis Air Force Base" : {
         "city" : "Fairfield",
         "name" : "Travis Air Force Base",
         "state" : "California",
         "code" : "KSUU"
      },
      "KCCR: Buchanan Field Airport" : {
         "city" : "Concord",
         "name" : "Buchanan Field Airport",
         "state" : "California",
         "code" : "KCCR"
      },
      "KVBG: Vandenberg Air Force Base" : {
         "city" : "Lompoc",
         "name" : "Vandenberg Air Force Base",
         "state" : "California",
         "code" : "KVBG"
      }
   },
   "Montana, United States" : {
      "KBZN: Gallatin Field Airport" : {
         "city" : "Bozeman",
         "name" : "Gallatin Field Airport",
         "state" : "Montana",
         "code" : "KBZN"
      },
      "KSDY: Sidney-Richland Municipal Airport" : {
         "city" : "Sidney",
         "name" : "Sidney-Richland Municipal Airport",
         "state" : "Montana",
         "code" : "KSDY"
      },
      "KLWT: Lewistown Municipal Airport" : {
         "city" : "Lewistown",
         "name" : "Lewistown Municipal Airport",
         "state" : "Montana",
         "code" : "KLWT"
      },
      "KLVM: Mission Field" : {
         "city" : "Livingston",
         "name" : "Mission Field",
         "state" : "Montana",
         "code" : "KLVM"
      },
      "KDRU: Drummond Airport" : {
         "city" : "Drummond",
         "name" : "Drummond Airport",
         "state" : "Montana",
         "code" : "KDRU"
      },
      "KMLS: Miles City Municipal Airport" : {
         "city" : "Miles City",
         "name" : "Miles City Municipal Airport",
         "state" : "Montana",
         "code" : "KMLS"
      },
      "KGGW: Glasgow Airport (U.S.)" : {
         "city" : "Glasgow",
         "name" : "Glasgow Airport (U.S.)",
         "state" : "Montana",
         "code" : "KGGW"
      },
      "KSBX: Shelby Airport" : {
         "city" : "Shelby",
         "name" : "Shelby Airport",
         "state" : "Montana",
         "code" : "KSBX"
      },
      "KMSO: Missoula International Airport" : {
         "city" : "Missoula",
         "name" : "Missoula International Airport",
         "state" : "Montana",
         "code" : "KMSO"
      },
      "KRPX: Roundup Airport" : {
         "city" : "Roundup",
         "name" : "Roundup Airport",
         "state" : "Montana",
         "code" : "KRPX"
      },
      "KCII: Choteau Airport" : {
         "city" : "Choteau",
         "name" : "Choteau Airport",
         "state" : "Montana",
         "code" : "KCII"
      },
      "KOLF: L. M. Clayton Airport" : {
         "city" : "Wolf Point",
         "name" : "L. M. Clayton Airport",
         "state" : "Montana",
         "code" : "KOLF"
      },
      "KBTM: Bert Mooney Airport" : {
         "city" : "Butte",
         "name" : "Bert Mooney Airport",
         "state" : "Montana",
         "code" : "KBTM"
      },
      "KRED: Red Lodge Airport" : {
         "city" : "Red Lodge",
         "name" : "Red Lodge Airport",
         "state" : "Montana",
         "code" : "KRED"
      },
      "KPWD: Sher-Wood Airport" : {
         "city" : "Plentywood",
         "name" : "Sher-Wood Airport",
         "state" : "Montana",
         "code" : "KPWD"
      },
      "KGTF: Great Falls International Airport" : {
         "city" : "Great Falls",
         "name" : "Great Falls International Airport",
         "state" : "Montana",
         "code" : "KGTF"
      },
      "KDLN: Dillon Airport" : {
         "city" : "Dillon",
         "name" : "Dillon Airport",
         "state" : "Montana",
         "code" : "KDLN"
      },
      "KJDN: Jordan Airport" : {
         "city" : "Jordan",
         "name" : "Jordan Airport",
         "state" : "Montana",
         "code" : "KJDN"
      },
      "KGDV: Dawson Community Airport" : {
         "city" : "Glendive",
         "name" : "Dawson Community Airport",
         "state" : "Montana",
         "code" : "KGDV"
      },
      "KGFA: Great Falls Air Force Base" : {
         "city" : "Great Falls",
         "name" : "Great Falls Air Force Base",
         "state" : "Montana",
         "code" : "KGFA"
      },
      "KHLN: Helena Regional Airport" : {
         "city" : "Helena",
         "name" : "Helena Regional Airport",
         "state" : "Montana",
         "code" : "KHLN"
      },
      "KBHK: Baker Municipal Airport" : {
         "city" : "Baker",
         "name" : "Baker Municipal Airport",
         "state" : "Montana",
         "code" : "KBHK"
      },
      "KHWQ: Wheatland County Airport" : {
         "city" : "Harlowton",
         "name" : "Wheatland County Airport",
         "state" : "Montana",
         "code" : "KHWQ"
      },
      "KCTB: Cut Bank Municipal Airport" : {
         "city" : "Cut Bank",
         "name" : "Cut Bank Municipal Airport",
         "state" : "Montana",
         "code" : "KCTB"
      },
      "KWYS: Yellowstone Airport" : {
         "city" : "West Yellowstone",
         "name" : "Yellowstone Airport",
         "state" : "Montana",
         "code" : "KWYS"
      },
      "KLTY: Liberty County Airport (Montana)" : {
         "city" : "Chester",
         "name" : "Liberty County Airport (Montana)",
         "state" : "Montana",
         "code" : "KLTY"
      },
      "KFCA: Glacier Park International Airport" : {
         "city" : "Kalispell",
         "name" : "Glacier Park International Airport",
         "state" : "Montana",
         "code" : "KFCA"
      },
      "KTHM: Thompson Falls Airport" : {
         "city" : "Thompson Falls",
         "name" : "Thompson Falls Airport",
         "state" : "Montana",
         "code" : "KTHM"
      },
      "KBIL: Billings Logan International Airport" : {
         "city" : "Billings",
         "name" : "Billings Logan International Airport",
         "state" : "Montana",
         "code" : "KBIL"
      },
      "KHVR: Havre City County Airport" : {
         "city" : "Havre",
         "name" : "Havre City County Airport",
         "state" : "Montana",
         "code" : "KHVR"
      }
   },
   "Washington, United States" : {
      "KOMK: Omak Airport" : {
         "city" : "Omak",
         "name" : "Omak Airport",
         "state" : "Washington",
         "code" : "KOMK"
      },
      "KBFI: Boeing Field" : {
         "city" : "Seattle",
         "name" : "Boeing Field",
         "state" : "Washington",
         "code" : "KBFI"
      },
      "KSFF: Felts Field" : {
         "city" : "Spokane",
         "name" : "Felts Field",
         "state" : "Washington",
         "code" : "KSFF"
      },
      "KELN: Bowers Field" : {
         "city" : "Ellensburg",
         "name" : "Bowers Field",
         "state" : "Washington",
         "code" : "KELN"
      },
      "KNUW: Whidbey Island Naval Air Station" : {
         "city" : "Oak Harbor",
         "name" : "Whidbey Island Naval Air Station",
         "state" : "Washington",
         "code" : "KNUW"
      },
      "KSKA: Fairchild Air Force Base" : {
         "city" : "Spokane",
         "name" : "Fairchild Air Force Base",
         "state" : "Washington",
         "code" : "KSKA"
      },
      "KEPH: Ephrata Municipal Airport" : {
         "city" : "Ephrata",
         "name" : "Ephrata Municipal Airport",
         "state" : "Washington",
         "code" : "KEPH"
      },
      "KDEW: Deer Park Airport" : {
         "city" : "Deer Park",
         "name" : "Deer Park Airport",
         "state" : "Washington",
         "code" : "KDEW"
      },
      "KORS: Orcas Island Airport" : {
         "city" : "Eastsound",
         "name" : "Orcas Island Airport",
         "state" : "Washington",
         "code" : "KORS"
      },
      "KRLD: Richland Airport (Washington)" : {
         "city" : "Richland",
         "name" : "Richland Airport (Washington)",
         "state" : "Washington",
         "code" : "KRLD"
      },
      "KKLS: Kelso-Longview Regional Airport" : {
         "city" : "Kelso",
         "name" : "Kelso-Longview Regional Airport",
         "state" : "Washington",
         "code" : "KKLS"
      },
      "KVUO: Pearson Field" : {
         "city" : "Vancouver",
         "name" : "Pearson Field",
         "state" : "Washington",
         "code" : "KVUO"
      },
      "KPAE: Snohomish County Airport" : {
         "city" : "Everett",
         "name" : "Snohomish County Airport",
         "state" : "Washington",
         "code" : "KPAE"
      },
      "KRNT: Renton Municipal Airport" : {
         "city" : "Renton",
         "name" : "Renton Municipal Airport",
         "state" : "Washington",
         "code" : "KRNT"
      },
      "KFCT: Vagabond Army Airfield" : {
         "city" : "Yakima",
         "name" : "Vagabond Army Airfield",
         "state" : "Washington",
         "code" : "KFCT"
      },
      "KTCM: McChord Air Force Base" : {
         "city" : "Tacoma",
         "name" : "McChord Air Force Base",
         "state" : "Washington",
         "code" : "KTCM"
      },
      "KPUW: Pullman-Moscow Regional Airport" : {
         "city" : "Pullman",
         "name" : "Pullman-Moscow Regional Airport",
         "state" : "Washington",
         "code" : "KPUW"
      },
      "KBVS: Skagit Regional Airport" : {
         "city" : "Burlington",
         "name" : "Skagit Regional Airport",
         "state" : "Washington",
         "code" : "KBVS"
      },
      "KFHR: Friday Harbor Airport" : {
         "city" : "Friday Harbor",
         "name" : "Friday Harbor Airport",
         "state" : "Washington",
         "code" : "KFHR"
      },
      "KALW: Walla Walla Regional Airport" : {
         "city" : "Walla Walla",
         "name" : "Walla Walla Regional Airport",
         "state" : "Washington",
         "code" : "KALW"
      },
      "KBLI: Bellingham International Airport" : {
         "city" : "Bellingham",
         "name" : "Bellingham International Airport",
         "state" : "Washington",
         "code" : "KBLI"
      },
      "KMWH: Grant County International Airport" : {
         "city" : "Moses Lake",
         "name" : "Grant County International Airport",
         "state" : "Washington",
         "code" : "KMWH"
      },
      "KNRA: Naval Outlying Field Coupeville" : {
         "city" : "Coupeville",
         "name" : "Naval Outlying Field Coupeville",
         "state" : "Washington",
         "code" : "KNRA"
      },
      "KTDO: Toledo-Winston Carlock Memorial Airport" : {
         "city" : "Toledo",
         "name" : "Toledo-Winston Carlock Memorial Airport",
         "state" : "Washington",
         "code" : "KTDO"
      },
      "KCLS: Chehalis-Centralia Airport" : {
         "city" : "Chehalis",
         "name" : "Chehalis-Centralia Airport",
         "state" : "Washington",
         "code" : "KCLS"
      },
      "KYKM: Yakima Air Terminal" : {
         "city" : "Yakima",
         "name" : "Yakima Air Terminal",
         "state" : "Washington",
         "code" : "KYKM"
      },
      "KUIL: Quillayute Airport" : {
         "city" : "Quillayute",
         "name" : "Quillayute Airport",
         "state" : "Washington",
         "code" : "KUIL"
      },
      "KCLM: William R. Fairchild International Airport" : {
         "city" : "Port Angeles",
         "name" : "William R. Fairchild International Airport",
         "state" : "Washington",
         "code" : "KCLM"
      },
      "KEAT: Pangborn Memorial Airport" : {
         "city" : "Wenatchee",
         "name" : "Pangborn Memorial Airport",
         "state" : "Washington",
         "code" : "KEAT"
      },
      "KAWO: Arlington Municipal Airport (Washington)" : {
         "city" : "Arlington",
         "name" : "Arlington Municipal Airport (Washington)",
         "state" : "Washington",
         "code" : "KAWO"
      },
      "KNOW: Coast Guard Air Station Port Angeles" : {
         "city" : "Port Angeles",
         "name" : "Coast Guard Air Station Port Angeles",
         "state" : "Washington",
         "code" : "KNOW"
      },
      "KGRF: Gray Army Airfield" : {
         "city" : "Fort Lewis",
         "name" : "Gray Army Airfield",
         "state" : "Washington",
         "code" : "KGRF"
      },
      "KSHN: Sanderson Airport" : {
         "city" : "Shelton",
         "name" : "Sanderson Airport",
         "state" : "Washington",
         "code" : "KSHN"
      },
      "KOLM: Olympia Airport" : {
         "city" : "Olympia",
         "name" : "Olympia Airport",
         "state" : "Washington",
         "code" : "KOLM"
      },
      "KPWT: Bremerton National Airport" : {
         "city" : "Bremerton",
         "name" : "Bremerton National Airport",
         "state" : "Washington",
         "code" : "KPWT"
      },
      "KHQM: Bowerman Airport" : {
         "city" : "Hoquiam",
         "name" : "Bowerman Airport",
         "state" : "Washington",
         "code" : "KHQM"
      },
      "KGEG: Spokane International Airport" : {
         "city" : "Spokane",
         "name" : "Spokane International Airport",
         "state" : "Washington",
         "code" : "KGEG"
      },
      "KESW: Easton State Airport" : {
         "city" : "Easton",
         "name" : "Easton State Airport",
         "state" : "Washington",
         "code" : "KESW"
      },
      "KSEA: Seattle-Tacoma International Airport" : {
         "city" : "Seattle",
         "name" : "Seattle-Tacoma International Airport",
         "state" : "Washington",
         "code" : "KSEA"
      },
      "KPSC: Tri-Cities Airport (Washington)" : {
         "city" : "Pasco",
         "name" : "Tri-Cities Airport (Washington)",
         "state" : "Washington",
         "code" : "KPSC"
      },
      "KTIW: Tacoma Narrows Airport" : {
         "city" : "Tacoma",
         "name" : "Tacoma Narrows Airport",
         "state" : "Washington",
         "code" : "KTIW"
      }
   },
   "Romania" : {
      "KQYB: Făurei" : {
         "city" : "Făurei",
         "name" : "Făurei",
         "state" : "Romania",
         "code" : "KQYB"
      }
   },
   "New Mexico, United States" : {
      "KLRU: Las Cruces International Airport" : {
         "city" : "Las Cruces",
         "name" : "Las Cruces International Airport",
         "state" : "New Mexico",
         "code" : "KLRU"
      },
      "KTCC: Tucumcari Municipal Airport" : {
         "city" : "Tucumcari",
         "name" : "Tucumcari Municipal Airport",
         "state" : "New Mexico",
         "code" : "KTCC"
      },
      "KSVC: Grant County Airport (New Mexico)" : {
         "city" : "Silver City",
         "name" : "Grant County Airport (New Mexico)",
         "state" : "New Mexico",
         "code" : "KSVC"
      },
      "KSXU: Santa Rosa Route 66 Airport" : {
         "city" : "Santa Rosa",
         "name" : "Santa Rosa Route 66 Airport",
         "state" : "New Mexico",
         "code" : "KSXU"
      },
      "KROW: Roswell International Air Center Airport" : {
         "city" : "Roswell",
         "name" : "Roswell International Air Center Airport",
         "state" : "New Mexico",
         "code" : "KROW"
      },
      "KAXX: Angel Fire Airport" : {
         "city" : "Angel Fire",
         "name" : "Angel Fire Airport",
         "state" : "New Mexico",
         "code" : "KAXX"
      },
      "KAEG: Double Eagle II Airport" : {
         "city" : "Albuquerque",
         "name" : "Double Eagle II Airport",
         "state" : "New Mexico",
         "code" : "KAEG"
      },
      "KSAF: Santa Fe Municipal Airport" : {
         "city" : "Santa Fe",
         "name" : "Santa Fe Municipal Airport",
         "state" : "New Mexico",
         "code" : "KSAF"
      },
      "KFMN: Four Corners Regional Airport" : {
         "city" : "Farmington",
         "name" : "Four Corners Regional Airport",
         "state" : "New Mexico",
         "code" : "KFMN"
      },
      "KHMN: Holloman Air Force Base" : {
         "city" : "Alamogordo",
         "name" : "Holloman Air Force Base",
         "state" : "New Mexico",
         "code" : "KHMN"
      },
      "KCVS: Cannon Air Force Base" : {
         "city" : "Clovis",
         "name" : "Cannon Air Force Base",
         "state" : "New Mexico",
         "code" : "KCVS"
      },
      "KZUN: Black Rock Airport" : {
         "city" : "Zuni Pueblo",
         "name" : "Black Rock Airport",
         "state" : "New Mexico",
         "code" : "KZUN"
      },
      "KSKX: Taos Regional Airport" : {
         "city" : "Taos",
         "name" : "Taos Regional Airport",
         "state" : "New Mexico",
         "code" : "KSKX"
      },
      "KHOB: Lea County Regional Airport" : {
         "city" : "Hobbs",
         "name" : "Lea County Regional Airport",
         "state" : "New Mexico",
         "code" : "KHOB"
      },
      "KSRR: Sierra Blanca Regional Airport" : {
         "city" : "Ruidoso",
         "name" : "Sierra Blanca Regional Airport",
         "state" : "New Mexico",
         "code" : "KSRR"
      },
      "KLAM: Los Alamos Airport" : {
         "city" : "Los Alamos",
         "name" : "Los Alamos Airport",
         "state" : "New Mexico",
         "code" : "KLAM"
      },
      "KFSU: Fort Sumner Municipal Airport" : {
         "city" : "Fort Sumner",
         "name" : "Fort Sumner Municipal Airport",
         "state" : "New Mexico",
         "code" : "KFSU"
      },
      "KTCS: Truth or Consequences Municipal Airport" : {
         "city" : "Truth or Consequences",
         "name" : "Truth or Consequences Municipal Airport",
         "state" : "New Mexico",
         "code" : "KTCS"
      },
      "KCVN: Clovis Municipal Airport" : {
         "city" : "Clovis",
         "name" : "Clovis Municipal Airport",
         "state" : "New Mexico",
         "code" : "KCVN"
      },
      "KCNM: Cavern City Air Terminal" : {
         "city" : "Carlsbad",
         "name" : "Cavern City Air Terminal",
         "state" : "New Mexico",
         "code" : "KCNM"
      },
      "KLSB: Lordsburg Municipal Airport" : {
         "city" : "Lordsburg",
         "name" : "Lordsburg Municipal Airport",
         "state" : "New Mexico",
         "code" : "KLSB"
      },
      "KALM: Alamogordo-White Sands Regional Airport" : {
         "city" : "Alamogordo",
         "name" : "Alamogordo-White Sands Regional Airport",
         "state" : "New Mexico",
         "code" : "KALM"
      },
      "KATS: Artesia Municipal Airport" : {
         "city" : "Artesia",
         "name" : "Artesia Municipal Airport",
         "state" : "New Mexico",
         "code" : "KATS"
      },
      "KABQ: Albuquerque International Sunport" : {
         "city" : "Albuquerque",
         "name" : "Albuquerque International Sunport",
         "state" : "New Mexico",
         "code" : "KABQ"
      },
      "KRTN: Raton Municipal Airport-Crews Field" : {
         "city" : "Raton",
         "name" : "Raton Municipal Airport-Crews Field",
         "state" : "New Mexico",
         "code" : "KRTN"
      },
      "KDMN: Deming Municipal Airport" : {
         "city" : "Deming",
         "name" : "Deming Municipal Airport",
         "state" : "New Mexico",
         "code" : "KDMN"
      },
      "KCAO: Clayton Municipal Airpark" : {
         "city" : "Clayton",
         "name" : "Clayton Municipal Airpark",
         "state" : "New Mexico",
         "code" : "KCAO"
      },
      "KQOS: North Oscura Peak" : {
         "city" : "Oscura Range",
         "name" : "North Oscura Peak",
         "state" : "New Mexico",
         "code" : "KQOS"
      },
      "KLVS: Las Vegas Municipal Airport" : {
         "city" : "Las Vegas",
         "name" : "Las Vegas Municipal Airport",
         "state" : "New Mexico",
         "code" : "KLVS"
      },
      "KGNT: Grants-Milan Municipal Airport" : {
         "city" : "Grants",
         "name" : "Grants-Milan Municipal Airport",
         "state" : "New Mexico",
         "code" : "KGNT"
      },
      "KGUP: Gallup Municipal Airport" : {
         "city" : "Gallup",
         "name" : "Gallup Municipal Airport",
         "state" : "New Mexico",
         "code" : "KGUP"
      }
   },
   "Kansas, United States" : {
      "KIXD: New Century AirCenter" : {
         "city" : "Olathe",
         "name" : "New Century AirCenter",
         "state" : "Kansas",
         "code" : "KIXD"
      },
      "KHUT: Hutchinson Municipal Airport (Kansas)" : {
         "city" : "Hutchinson",
         "name" : "Hutchinson Municipal Airport (Kansas)",
         "state" : "Kansas",
         "code" : "KHUT"
      },
      "KIAB: McConnell Air Force Base" : {
         "city" : "Wichita",
         "name" : "McConnell Air Force Base",
         "state" : "Kansas",
         "code" : "KIAB"
      },
      "KEHA: Elkhart-Morton County Airport" : {
         "city" : "Elkhart",
         "name" : "Elkhart-Morton County Airport",
         "state" : "Kansas",
         "code" : "KEHA"
      },
      "KRSL: Russell Municipal Airport" : {
         "city" : "Russell",
         "name" : "Russell Municipal Airport",
         "state" : "Kansas",
         "code" : "KRSL"
      },
      "KEWK: Newton City/County Airport" : {
         "city" : "Newton",
         "name" : "Newton City/County Airport",
         "state" : "Kansas",
         "code" : "KEWK"
      },
      "KCEA: Cessna Aircraft Field" : {
         "city" : "Wichita",
         "name" : "Cessna Aircraft Field",
         "state" : "Kansas",
         "code" : "KCEA"
      },
      "KLQR: Larned-Pawnee County Airport" : {
         "city" : "Larned",
         "name" : "Larned-Pawnee County Airport",
         "state" : "Kansas",
         "code" : "KLQR"
      },
      "KHRU: Herington Regional Airport" : {
         "city" : "Herington",
         "name" : "Herington Regional Airport",
         "state" : "Kansas",
         "code" : "KHRU"
      },
      "KMHK: Manhattan Regional Airport" : {
         "city" : "Manhattan",
         "name" : "Manhattan Regional Airport",
         "state" : "Kansas",
         "code" : "KMHK"
      },
      "KDDC: Dodge City Regional Airport" : {
         "city" : "Dodge City",
         "name" : "Dodge City Regional Airport",
         "state" : "Kansas",
         "code" : "KDDC"
      },
      "KFRI: Marshall Army Airfield" : {
         "city" : "Fort Riley",
         "name" : "Marshall Army Airfield",
         "state" : "Kansas",
         "code" : "KFRI"
      },
      "KPPF: Tri-City Airport (Kansas)" : {
         "city" : "Parsons",
         "name" : "Tri-City Airport (Kansas)",
         "state" : "Kansas",
         "code" : "KPPF"
      },
      "KOEL: Oakley Municipal Airport" : {
         "city" : "Oakley",
         "name" : "Oakley Municipal Airport",
         "state" : "Kansas",
         "code" : "KOEL"
      },
      "KOIN: Oberlin Municipal Airport" : {
         "city" : "Oberlin",
         "name" : "Oberlin Municipal Airport",
         "state" : "Kansas",
         "code" : "KOIN"
      },
      "KFSK: Fort Scott Municipal Airport" : {
         "city" : "Fort Scott",
         "name" : "Fort Scott Municipal Airport",
         "state" : "Kansas",
         "code" : "KFSK"
      },
      "KPHG: Phillipsburg Municipal Airport" : {
         "city" : "Phillipsburg",
         "name" : "Phillipsburg Municipal Airport",
         "state" : "Kansas",
         "code" : "KPHG"
      },
      "KANY: Anthony Municipal Airport" : {
         "city" : "Anthony",
         "name" : "Anthony Municipal Airport",
         "state" : "Kansas",
         "code" : "KANY"
      },
      "KEMP: Emporia Municipal Airport" : {
         "city" : "Emporia",
         "name" : "Emporia Municipal Airport",
         "state" : "Kansas",
         "code" : "KEMP"
      },
      "KHQG: Hugoton Municipal Airport" : {
         "city" : "Hugoton",
         "name" : "Hugoton Municipal Airport",
         "state" : "Kansas",
         "code" : "KHQG"
      },
      "KHLC: Hill City Municipal Airport" : {
         "city" : "Hill City",
         "name" : "Hill City Municipal Airport",
         "state" : "Kansas",
         "code" : "KHLC"
      },
      "KCNK: Blosser Municipal Airport" : {
         "city" : "Concordia",
         "name" : "Blosser Municipal Airport",
         "state" : "Kansas",
         "code" : "KCNK"
      },
      "KOJC: Johnson County Executive Airport" : {
         "city" : "Olathe",
         "name" : "Johnson County Executive Airport",
         "state" : "Kansas",
         "code" : "KOJC"
      },
      "KSYF: Cheyenne County Municipal Airport" : {
         "city" : "St. Francis",
         "name" : "Cheyenne County Municipal Airport",
         "state" : "Kansas",
         "code" : "KSYF"
      },
      "KBEC: Beech Factory Airport" : {
         "city" : "Wichita",
         "name" : "Beech Factory Airport",
         "state" : "Kansas",
         "code" : "KBEC"
      },
      "KIDP: Independence Municipal Airport (Kansas)" : {
         "city" : "Independence",
         "name" : "Independence Municipal Airport (Kansas)",
         "state" : "Kansas",
         "code" : "KIDP"
      },
      "KUKL: Coffey County Airport" : {
         "city" : "Burlington",
         "name" : "Coffey County Airport",
         "state" : "Kansas",
         "code" : "KUKL"
      },
      "KNRN: Norton Municipal Airport" : {
         "city" : "Norton",
         "name" : "Norton Municipal Airport",
         "state" : "Kansas",
         "code" : "KNRN"
      },
      "KHYS: Hays Regional Airport" : {
         "city" : "Hays",
         "name" : "Hays Regional Airport",
         "state" : "Kansas",
         "code" : "KHYS"
      },
      "KOWI: Ottawa Municipal Airport" : {
         "city" : "Ottawa",
         "name" : "Ottawa Municipal Airport",
         "state" : "Kansas",
         "code" : "KOWI"
      },
      "KICT: Wichita Mid-Continent Airport" : {
         "city" : "Wichita",
         "name" : "Wichita Mid-Continent Airport",
         "state" : "Kansas",
         "code" : "KICT"
      },
      "KTOP: Billard Municipal Airport" : {
         "city" : "Topeka",
         "name" : "Billard Municipal Airport",
         "state" : "Kansas",
         "code" : "KTOP"
      },
      "KTKO: Mankato Airport" : {
         "city" : "Mankato",
         "name" : "Mankato Airport",
         "state" : "Kansas",
         "code" : "KTKO"
      },
      "KLYO: Lyons-Rice County Municipal Airport" : {
         "city" : "Lyons",
         "name" : "Lyons-Rice County Municipal Airport",
         "state" : "Kansas",
         "code" : "KLYO"
      },
      "KGCK: Garden City Regional Airport" : {
         "city" : "Garden City",
         "name" : "Garden City Regional Airport",
         "state" : "Kansas",
         "code" : "KGCK"
      },
      "KGLD: Renner Goodland Municipal Airport" : {
         "city" : "Goodland",
         "name" : "Renner Goodland Municipal Airport",
         "state" : "Kansas",
         "code" : "KGLD"
      },
      "KCFV: Coffeyville Municipal Airport" : {
         "city" : "Coffeyville",
         "name" : "Coffeyville Municipal Airport",
         "state" : "Kansas",
         "code" : "KCFV"
      },
      "KPTS: Atkinson Municipal Airport" : {
         "city" : "Pittsburg",
         "name" : "Atkinson Municipal Airport",
         "state" : "Kansas",
         "code" : "KPTS"
      },
      "KFLV: Sherman Army Airfield" : {
         "city" : "Fort Leavenworth",
         "name" : "Sherman Army Airfield",
         "state" : "Kansas",
         "code" : "KFLV"
      },
      "KGBD: Great Bend Municipal Airport" : {
         "city" : "Great Bend",
         "name" : "Great Bend Municipal Airport",
         "state" : "Kansas",
         "code" : "KGBD"
      },
      "KEGT: Wellington Municipal Airport" : {
         "city" : "Wellington",
         "name" : "Wellington Municipal Airport",
         "state" : "Kansas",
         "code" : "KEGT"
      },
      "KRPB: Belleville Municipal Airport" : {
         "city" : "Belleville",
         "name" : "Belleville Municipal Airport",
         "state" : "Kansas",
         "code" : "KRPB"
      },
      "KFOE: Forbes Field (Kansas)" : {
         "city" : "Topeka",
         "name" : "Forbes Field (Kansas)",
         "state" : "Kansas",
         "code" : "KFOE"
      },
      "KPTT: Pratt Industrial Airport" : {
         "city" : "Pratt",
         "name" : "Pratt Industrial Airport",
         "state" : "Kansas",
         "code" : "KPTT"
      },
      "KWLD: Strother Field" : {
         "city" : "Winfield",
         "name" : "Strother Field",
         "state" : "Kansas",
         "code" : "KWLD"
      },
      "KLWC: Lawrence Municipal Airport (Kansas)" : {
         "city" : "Lawrence",
         "name" : "Lawrence Municipal Airport (Kansas)",
         "state" : "Kansas",
         "code" : "KLWC"
      },
      "KAAO: Colonel James Jabara Airport" : {
         "city" : "Wichita",
         "name" : "Colonel James Jabara Airport",
         "state" : "Kansas",
         "code" : "KAAO"
      },
      "KEQA: Captain Jack Thomas/El Dorado Airport" : {
         "city" : "El Dorado",
         "name" : "Captain Jack Thomas/El Dorado Airport",
         "state" : "Kansas",
         "code" : "KEQA"
      },
      "KMPR: McPherson Airport" : {
         "city" : "McPherson",
         "name" : "McPherson Airport",
         "state" : "Kansas",
         "code" : "KMPR"
      },
      "KCBK: Shalz Field" : {
         "city" : "Colby",
         "name" : "Shalz Field",
         "state" : "Kansas",
         "code" : "KCBK"
      },
      "KMYZ: Marysville Municipal Airport" : {
         "city" : "Marysville",
         "name" : "Marysville Municipal Airport",
         "state" : "Kansas",
         "code" : "KMYZ"
      },
      "KLBL: Liberal Municipal Airport" : {
         "city" : "Liberal",
         "name" : "Liberal Municipal Airport",
         "state" : "Kansas",
         "code" : "KLBL"
      },
      "KCYW: Clay Center Municipal Airport" : {
         "city" : "Clay Center",
         "name" : "Clay Center Municipal Airport",
         "state" : "Kansas",
         "code" : "KCYW"
      },
      "KSLN: Salina Municipal Airport" : {
         "city" : "Salina",
         "name" : "Salina Municipal Airport",
         "state" : "Kansas",
         "code" : "KSLN"
      },
      "KULS: Ulysses Airport" : {
         "city" : "Ulysses",
         "name" : "Ulysses Airport",
         "state" : "Kansas",
         "code" : "KULS"
      },
      "KMEJ: Meade Municipal Airport" : {
         "city" : "Meade",
         "name" : "Meade Municipal Airport",
         "state" : "Kansas",
         "code" : "KMEJ"
      },
      "KCNU: Chanute Martin Johnson Airport" : {
         "city" : "Chanute",
         "name" : "Chanute Martin Johnson Airport",
         "state" : "Kansas",
         "code" : "KCNU"
      },
      "KADT: Atwood-Rawlins County City-County Airport" : {
         "city" : "Atwood",
         "name" : "Atwood-Rawlins County City-County Airport",
         "state" : "Kansas",
         "code" : "KADT"
      }
   },
   "Qatar" : {
      "KQIR: Al Udeid Air Base" : {
         "city" : "Al Udeid AB",
         "name" : "Al Udeid Air Base",
         "state" : "Qatar",
         "code" : "KQIR"
      }
   },
   "Pennsylvania, United States" : {
      "KWAY: Greene County Airport (Pennsylvania)" : {
         "city" : "Waynesburg",
         "name" : "Greene County Airport (Pennsylvania)",
         "state" : "Pennsylvania",
         "code" : "KWAY"
      },
      "KAOO: Altoona-Blair County Airport" : {
         "city" : "Altoona",
         "name" : "Altoona-Blair County Airport",
         "state" : "Pennsylvania",
         "code" : "KAOO"
      },
      "KTHV: York Airport (Pennsylvania)" : {
         "city" : "York",
         "name" : "York Airport (Pennsylvania)",
         "state" : "Pennsylvania",
         "code" : "KTHV"
      },
      "KBFD: Bradford Regional Airport" : {
         "city" : "Bradford",
         "name" : "Bradford Regional Airport",
         "state" : "Pennsylvania",
         "code" : "KBFD"
      },
      "KUNV: University Park Airport" : {
         "city" : "University Park",
         "name" : "University Park Airport",
         "state" : "Pennsylvania",
         "code" : "KUNV"
      },
      "KMUU: Huntingdon County Airport" : {
         "city" : "Mount Union",
         "name" : "Huntingdon County Airport",
         "state" : "Pennsylvania",
         "code" : "KMUU"
      },
      "KPHL: Philadelphia International Airport" : {
         "city" : "Philadelphia",
         "name" : "Philadelphia International Airport",
         "state" : "Pennsylvania",
         "code" : "KPHL"
      },
      "KAGC: Allegheny County Airport" : {
         "city" : "West Mifflin",
         "name" : "Allegheny County Airport",
         "state" : "Pennsylvania",
         "code" : "KAGC"
      },
      "KMDT: Harrisburg International Airport" : {
         "city" : "Harrisburg International Airport – Middletown",
         "name" : "Harrisburg International Airport",
         "state" : "Pennsylvania",
         "code" : "KMDT"
      },
      "KRDG: Reading Regional Airport" : {
         "city" : "Reading",
         "name" : "Reading Regional Airport",
         "state" : "Pennsylvania",
         "code" : "KRDG"
      },
      "KBVI: Beaver County Airport" : {
         "city" : "Beaver Falls",
         "name" : "Beaver County Airport",
         "state" : "Pennsylvania",
         "code" : "KBVI"
      },
      "KSEG: Penn Valley Airport" : {
         "city" : "Selinsgrove",
         "name" : "Penn Valley Airport",
         "state" : "Pennsylvania",
         "code" : "KSEG"
      },
      "KRVL: Mifflin County Airport" : {
         "city" : "Reedsville",
         "name" : "Mifflin County Airport",
         "state" : "Pennsylvania",
         "code" : "KRVL"
      },
      "KJST: Johnstown-Cambria County Airport" : {
         "city" : "Johnstown",
         "name" : "Johnstown-Cambria County Airport",
         "state" : "Pennsylvania",
         "code" : "KJST"
      },
      "KMQS: Chester County G O Carlson Airport" : {
         "city" : "Coatesville",
         "name" : "Chester County G O Carlson Airport",
         "state" : "Pennsylvania",
         "code" : "KMQS"
      },
      "KAVP: Wilkes-Barre/Scranton International Airport" : {
         "city" : "Avoca",
         "name" : "Wilkes-Barre/Scranton International Airport",
         "state" : "Pennsylvania",
         "code" : "KAVP"
      },
      "KDYL: Doylestown Airport" : {
         "city" : "Doylestown",
         "name" : "Doylestown Airport",
         "state" : "Pennsylvania",
         "code" : "KDYL"
      },
      "KZER: Schuylkill County Airport" : {
         "city" : "Pottsville",
         "name" : "Schuylkill County Airport",
         "state" : "Pennsylvania",
         "code" : "KZER"
      },
      "KERI: Erie International Airport" : {
         "city" : "Erie",
         "name" : "Erie International Airport",
         "state" : "Pennsylvania",
         "code" : "KERI"
      },
      "KMPO: Pocono Mountains Municipal Airport" : {
         "city" : "Mount Pocono",
         "name" : "Pocono Mountains Municipal Airport",
         "state" : "Pennsylvania",
         "code" : "KMPO"
      },
      "KHMZ: Bedford County Airport" : {
         "city" : "Bedford",
         "name" : "Bedford County Airport",
         "state" : "Pennsylvania",
         "code" : "KHMZ"
      },
      "KOYM: St. Marys Municipal Airport" : {
         "city" : "Saint Marys",
         "name" : "St. Marys Municipal Airport",
         "state" : "Pennsylvania",
         "code" : "KOYM"
      },
      "KLNS: Lancaster Airport (Pennsylvania)" : {
         "city" : "Lancaster",
         "name" : "Lancaster Airport (Pennsylvania)",
         "state" : "Pennsylvania",
         "code" : "KLNS"
      },
      "KHZL: Hazleton Municipal Airport" : {
         "city" : "Hazleton",
         "name" : "Hazleton Municipal Airport",
         "state" : "Pennsylvania",
         "code" : "KHZL"
      },
      "KPTW: Pottstown Limerick Airport" : {
         "city" : "Pottstown",
         "name" : "Pottstown Limerick Airport",
         "state" : "Pennsylvania",
         "code" : "KPTW"
      },
      "KVVS: Joseph A. Hardy Connellsville Airport" : {
         "city" : "Connellsville",
         "name" : "Joseph A. Hardy Connellsville Airport",
         "state" : "Pennsylvania",
         "code" : "KVVS"
      },
      "KIDI: Indiana County-Jimmy Stewart Airport" : {
         "city" : "Indiana",
         "name" : "Indiana County-Jimmy Stewart Airport",
         "state" : "Pennsylvania",
         "code" : "KIDI"
      },
      "KIPT: Williamsport Regional Airport" : {
         "city" : "Williamsport",
         "name" : "Williamsport Regional Airport",
         "state" : "Pennsylvania",
         "code" : "KIPT"
      },
      "KPSB: Mid-State Airport" : {
         "city" : "Philipsburg",
         "name" : "Mid-State Airport",
         "state" : "Pennsylvania",
         "code" : "KPSB"
      },
      "KBTP: Butler County Airport" : {
         "city" : "Butler",
         "name" : "Butler County Airport",
         "state" : "Pennsylvania",
         "code" : "KBTP"
      },
      "KUCP: New Castle Municipal Airport" : {
         "city" : "New Castle",
         "name" : "New Castle Municipal Airport",
         "state" : "Pennsylvania",
         "code" : "KUCP"
      },
      "KNXX: Naval Air Station Joint Reserve Base Willow Grove" : {
         "city" : "Willow Grove",
         "name" : "Naval Air Station Joint Reserve Base Willow Grove",
         "state" : "Pennsylvania",
         "code" : "KNXX"
      },
      "KFKL: Venango Regional Airport" : {
         "city" : "Franklin",
         "name" : "Venango Regional Airport",
         "state" : "Pennsylvania",
         "code" : "KFKL"
      },
      "KUKT: Quakertown Airport" : {
         "city" : "Quakertown",
         "name" : "Quakertown Airport",
         "state" : "Pennsylvania",
         "code" : "KUKT"
      },
      "KPNE: Northeast Philadelphia Airport" : {
         "city" : "Philadelphia",
         "name" : "Northeast Philadelphia Airport",
         "state" : "Pennsylvania",
         "code" : "KPNE"
      },
      "KPIT: Pittsburgh International Airport" : {
         "city" : "Pittsburgh",
         "name" : "Pittsburgh International Airport",
         "state" : "Pennsylvania",
         "code" : "KPIT"
      },
      "KAFJ: Washington County Airport (Pennsylvania)" : {
         "city" : "Washington",
         "name" : "Washington County Airport (Pennsylvania)",
         "state" : "Pennsylvania",
         "code" : "KAFJ"
      },
      "KLBE: Arnold Palmer Regional Airport" : {
         "city" : "Latrobe",
         "name" : "Arnold Palmer Regional Airport",
         "state" : "Pennsylvania",
         "code" : "KLBE"
      },
      "KLHV: William T. Piper Memorial Airport" : {
         "city" : "Lock Haven",
         "name" : "William T. Piper Memorial Airport",
         "state" : "Pennsylvania",
         "code" : "KLHV"
      },
      "KWBW: Wilkes-Barre Wyoming Valley Airport" : {
         "city" : "Wilkes-Barre",
         "name" : "Wilkes-Barre Wyoming Valley Airport",
         "state" : "Pennsylvania",
         "code" : "KWBW"
      },
      "KDUJ: DuBois Regional Airport" : {
         "city" : "DuBois",
         "name" : "DuBois Regional Airport",
         "state" : "Pennsylvania",
         "code" : "KDUJ"
      },
      "KGKJ: Port Meadville Airport" : {
         "city" : "Meadville",
         "name" : "Port Meadville Airport",
         "state" : "Pennsylvania",
         "code" : "KGKJ"
      },
      "KCXY: Capital City Airport (Pennsylvania)" : {
         "city" : "Harrisburg",
         "name" : "Capital City Airport (Pennsylvania)",
         "state" : "Pennsylvania",
         "code" : "KCXY"
      },
      "KFIG: Clearfield-Lawrence Airport" : {
         "city" : "Clearfield",
         "name" : "Clearfield-Lawrence Airport",
         "state" : "Pennsylvania",
         "code" : "KFIG"
      },
      "KAXQ: Clarion County Airport" : {
         "city" : "Clarion",
         "name" : "Clarion County Airport",
         "state" : "Pennsylvania",
         "code" : "KAXQ"
      },
      "KABE: Lehigh Valley International Airport" : {
         "city" : "Allentown",
         "name" : "Lehigh Valley International Airport",
         "state" : "Pennsylvania",
         "code" : "KABE"
      },
      "KFWQ: Rostraver Airport" : {
         "city" : "Monongahela",
         "name" : "Rostraver Airport",
         "state" : "Pennsylvania",
         "code" : "KFWQ"
      },
      "KOQN: Brandywine Airport" : {
         "city" : "West Chester",
         "name" : "Brandywine Airport",
         "state" : "Pennsylvania",
         "code" : "KOQN"
      }
   },
   "Michigan, United States" : {
      "KFNT: Bishop International Airport" : {
         "city" : "Flint",
         "name" : "Bishop International Airport",
         "state" : "Michigan",
         "code" : "KFNT"
      },
      "KCIU: Chippewa County International Airport" : {
         "city" : "Sault Ste Marie",
         "name" : "Chippewa County International Airport",
         "state" : "Michigan",
         "code" : "KCIU"
      },
      "KERY: Luce County Airport" : {
         "city" : "Newberry",
         "name" : "Luce County Airport",
         "state" : "Michigan",
         "code" : "KERY"
      },
      "KDUH: Toledo Suburban Airport" : {
         "city" : "Lambertville",
         "name" : "Toledo Suburban Airport",
         "state" : "Michigan",
         "code" : "KDUH"
      },
      "KMBS: MBS International Airport" : {
         "city" : "Midland",
         "name" : "MBS International Airport",
         "state" : "Michigan",
         "code" : "KMBS"
      },
      "KRCT: Nartron Field" : {
         "city" : "Reed City",
         "name" : "Nartron Field",
         "state" : "Michigan",
         "code" : "KRCT"
      },
      "KGRR: Gerald R. Ford International Airport" : {
         "city" : "Grand Rapids",
         "name" : "Gerald R. Ford International Airport",
         "state" : "Michigan",
         "code" : "KGRR"
      },
      "KARB: Ann Arbor Municipal Airport" : {
         "city" : "Ann Arbor",
         "name" : "Ann Arbor Municipal Airport",
         "state" : "Michigan",
         "code" : "KARB"
      },
      "KDET: Coleman A. Young Municipal Airport" : {
         "city" : "Detroit",
         "name" : "Coleman A. Young Municipal Airport",
         "state" : "Michigan",
         "code" : "KDET"
      },
      "KBEH: Southwest Michigan Regional Airport" : {
         "city" : "Benton Harbor",
         "name" : "Southwest Michigan Regional Airport",
         "state" : "Michigan",
         "code" : "KBEH"
      },
      "KACB: Antrim County Airport" : {
         "city" : "Bellaire",
         "name" : "Antrim County Airport",
         "state" : "Michigan",
         "code" : "KACB"
      },
      "KTTF: Custer Airport" : {
         "city" : "Monroe",
         "name" : "Custer Airport",
         "state" : "Michigan",
         "code" : "KTTF"
      },
      "KCFS: Tuscola Area Airport" : {
         "city" : "Caro",
         "name" : "Tuscola Area Airport",
         "state" : "Michigan",
         "code" : "KCFS"
      },
      "KAPN: Alpena County Regional Airport" : {
         "city" : "Alpena",
         "name" : "Alpena County Regional Airport",
         "state" : "Michigan",
         "code" : "KAPN"
      },
      "KFKS: Frankfort Dow Memorial Field" : {
         "city" : "Frankfort",
         "name" : "Frankfort Dow Memorial Field",
         "state" : "Michigan",
         "code" : "KFKS"
      },
      "KAMN: Gratiot Community Airport" : {
         "city" : "Alma",
         "name" : "Gratiot Community Airport",
         "state" : "Michigan",
         "code" : "KAMN"
      },
      "KDTW: Detroit Metropolitan Wayne County Airport" : {
         "city" : "Romulus",
         "name" : "Detroit Metropolitan Wayne County Airport",
         "state" : "Michigan",
         "code" : "KDTW"
      },
      "KMTC: Selfridge Air National Guard Base" : {
         "city" : "Mt. Clemens",
         "name" : "Selfridge Air National Guard Base",
         "state" : "Michigan",
         "code" : "KMTC"
      },
      "KFPK: Fitch H. Beach Airport" : {
         "city" : "Charlotte",
         "name" : "Fitch H. Beach Airport",
         "state" : "Michigan",
         "code" : "KFPK"
      },
      "KADG: Lenawee County Airport" : {
         "city" : "Adrian",
         "name" : "Lenawee County Airport",
         "state" : "Michigan",
         "code" : "KADG"
      },
      "KAZO: Kalamazoo/Battle Creek International Airport" : {
         "city" : "Kalamazoo",
         "name" : "Kalamazoo/Battle Creek International Airport",
         "state" : "Michigan",
         "code" : "KAZO"
      },
      "KBAX: Huron County Memorial Airport" : {
         "city" : "Bad Axe",
         "name" : "Huron County Memorial Airport",
         "state" : "Michigan",
         "code" : "KBAX"
      },
      "KYIP: Willow Run Airport" : {
         "city" : "Detroit",
         "name" : "Willow Run Airport",
         "state" : "Michigan",
         "code" : "KYIP"
      },
      "KHYX: Saginaw County H.W. Browne Airport" : {
         "city" : "Saginaw",
         "name" : "Saginaw County H.W. Browne Airport",
         "state" : "Michigan",
         "code" : "KHYX"
      },
      "KANJ: Sault Ste Marie Municipal Airport" : {
         "city" : "Sault Ste Marie",
         "name" : "Sault Ste Marie Municipal Airport",
         "state" : "Michigan",
         "code" : "KANJ"
      },
      "KVLL: Oakland/Troy Airport" : {
         "city" : "Troy",
         "name" : "Oakland/Troy Airport",
         "state" : "Michigan",
         "code" : "KVLL"
      },
      "KBFA: Boyne Mountain Airport" : {
         "city" : "Boyne Falls",
         "name" : "Boyne Mountain Airport",
         "state" : "Michigan",
         "code" : "KBFA"
      },
      "KCVX: Charlevoix Municipal Airport" : {
         "city" : "Charlevoix",
         "name" : "Charlevoix Municipal Airport",
         "state" : "Michigan",
         "code" : "KCVX"
      },
      "KCAD: Wexford County Airport" : {
         "city" : "Cadillac",
         "name" : "Wexford County Airport",
         "state" : "Michigan",
         "code" : "KCAD"
      },
      "KCMX: Houghton County Memorial Airport" : {
         "city" : "Hancock",
         "name" : "Houghton County Memorial Airport",
         "state" : "Michigan",
         "code" : "KCMX"
      },
      "KBTL: W. K. Kellogg Regional Airport" : {
         "city" : "Battle Creek",
         "name" : "W. K. Kellogg Regional Airport",
         "state" : "Michigan",
         "code" : "KBTL"
      },
      "KESC: Delta County Airport" : {
         "city" : "Escanaba",
         "name" : "Delta County Airport",
         "state" : "Michigan",
         "code" : "KESC"
      },
      "KBIV: Tulip City Airport" : {
         "city" : "Holland",
         "name" : "Tulip City Airport",
         "state" : "Michigan",
         "code" : "KBIV"
      }
   },
   "Greece" : {
      "KQNC: Souda Bay" : {
         "city" : "Souda Bay AB",
         "name" : "Souda Bay",
         "state" : "Greece",
         "code" : "KQNC"
      }
   },
   "Tennessee, United States" : {
      "KDKX: Knoxville Downtown Island Airport" : {
         "city" : "Knoxville",
         "name" : "Knoxville Downtown Island Airport",
         "state" : "Tennessee",
         "code" : "KDKX"
      },
      "KAPT: Marion County Airport (Tennessee)" : {
         "city" : "Jasper",
         "name" : "Marion County Airport (Tennessee)",
         "state" : "Tennessee",
         "code" : "KAPT"
      },
      "KLUG: Ellington Airport (Tennessee)" : {
         "city" : "Lewisburg",
         "name" : "Ellington Airport (Tennessee)",
         "state" : "Tennessee",
         "code" : "KLUG"
      },
      "KFYE: Fayette County Airport (Tennessee)" : {
         "city" : "Somerville",
         "name" : "Fayette County Airport (Tennessee)",
         "state" : "Tennessee",
         "code" : "KFYE"
      },
      "KBNA: Nashville International Airport" : {
         "city" : "Nashville",
         "name" : "Nashville International Airport",
         "state" : "Tennessee",
         "code" : "KBNA"
      },
      "KMEM: Memphis International Airport" : {
         "city" : "Memphis",
         "name" : "Memphis International Airport",
         "state" : "Tennessee",
         "code" : "KMEM"
      },
      "KGHM: Centerville Municipal Airport (Tennessee)" : {
         "city" : "Centerville",
         "name" : "Centerville Municipal Airport (Tennessee)",
         "state" : "Tennessee",
         "code" : "KGHM"
      },
      "KUOS: Franklin County Airport (Tennessee)" : {
         "city" : "Sewanee",
         "name" : "Franklin County Airport (Tennessee)",
         "state" : "Tennessee",
         "code" : "KUOS"
      },
      "KMOR: Moore-Murrell Airport" : {
         "city" : "Morristown",
         "name" : "Moore-Murrell Airport",
         "state" : "Tennessee",
         "code" : "KMOR"
      },
      "KDYR: Dyersburg Regional Airport" : {
         "city" : "Dyersburg",
         "name" : "Dyersburg Regional Airport",
         "state" : "Tennessee",
         "code" : "KDYR"
      },
      "KNQA: Millington Regional Jetport" : {
         "city" : "Millington",
         "name" : "Millington Regional Jetport",
         "state" : "Tennessee",
         "code" : "KNQA"
      },
      "KCHA: Chattanooga Metropolitan Airport" : {
         "city" : "Chattanooga",
         "name" : "Chattanooga Metropolitan Airport",
         "state" : "Tennessee",
         "code" : "KCHA"
      },
      "KCSV: Crossville Memorial Airport" : {
         "city" : "Crossville",
         "name" : "Crossville Memorial Airport",
         "state" : "Tennessee",
         "code" : "KCSV"
      },
      "KMMI: McMinn County Airport" : {
         "city" : "Athens",
         "name" : "McMinn County Airport",
         "state" : "Tennessee",
         "code" : "KMMI"
      },
      "KAYX: Arnold Air Force Base" : {
         "city" : "Tullahoma",
         "name" : "Arnold Air Force Base",
         "state" : "Tennessee",
         "code" : "KAYX"
      },
      "KCKV: Outlaw Field" : {
         "city" : "Clarksville",
         "name" : "Outlaw Field",
         "state" : "Tennessee",
         "code" : "KCKV"
      },
      "KTYS: McGhee Tyson Airport" : {
         "city" : "Knoxville",
         "name" : "McGhee Tyson Airport",
         "state" : "Tennessee",
         "code" : "KTYS"
      },
      "KMKL: Mc Kellar-Sipes Regional Airport" : {
         "city" : "Jackson",
         "name" : "Mc Kellar-Sipes Regional Airport",
         "state" : "Tennessee",
         "code" : "KMKL"
      },
      "KTRI: Tri-Cities Regional Airport" : {
         "city" : "Blountville",
         "name" : "Tri-Cities Regional Airport",
         "state" : "Tennessee",
         "code" : "KTRI"
      },
      "KSYI: Shelbyville Municipal Airport (Tennessee)" : {
         "city" : "Shelbyville",
         "name" : "Shelbyville Municipal Airport (Tennessee)",
         "state" : "Tennessee",
         "code" : "KSYI"
      },
      "KMQY: Smyrna Airport" : {
         "city" : "Smyrna",
         "name" : "Smyrna Airport",
         "state" : "Tennessee",
         "code" : "KMQY"
      },
      "KBGF: Winchester Municipal Airport" : {
         "city" : "Winchester",
         "name" : "Winchester Municipal Airport",
         "state" : "Tennessee",
         "code" : "KBGF"
      },
      "KGKT: Gatlinburg-Pigeon Forge Airport" : {
         "city" : "Sevier County",
         "name" : "Gatlinburg-Pigeon Forge Airport",
         "state" : "Tennessee",
         "code" : "KGKT"
      },
      "KFYM: Fayetteville Municipal Airport (Tennessee)" : {
         "city" : "Fayetteville",
         "name" : "Fayetteville Municipal Airport (Tennessee)",
         "state" : "Tennessee",
         "code" : "KFYM"
      },
      "KPHT: Henry County Airport (Tennessee)" : {
         "city" : "Paris",
         "name" : "Henry County Airport (Tennessee)",
         "state" : "Tennessee",
         "code" : "KPHT"
      },
      "KMBT: Murfreesboro Municipal Airport" : {
         "city" : "Murfreesboro",
         "name" : "Murfreesboro Municipal Airport",
         "state" : "Tennessee",
         "code" : "KMBT"
      }
   },
   "Italy" : {
      "KQNS: Naval Air Station Sigonella" : {
         "city" : "NAS Sigonella",
         "name" : "Naval Air Station Sigonella",
         "state" : "Italy",
         "code" : "KQNS"
      },
      "KQNN: Naples" : {
         "city" : "Naples",
         "name" : "Naples",
         "state" : "Italy",
         "code" : "KQNN"
      }
   },
   "Alaska, United States" : {
      "PAUO: Willow Airport" : {
         "city" : "Willow",
         "name" : "Willow Airport",
         "state" : "Alaska",
         "code" : "PAUO"
      },
      "PAHN: Haines Airport" : {
         "city" : "Haines",
         "name" : "Haines Airport",
         "state" : "Alaska",
         "code" : "PAHN"
      },
      "PAED: Elmendorf Air Force Base" : {
         "city" : "Anchorage",
         "name" : "Elmendorf Air Force Base",
         "state" : "Alaska",
         "code" : "PAED"
      },
      "PAHP: Hooper Bay Airport" : {
         "city" : "Hooper Bay",
         "name" : "Hooper Bay Airport",
         "state" : "Alaska",
         "code" : "PAHP"
      },
      "PAFS: Nikolai Airport" : {
         "city" : "Nikolai",
         "name" : "Nikolai Airport",
         "state" : "Alaska",
         "code" : "PAFS"
      },
      "PFKW: Kwethluk Airport" : {
         "city" : "Kwethluk",
         "name" : "Kwethluk Airport",
         "state" : "Alaska",
         "code" : "PFKW"
      },
      "PAUM: Umiat Airport" : {
         "city" : "Umiat",
         "name" : "Umiat Airport",
         "state" : "Alaska",
         "code" : "PAUM"
      },
      "PANW: New Stuyahok Airport" : {
         "city" : "New Stuyahok",
         "name" : "New Stuyahok Airport",
         "state" : "Alaska",
         "code" : "PANW"
      },
      "PACH: Chuathbaluk Airport" : {
         "city" : "Chuathbaluk",
         "name" : "Chuathbaluk Airport",
         "state" : "Alaska",
         "code" : "PACH"
      },
      "PFNO: Robert (Bob) Curtis Memorial Airport" : {
         "city" : "Noorvik",
         "name" : "Robert (Bob) Curtis Memorial Airport",
         "state" : "Alaska",
         "code" : "PFNO"
      },
      "PAQH: Quinhagak Airport" : {
         "city" : "Quinhagak",
         "name" : "Quinhagak Airport",
         "state" : "Alaska",
         "code" : "PAQH"
      },
      "PATK: Talkeetna Airport" : {
         "city" : "Talkeetna",
         "name" : "Talkeetna Airport",
         "state" : "Alaska",
         "code" : "PATK"
      },
      "PATE: Teller Airport" : {
         "city" : "Teller",
         "name" : "Teller Airport",
         "state" : "Alaska",
         "code" : "PATE"
      },
      "PAPG: Petersburg James A. Johnson Airport" : {
         "city" : "Petersburg",
         "name" : "Petersburg James A. Johnson Airport",
         "state" : "Alaska",
         "code" : "PAPG"
      },
      "PACE: Central Airport" : {
         "city" : "Central",
         "name" : "Central Airport",
         "state" : "Alaska",
         "code" : "PACE"
      },
      "PABI: Allen Army Airfield" : {
         "city" : "Delta Junction",
         "name" : "Allen Army Airfield",
         "state" : "Alaska",
         "code" : "PABI"
      },
      "PFEL: Elim Airport" : {
         "city" : "Elim",
         "name" : "Elim Airport",
         "state" : "Alaska",
         "code" : "PFEL"
      },
      "PAGY: Skagway Airport" : {
         "city" : "Skagway",
         "name" : "Skagway Airport",
         "state" : "Alaska",
         "code" : "PAGY"
      },
      "PAHL: Huslia Airport" : {
         "city" : "Huslia",
         "name" : "Huslia Airport",
         "state" : "Alaska",
         "code" : "PAHL"
      },
      "PAFB: Ladd Army Airfield" : {
         "city" : "Fairbanks",
         "name" : "Ladd Army Airfield",
         "state" : "Alaska",
         "code" : "PAFB"
      },
      "PAEH: Cape Newenham LRRS Airport" : {
         "city" : "Cape Newenham",
         "name" : "Cape Newenham LRRS Airport",
         "state" : "Alaska",
         "code" : "PAEH"
      },
      "PAAN: Gold King Creek Airport" : {
         "city" : "Fairbanks",
         "name" : "Gold King Creek Airport",
         "state" : "Alaska",
         "code" : "PAAN"
      },
      "PAFW: Farewell Airport" : {
         "city" : "Farewell",
         "name" : "Farewell Airport",
         "state" : "Alaska",
         "code" : "PAFW"
      },
      "PAIN: McKinley National Park Airport" : {
         "city" : "McKinley Park",
         "name" : "McKinley National Park Airport",
         "state" : "Alaska",
         "code" : "PAIN"
      },
      "PANI: Aniak Airport" : {
         "city" : "Aniak",
         "name" : "Aniak Airport",
         "state" : "Alaska",
         "code" : "PANI"
      },
      "PAKU: Ugnu-Kuparuk Airport" : {
         "city" : "Kuparuk",
         "name" : "Ugnu-Kuparuk Airport",
         "state" : "Alaska",
         "code" : "PAKU"
      },
      "PAIW: Wales Airport (Alaska)" : {
         "city" : "Wales",
         "name" : "Wales Airport (Alaska)",
         "state" : "Alaska",
         "code" : "PAIW"
      },
      "PASY: Eareckson Air Station" : {
         "city" : "– Eareckson Air Station – Shemya Island",
         "name" : "Eareckson Air Station",
         "state" : "Alaska",
         "code" : "PASY"
      },
      "PFKO: Kotlik Airport" : {
         "city" : "Kotlik",
         "name" : "Kotlik Airport",
         "state" : "Alaska",
         "code" : "PFKO"
      },
      "PATA: Ralph M. Calhoun Memorial Airport" : {
         "city" : "Tanana",
         "name" : "Ralph M. Calhoun Memorial Airport",
         "state" : "Alaska",
         "code" : "PATA"
      },
      "PAGK: Gulkana Airport" : {
         "city" : "Gulkana",
         "name" : "Gulkana Airport",
         "state" : "Alaska",
         "code" : "PAGK"
      },
      "PAII: Egegik Airport" : {
         "city" : "Egegik",
         "name" : "Egegik Airport",
         "state" : "Alaska",
         "code" : "PAII"
      },
      "PASI: Sitka Rocky Gutierrez Airport" : {
         "city" : "Sitka",
         "name" : "Sitka Rocky Gutierrez Airport",
         "state" : "Alaska",
         "code" : "PASI"
      },
      "PAKW: Klawock Airport" : {
         "city" : "Klawock",
         "name" : "Klawock Airport",
         "state" : "Alaska",
         "code" : "PAKW"
      },
      "PAAL: Port Moller Air Force Station" : {
         "city" : "Cold Bay",
         "name" : "Port Moller Air Force Station",
         "state" : "Alaska",
         "code" : "PAAL"
      },
      "PATC: Tin City LRRS Airport" : {
         "city" : "Tin City",
         "name" : "Tin City LRRS Airport",
         "state" : "Alaska",
         "code" : "PATC"
      },
      "PFCL: Clarks Point Airport" : {
         "city" : "Clarks Point",
         "name" : "Clarks Point Airport",
         "state" : "Alaska",
         "code" : "PFCL"
      },
      "PAAM: Driftwood Bay Air Force Station" : {
         "city" : "Dutch Harbor",
         "name" : "Driftwood Bay Air Force Station",
         "state" : "Alaska",
         "code" : "PAAM"
      },
      "PADL: Dillingham Airport" : {
         "city" : "Dillingham",
         "name" : "Dillingham Airport",
         "state" : "Alaska",
         "code" : "PADL"
      },
      "PANN: Nenana Municipal Airport" : {
         "city" : "Nenana",
         "name" : "Nenana Municipal Airport",
         "state" : "Alaska",
         "code" : "PANN"
      },
      "PAKI: Kipnuk Airport" : {
         "city" : "Kipnuk",
         "name" : "Kipnuk Airport",
         "state" : "Alaska",
         "code" : "PAKI"
      },
      "PAVE: Venetie Airport" : {
         "city" : "Venetie",
         "name" : "Venetie Airport",
         "state" : "Alaska",
         "code" : "PAVE"
      },
      "PFSH: Shaktoolik Airport" : {
         "city" : "Shaktoolik",
         "name" : "Shaktoolik Airport",
         "state" : "Alaska",
         "code" : "PFSH"
      },
      "PASA: Savoonga Airport" : {
         "city" : "Savoonga",
         "name" : "Savoonga Airport",
         "state" : "Alaska",
         "code" : "PASA"
      },
      "PANV: Anvik Airport" : {
         "city" : "Anvik",
         "name" : "Anvik Airport",
         "state" : "Alaska",
         "code" : "PANV"
      },
      "PAKN: King Salmon Airport" : {
         "city" : "King Salmon",
         "name" : "King Salmon Airport",
         "state" : "Alaska",
         "code" : "PAKN"
      },
      "POLI: Oliktok LRRS Airport" : {
         "city" : "Oliktok Point",
         "name" : "Oliktok LRRS Airport",
         "state" : "Alaska",
         "code" : "POLI"
      },
      "PADE: Deering Airport" : {
         "city" : "Deering",
         "name" : "Deering Airport",
         "state" : "Alaska",
         "code" : "PADE"
      },
      "PAPB: St. George Airport (Alaska)" : {
         "city" : "St. George",
         "name" : "St. George Airport (Alaska)",
         "state" : "Alaska",
         "code" : "PAPB"
      },
      "PABG: Beluga Airport" : {
         "city" : "Beluga",
         "name" : "Beluga Airport",
         "state" : "Alaska",
         "code" : "PABG"
      },
      "PALP: Alpine Airstrip" : {
         "city" : "Deadhorse",
         "name" : "Alpine Airstrip",
         "state" : "Alaska",
         "code" : "PALP"
      },
      "PAKO: Nikolski Air Station" : {
         "city" : "Nikolski",
         "name" : "Nikolski Air Station",
         "state" : "Alaska",
         "code" : "PAKO"
      },
      "PABE: Bethel Airport" : {
         "city" : "Bethel",
         "name" : "Bethel Airport",
         "state" : "Alaska",
         "code" : "PABE"
      },
      "PAFE: Kake Airport" : {
         "city" : "Kake",
         "name" : "Kake Airport",
         "state" : "Alaska",
         "code" : "PAFE"
      },
      "PATQ: Atqasuk Edward Burnell Sr. Memorial Airport" : {
         "city" : "Atqasuk",
         "name" : "Atqasuk Edward Burnell Sr. Memorial Airport",
         "state" : "Alaska",
         "code" : "PATQ"
      },
      "PABN: Devils Mountain Lodge Airport" : {
         "city" : "Nabesna",
         "name" : "Devils Mountain Lodge Airport",
         "state" : "Alaska",
         "code" : "PABN"
      },
      "PATJ: Tok Airport" : {
         "city" : "Tok",
         "name" : "Tok Airport",
         "state" : "Alaska",
         "code" : "PATJ"
      },
      "PAEI: Eielson Air Force Base" : {
         "city" : "Fairbanks",
         "name" : "Eielson Air Force Base",
         "state" : "Alaska",
         "code" : "PAEI"
      },
      "PANR: Funter Bay Seaplane Base" : {
         "city" : "– Funter Bay Seaplane Base – Funter Bay",
         "name" : "Funter Bay Seaplane Base",
         "state" : "Alaska",
         "code" : "PANR"
      },
      "PAMX: McCarthy Airport" : {
         "city" : "McCarthy",
         "name" : "McCarthy Airport",
         "state" : "Alaska",
         "code" : "PAMX"
      },
      "PALN: Lonely Air Station" : {
         "city" : "Lonely",
         "name" : "Lonely Air Station",
         "state" : "Alaska",
         "code" : "PALN"
      },
      "PAJZ: Koliganek Airport" : {
         "city" : "Koliganek",
         "name" : "Koliganek Airport",
         "state" : "Alaska",
         "code" : "PAJZ"
      },
      "PACL: Clear Airport" : {
         "city" : "Clear",
         "name" : "Clear Airport",
         "state" : "Alaska",
         "code" : "PACL"
      },
      "PALU: Cape Lisburne LRRS Airport" : {
         "city" : "Cape Lisburne",
         "name" : "Cape Lisburne LRRS Airport",
         "state" : "Alaska",
         "code" : "PALU"
      },
      "PAIK: Bob Baker Memorial Airport" : {
         "city" : "Kiana",
         "name" : "Bob Baker Memorial Airport",
         "state" : "Alaska",
         "code" : "PAIK"
      },
      "PAEW: Newtok Airport" : {
         "city" : "Newtok",
         "name" : "Newtok Airport",
         "state" : "Alaska",
         "code" : "PAEW"
      },
      "PAQT: Nuiqsut Airport" : {
         "city" : "Nuiqsut",
         "name" : "Nuiqsut Airport",
         "state" : "Alaska",
         "code" : "PAQT"
      },
      "PASX: Soldotna Airport" : {
         "city" : "Soldotna",
         "name" : "Soldotna Airport",
         "state" : "Alaska",
         "code" : "PASX"
      },
      "PACV: Merle K. (Mudhole) Smith Airport" : {
         "city" : "Cordova",
         "name" : "Merle K. (Mudhole) Smith Airport",
         "state" : "Alaska",
         "code" : "PACV"
      },
      "PAPK: Napaskiak Airport" : {
         "city" : "Napaskiak",
         "name" : "Napaskiak Airport",
         "state" : "Alaska",
         "code" : "PAPK"
      },
      "PFKA: Kasigluk Airport" : {
         "city" : "Kasigluk",
         "name" : "Kasigluk Airport",
         "state" : "Alaska",
         "code" : "PFKA"
      },
      "PADG: Red Dog Airport" : {
         "city" : "Red Dog Mine",
         "name" : "Red Dog Airport",
         "state" : "Alaska",
         "code" : "PADG"
      },
      "PAVD: Valdez Airport" : {
         "city" : "Valdez",
         "name" : "Valdez Airport",
         "state" : "Alaska",
         "code" : "PAVD"
      },
      "PAWR: Whittier Airport" : {
         "city" : "Whittier",
         "name" : "Whittier Airport",
         "state" : "Alaska",
         "code" : "PAWR"
      },
      "PADM: Marshall Don Hunter Sr. Airport" : {
         "city" : "Marshall",
         "name" : "Marshall Don Hunter Sr. Airport",
         "state" : "Alaska",
         "code" : "PADM"
      },
      "PAOT: Ralph Wien Memorial Airport" : {
         "city" : "Kotzebue",
         "name" : "Ralph Wien Memorial Airport",
         "state" : "Alaska",
         "code" : "PAOT"
      },
      "PAMB: Manokotak Airport" : {
         "city" : "Manokotak",
         "name" : "Manokotak Airport",
         "state" : "Alaska",
         "code" : "PAMB"
      },
      "PAFR: Bryant Army Heliport - Fort Richardson" : {
         "city" : "Anchorage",
         "name" : "Bryant Army Heliport - Fort Richardson",
         "state" : "Alaska",
         "code" : "PAFR"
      },
      "PAHU: Hughes Airport (Alaska)" : {
         "city" : "Hughes",
         "name" : "Hughes Airport (Alaska)",
         "state" : "Alaska",
         "code" : "PAHU"
      },
      "PADU: Unalaska Airport" : {
         "city" : "Unalaska",
         "name" : "Unalaska Airport",
         "state" : "Alaska",
         "code" : "PADU"
      },
      "PADQ: Kodiak Airport" : {
         "city" : "Kodiak",
         "name" : "Kodiak Airport",
         "state" : "Alaska",
         "code" : "PADQ"
      },
      "PACR: Circle City Airport" : {
         "city" : "Circle",
         "name" : "Circle City Airport",
         "state" : "Alaska",
         "code" : "PACR"
      },
      "PATW: Cantwell Airport" : {
         "city" : "Cantwell",
         "name" : "Cantwell Airport",
         "state" : "Alaska",
         "code" : "PATW"
      },
      "PAIL: Iliamna Airport" : {
         "city" : "Iliamna",
         "name" : "Iliamna Airport",
         "state" : "Alaska",
         "code" : "PAIL"
      },
      "PACI: Chalkyitsik Airport" : {
         "city" : "Chalkyitsik",
         "name" : "Chalkyitsik Airport",
         "state" : "Alaska",
         "code" : "PACI"
      },
      "PAGA: Edward G. Pitka Sr. Airport" : {
         "city" : "Galena",
         "name" : "Edward G. Pitka Sr. Airport",
         "state" : "Alaska",
         "code" : "PAGA"
      },
      "PAFA: Fairbanks International Airport" : {
         "city" : "Fairbanks",
         "name" : "Fairbanks International Airport",
         "state" : "Alaska",
         "code" : "PAFA"
      },
      "PALJ: Port Alsworth Airport" : {
         "city" : "Port Alsworth",
         "name" : "Port Alsworth Airport",
         "state" : "Alaska",
         "code" : "PALJ"
      },
      "PAAP: Port Alexander Seaplane Base" : {
         "city" : "Port Alexander",
         "name" : "Port Alexander Seaplane Base",
         "state" : "Alaska",
         "code" : "PAAP"
      },
      "PANC: Ted Stevens Anchorage International Airport" : {
         "city" : "Anchorage",
         "name" : "Ted Stevens Anchorage International Airport",
         "state" : "Alaska",
         "code" : "PANC"
      },
      "PAVL: Kivalina Airport" : {
         "city" : "Kivalina",
         "name" : "Kivalina Airport",
         "state" : "Alaska",
         "code" : "PAVL"
      },
      "PAOU: Nelson Lagoon Airport" : {
         "city" : "Nelson Lagoon",
         "name" : "Nelson Lagoon Airport",
         "state" : "Alaska",
         "code" : "PAOU"
      },
      "PAJO: Johnstone Point Airport" : {
         "city" : "Hinchinbrook",
         "name" : "Johnstone Point Airport",
         "state" : "Alaska",
         "code" : "PAJO"
      },
      "PABU: Bullen Point Air Force Station" : {
         "city" : "Kaktovik",
         "name" : "Bullen Point Air Force Station",
         "state" : "Alaska",
         "code" : "PABU"
      },
      "PAHY: Hydaburg Seaplane Base" : {
         "city" : "Hydaburg",
         "name" : "Hydaburg Seaplane Base",
         "state" : "Alaska",
         "code" : "PAHY"
      },
      "PAGN: Angoon Seaplane Base" : {
         "city" : "Angoon",
         "name" : "Angoon Seaplane Base",
         "state" : "Alaska",
         "code" : "PAGN"
      },
      "PAVC: King Cove Airport" : {
         "city" : "King Cove",
         "name" : "King Cove Airport",
         "state" : "Alaska",
         "code" : "PAVC"
      },
      "PASM: St. Mary's Airport (Alaska)" : {
         "city" : "St. Mary's",
         "name" : "St. Mary's Airport (Alaska)",
         "state" : "Alaska",
         "code" : "PASM"
      },
      "PAEM: Emmonak Airport" : {
         "city" : "Emmonak",
         "name" : "Emmonak Airport",
         "state" : "Alaska",
         "code" : "PAEM"
      },
      "PAGT: Nightmute Airport" : {
         "city" : "Nightmute",
         "name" : "Nightmute Airport",
         "state" : "Alaska",
         "code" : "PAGT"
      },
      "PAGS: Gustavus Airport" : {
         "city" : "Gustavus",
         "name" : "Gustavus Airport",
         "state" : "Alaska",
         "code" : "PAGS"
      },
      "PABV: Birchwood Airport" : {
         "city" : "Birchwood",
         "name" : "Birchwood Airport",
         "state" : "Alaska",
         "code" : "PABV"
      },
      "PASC: Deadhorse Airport" : {
         "city" : "Deadhorse",
         "name" : "Deadhorse Airport",
         "state" : "Alaska",
         "code" : "PASC"
      },
      "PASD: Sand Point Airport" : {
         "city" : "Sand Point",
         "name" : "Sand Point Airport",
         "state" : "Alaska",
         "code" : "PASD"
      },
      "PALR: Chandalar Lake Airport" : {
         "city" : "Chandalar Lake",
         "name" : "Chandalar Lake Airport",
         "state" : "Alaska",
         "code" : "PALR"
      },
      "PAWD: Seward Airport" : {
         "city" : "Seward",
         "name" : "Seward Airport",
         "state" : "Alaska",
         "code" : "PAWD"
      },
      "PAFM: Ambler Airport" : {
         "city" : "Ambler",
         "name" : "Ambler Airport",
         "state" : "Alaska",
         "code" : "PAFM"
      },
      "PAAK: Atka Airport" : {
         "city" : "Atka",
         "name" : "Atka Airport",
         "state" : "Alaska",
         "code" : "PAAK"
      },
      "PAKF: False Pass Airport" : {
         "city" : "– False Pass Airport – Aleutians East Borough",
         "name" : "False Pass Airport",
         "state" : "Alaska",
         "code" : "PAKF"
      },
      "PAKH: Akhiok Airport" : {
         "city" : "Akhiok",
         "name" : "Akhiok Airport",
         "state" : "Alaska",
         "code" : "PAKH"
      },
      "PAEG: Eagle Airport" : {
         "city" : "Eagle",
         "name" : "Eagle Airport",
         "state" : "Alaska",
         "code" : "PAEG"
      },
      "PAMO: Mountain Village Airport" : {
         "city" : "Mountain Village",
         "name" : "Mountain Village Airport",
         "state" : "Alaska",
         "code" : "PAMO"
      },
      "PAGH: Shungnak Airport" : {
         "city" : "Shungnak",
         "name" : "Shungnak Airport",
         "state" : "Alaska",
         "code" : "PAGH"
      },
      "PASL: Sleetmute Airport" : {
         "city" : "Sleetmute",
         "name" : "Sleetmute Airport",
         "state" : "Alaska",
         "code" : "PASL"
      },
      "PAPE: Perryville Airport" : {
         "city" : "Perryville",
         "name" : "Perryville Airport",
         "state" : "Alaska",
         "code" : "PAPE"
      },
      "PAEL: Elfin Cove Seaplane Base" : {
         "city" : "Elfin Cove",
         "name" : "Elfin Cove Seaplane Base",
         "state" : "Alaska",
         "code" : "PAEL"
      },
      "PASW: Skwentna Airport" : {
         "city" : "Skwentna",
         "name" : "Skwentna Airport",
         "state" : "Alaska",
         "code" : "PASW"
      },
      "PAOR: Northway Airport" : {
         "city" : "Northway",
         "name" : "Northway Airport",
         "state" : "Alaska",
         "code" : "PAOR"
      },
      "PACZ: Cape Romanzof LRRS Airport" : {
         "city" : "Cape Romanzof",
         "name" : "Cape Romanzof LRRS Airport",
         "state" : "Alaska",
         "code" : "PACZ"
      },
      "PAIG: Igiugig Airport" : {
         "city" : "Igiugig",
         "name" : "Igiugig Airport",
         "state" : "Alaska",
         "code" : "PAIG"
      },
      "PFKU: Koyukuk Airport" : {
         "city" : "Koyukuk",
         "name" : "Koyukuk Airport",
         "state" : "Alaska",
         "code" : "PFKU"
      },
      "PATL: Tatalina LRRS Airport" : {
         "city" : "Tatalina",
         "name" : "Tatalina LRRS Airport",
         "state" : "Alaska",
         "code" : "PATL"
      },
      "PAIM: Indian Mountain LRRS Airport" : {
         "city" : "Indian Mountain",
         "name" : "Indian Mountain LRRS Airport",
         "state" : "Alaska",
         "code" : "PAIM"
      },
      "PAWT: Wainwright Air Station" : {
         "city" : "Wainwright",
         "name" : "Wainwright Air Station",
         "state" : "Alaska",
         "code" : "PAWT"
      },
      "PAKL: Kulik Lake Airport" : {
         "city" : "Kulik Lake",
         "name" : "Kulik Lake Airport",
         "state" : "Alaska",
         "code" : "PAKL"
      },
      "PAWI: Wainwright Airport (Alaska)" : {
         "city" : "Wainwright",
         "name" : "Wainwright Airport (Alaska)",
         "state" : "Alaska",
         "code" : "PAWI"
      },
      "PAEE: Eek Airport" : {
         "city" : "Eek",
         "name" : "Eek Airport",
         "state" : "Alaska",
         "code" : "PAEE"
      },
      "PAMD: Middleton Island Airport" : {
         "city" : "Middleton Island",
         "name" : "Middleton Island Airport",
         "state" : "Alaska",
         "code" : "PAMD"
      },
      "PANT: Annette Island Airport" : {
         "city" : "– Annette Island Airport – Annette Island",
         "name" : "Annette Island Airport",
         "state" : "Alaska",
         "code" : "PANT"
      },
      "PAMC: McGrath Airport" : {
         "city" : "McGrath",
         "name" : "McGrath Airport",
         "state" : "Alaska",
         "code" : "PAMC"
      },
      "PFAL: Allakaket Airport" : {
         "city" : "Allakaket",
         "name" : "Allakaket Airport",
         "state" : "Alaska",
         "code" : "PFAL"
      },
      "PAGM: Gambell Airport" : {
         "city" : "Gambell",
         "name" : "Gambell Airport",
         "state" : "Alaska",
         "code" : "PAGM"
      },
      "PAGB: Galbraith Lake Airport" : {
         "city" : "Galbraith Lake",
         "name" : "Galbraith Lake Airport",
         "state" : "Alaska",
         "code" : "PAGB"
      },
      "PARC: Arctic Village Airport" : {
         "city" : "Arctic Village",
         "name" : "Arctic Village Airport",
         "state" : "Alaska",
         "code" : "PARC"
      },
      "PAMY: Mekoryuk Airport" : {
         "city" : "Mekoryuk",
         "name" : "Mekoryuk Airport",
         "state" : "Alaska",
         "code" : "PAMY"
      },
      "PABM: Big Mountain Air Force Station" : {
         "city" : "Big Mountain",
         "name" : "Big Mountain Air Force Station",
         "state" : "Alaska",
         "code" : "PABM"
      },
      "PALH: Lake Hood Seaplane Base" : {
         "city" : "Anchorage",
         "name" : "Lake Hood Seaplane Base",
         "state" : "Alaska",
         "code" : "PALH"
      },
      "PASV: Sparrevohn LRRS Airport" : {
         "city" : "Sparrevohn",
         "name" : "Sparrevohn LRRS Airport",
         "state" : "Alaska",
         "code" : "PASV"
      },
      "PAJC: Chignik Airport" : {
         "city" : "Chignik",
         "name" : "Chignik Airport",
         "state" : "Alaska",
         "code" : "PAJC"
      },
      "PAMH: Minchumina Airport" : {
         "city" : "Lake Minchumina",
         "name" : "Minchumina Airport",
         "state" : "Alaska",
         "code" : "PAMH"
      },
      "PAWG: Wrangell Airport" : {
         "city" : "Wrangell",
         "name" : "Wrangell Airport",
         "state" : "Alaska",
         "code" : "PAWG"
      },
      "PASH: Shishmaref Airport" : {
         "city" : "Shishmaref",
         "name" : "Shishmaref Airport",
         "state" : "Alaska",
         "code" : "PASH"
      },
      "PAXK: Paxson Airport" : {
         "city" : "Paxson",
         "name" : "Paxson Airport",
         "state" : "Alaska",
         "code" : "PAXK"
      },
      "PAST: Summit Airport (Alaska)" : {
         "city" : "Summit",
         "name" : "Summit Airport (Alaska)",
         "state" : "Alaska",
         "code" : "PAST"
      },
      "PAKP: Anaktuvuk Pass Airport" : {
         "city" : "Anaktuvuk Pass",
         "name" : "Anaktuvuk Pass Airport",
         "state" : "Alaska",
         "code" : "PAKP"
      },
      "PACD: Cold Bay Airport" : {
         "city" : "Cold Bay",
         "name" : "Cold Bay Airport",
         "state" : "Alaska",
         "code" : "PACD"
      },
      "PAMK: St. Michael Airport" : {
         "city" : "St. Michael",
         "name" : "St. Michael Airport",
         "state" : "Alaska",
         "code" : "PAMK"
      },
      "PFKK: Kokhanok Airport" : {
         "city" : "Kokhanok",
         "name" : "Kokhanok Airport",
         "state" : "Alaska",
         "code" : "PFKK"
      },
      "PAHV: Healy River Airport" : {
         "city" : "Healy",
         "name" : "Healy River Airport",
         "state" : "Alaska",
         "code" : "PAHV"
      },
      "PAYA: Yakutat Airport" : {
         "city" : "Yakutat",
         "name" : "Yakutat Airport",
         "state" : "Alaska",
         "code" : "PAYA"
      },
      "PAPM: Platinum Airport" : {
         "city" : "Platinum",
         "name" : "Platinum Airport",
         "state" : "Alaska",
         "code" : "PAPM"
      },
      "PALB: Larsen Bay Airport" : {
         "city" : "Larsen Bay",
         "name" : "Larsen Bay Airport",
         "state" : "Alaska",
         "code" : "PALB"
      },
      "PARY: Ruby Airport" : {
         "city" : "Ruby",
         "name" : "Ruby Airport",
         "state" : "Alaska",
         "code" : "PARY"
      },
      "PAAT: Casco Cove Coast Guard Station" : {
         "city" : "Attu Island",
         "name" : "Casco Cove Coast Guard Station",
         "state" : "Alaska",
         "code" : "PAAT"
      },
      "PASN: St. Paul Island Airport" : {
         "city" : "– St. Paul Island Airport – St. Paul Island",
         "name" : "St. Paul Island Airport",
         "state" : "Alaska",
         "code" : "PASN"
      },
      "PAWS: Wasilla Airport" : {
         "city" : "Wasilla",
         "name" : "Wasilla Airport",
         "state" : "Alaska",
         "code" : "PAWS"
      },
      "PACK: Chefornak Airport" : {
         "city" : "Chefornak",
         "name" : "Chefornak Airport",
         "state" : "Alaska",
         "code" : "PACK"
      },
      "PABT: Bettles Airport" : {
         "city" : "Bettles",
         "name" : "Bettles Airport",
         "state" : "Alaska",
         "code" : "PABT"
      },
      "PAOO: Toksook Bay Airport" : {
         "city" : "Toksook Bay",
         "name" : "Toksook Bay Airport",
         "state" : "Alaska",
         "code" : "PAOO"
      },
      "PAEN: Kenai Municipal Airport" : {
         "city" : "Kenai",
         "name" : "Kenai Municipal Airport",
         "state" : "Alaska",
         "code" : "PAEN"
      },
      "PPIZ: Point Lay LRRS Airport" : {
         "city" : "Point Lay",
         "name" : "Point Lay LRRS Airport",
         "state" : "Alaska",
         "code" : "PPIZ"
      },
      "PAWB: Beaver Airport" : {
         "city" : "Beaver",
         "name" : "Beaver Airport",
         "state" : "Alaska",
         "code" : "PAWB"
      },
      "PAPO: Point Hope Airport" : {
         "city" : "Point Hope",
         "name" : "Point Hope Airport",
         "state" : "Alaska",
         "code" : "PAPO"
      },
      "PAML: Manley Hot Springs Airport" : {
         "city" : "Manley Hot Springs",
         "name" : "Manley Hot Springs Airport",
         "state" : "Alaska",
         "code" : "PAML"
      },
      "PABA: Barter Island LRRS Airport" : {
         "city" : "Barter Island",
         "name" : "Barter Island LRRS Airport",
         "state" : "Alaska",
         "code" : "PABA"
      },
      "PAOM: Nome Airport" : {
         "city" : "Nome",
         "name" : "Nome Airport",
         "state" : "Alaska",
         "code" : "PAOM"
      },
      "PAOB: Kobuk Airport" : {
         "city" : "– Kobuk Airport – Kobuk",
         "name" : "Kobuk Airport",
         "state" : "Alaska",
         "code" : "PAOB"
      },
      "PARS: Russian Mission Airport" : {
         "city" : "Russian Mission",
         "name" : "Russian Mission Airport",
         "state" : "Alaska",
         "code" : "PARS"
      },
      "PAPC: Port Clarence Coast Guard Station" : {
         "city" : "Port Clarence",
         "name" : "Port Clarence Coast Guard Station",
         "state" : "Alaska",
         "code" : "PAPC"
      },
      "PACY: Yakataga Airport" : {
         "city" : "Cape Yakataga",
         "name" : "Yakataga Airport",
         "state" : "Alaska",
         "code" : "PACY"
      },
      "PACA: Cape Spencer Coast Guard Heliport" : {
         "city" : "Cape Spencer",
         "name" : "Cape Spencer Coast Guard Heliport",
         "state" : "Alaska",
         "code" : "PACA"
      },
      "PABL: Buckland Airport" : {
         "city" : "Buckland",
         "name" : "Buckland Airport",
         "state" : "Alaska",
         "code" : "PABL"
      },
      "PASO: Seldovia Airport" : {
         "city" : "Seldovia",
         "name" : "Seldovia Airport",
         "state" : "Alaska",
         "code" : "PASO"
      },
      "PAPH: Port Heiden Airport" : {
         "city" : "Port Heiden",
         "name" : "Port Heiden Airport",
         "state" : "Alaska",
         "code" : "PAPH"
      },
      "PADY: Kongiganak Airport" : {
         "city" : "Kongiganak",
         "name" : "Kongiganak Airport",
         "state" : "Alaska",
         "code" : "PADY"
      },
      "PACS: Cape Sarichef Airport" : {
         "city" : "Cape Sarichef",
         "name" : "Cape Sarichef Airport",
         "state" : "Alaska",
         "code" : "PACS"
      },
      "PFTO: Tok Junction Airport" : {
         "city" : "Tok",
         "name" : "Tok Junction Airport",
         "state" : "Alaska",
         "code" : "PFTO"
      },
      "PAMM: Metlakatla Seaplane Base" : {
         "city" : "Metlakatla",
         "name" : "Metlakatla Seaplane Base",
         "state" : "Alaska",
         "code" : "PAMM"
      },
      "PFAK: Akiak Airport" : {
         "city" : "Akiak",
         "name" : "Akiak Airport",
         "state" : "Alaska",
         "code" : "PFAK"
      },
      "PAGG: Kwigillingok Airport" : {
         "city" : "Kwigillingok",
         "name" : "Kwigillingok Airport",
         "state" : "Alaska",
         "code" : "PAGG"
      },
      "PAJN: Juneau International Airport" : {
         "city" : "Juneau",
         "name" : "Juneau International Airport",
         "state" : "Alaska",
         "code" : "PAJN"
      },
      "PFCB: Chenega Bay Airport" : {
         "city" : "Chenega",
         "name" : "Chenega Bay Airport",
         "state" : "Alaska",
         "code" : "PFCB"
      },
      "PAQC: Klawock Seaplane Base" : {
         "city" : "Klawock",
         "name" : "Klawock Seaplane Base",
         "state" : "Alaska",
         "code" : "PAQC"
      },
      "PFKT: Brevig Mission Airport" : {
         "city" : "Brevig Mission",
         "name" : "Brevig Mission Airport",
         "state" : "Alaska",
         "code" : "PFKT"
      },
      "PAOH: Hoonah Airport" : {
         "city" : "Hoonah",
         "name" : "Hoonah Airport",
         "state" : "Alaska",
         "code" : "PAOH"
      },
      "PAFK: Farewell Lake Seaplane Base" : {
         "city" : "Farewell Lake",
         "name" : "Farewell Lake Seaplane Base",
         "state" : "Alaska",
         "code" : "PAFK"
      },
      "PAKY: Karluk Airport" : {
         "city" : "Karluk",
         "name" : "Karluk Airport",
         "state" : "Alaska",
         "code" : "PAKY"
      },
      "PAWN: Noatak Airport" : {
         "city" : "Noatak",
         "name" : "Noatak Airport",
         "state" : "Alaska",
         "code" : "PAWN"
      },
      "PAKD: Kodiak Municipal Airport" : {
         "city" : "Kodiak",
         "name" : "Kodiak Municipal Airport",
         "state" : "Alaska",
         "code" : "PAKD"
      },
      "PATG: Togiak Airport" : {
         "city" : "Togiak",
         "name" : "Togiak Airport",
         "state" : "Alaska",
         "code" : "PATG"
      },
      "PAOC: Portage Creek Airport" : {
         "city" : "Portage Creek",
         "name" : "Portage Creek Airport",
         "state" : "Alaska",
         "code" : "PAOC"
      },
      "PAHX: Shageluk Airport" : {
         "city" : "Shageluk",
         "name" : "Shageluk Airport",
         "state" : "Alaska",
         "code" : "PAHX"
      },
      "PAJV: Jonesville Mine Airport" : {
         "city" : "Sutton",
         "name" : "Jonesville Mine Airport",
         "state" : "Alaska",
         "code" : "PAJV"
      },
      "PAUN: Unalakleet Airport" : {
         "city" : "Unalakleet",
         "name" : "Unalakleet Airport",
         "state" : "Alaska",
         "code" : "PAUN"
      },
      "PAHO: Homer Airport" : {
         "city" : "Homer",
         "name" : "Homer Airport",
         "state" : "Alaska",
         "code" : "PAHO"
      },
      "PAHC: Holy Cross Airport" : {
         "city" : "Holy Cross",
         "name" : "Holy Cross Airport",
         "state" : "Alaska",
         "code" : "PAHC"
      },
      "PFWS: South Naknek Nr 2 Airport" : {
         "city" : "South Naknek",
         "name" : "South Naknek Nr 2 Airport",
         "state" : "Alaska",
         "code" : "PFWS"
      },
      "PAWM: White Mountain Airport" : {
         "city" : "White Mountain",
         "name" : "White Mountain Airport",
         "state" : "Alaska",
         "code" : "PAWM"
      },
      "PACM: Scammon Bay Airport" : {
         "city" : "Scammon Bay",
         "name" : "Scammon Bay Airport",
         "state" : "Alaska",
         "code" : "PACM"
      },
      "PAPN: Pilot Point Airport" : {
         "city" : "Pilot Point",
         "name" : "Pilot Point Airport",
         "state" : "Alaska",
         "code" : "PAPN"
      },
      "PAAQ: Palmer Municipal Airport" : {
         "city" : "Palmer",
         "name" : "Palmer Municipal Airport",
         "state" : "Alaska",
         "code" : "PAAQ"
      },
      "PAMR: Merrill Field" : {
         "city" : "Anchorage",
         "name" : "Merrill Field",
         "state" : "Alaska",
         "code" : "PAMR"
      },
      "PFYU: Fort Yukon Airport" : {
         "city" : "Fort Yukon",
         "name" : "Fort Yukon Airport",
         "state" : "Alaska",
         "code" : "PFYU"
      },
      "PAFL: Tin Creek Airport" : {
         "city" : "Farewell Lake",
         "name" : "Tin Creek Airport",
         "state" : "Alaska",
         "code" : "PAFL"
      },
      "PANU: Nulato Airport" : {
         "city" : "Nulato",
         "name" : "Nulato Airport",
         "state" : "Alaska",
         "code" : "PANU"
      },
      "PAGL: Golovin Airport" : {
         "city" : "Golovin",
         "name" : "Golovin Airport",
         "state" : "Alaska",
         "code" : "PAGL"
      },
      "PASP: Sheep Mountain Airport" : {
         "city" : "Sheep Mountain",
         "name" : "Sheep Mountain Airport",
         "state" : "Alaska",
         "code" : "PASP"
      },
      "PANA: Napakiak Airport" : {
         "city" : "Napakiak",
         "name" : "Napakiak Airport",
         "state" : "Alaska",
         "code" : "PANA"
      },
      "PAVA: Chevak Airport" : {
         "city" : "Chevak",
         "name" : "Chevak Airport",
         "state" : "Alaska",
         "code" : "PAVA"
      },
      "PAGQ: Big Lake Airport" : {
         "city" : "Big Lake",
         "name" : "Big Lake Airport",
         "state" : "Alaska",
         "code" : "PAGQ"
      },
      "PABR: Wiley Post-Will Rogers Memorial Airport" : {
         "city" : "Barrow",
         "name" : "Wiley Post-Will Rogers Memorial Airport",
         "state" : "Alaska",
         "code" : "PABR"
      },
      "PAKK: Koyuk Alfred Adams Airport" : {
         "city" : "Koyuk",
         "name" : "Koyuk Alfred Adams Airport",
         "state" : "Alaska",
         "code" : "PAKK"
      },
      "PALG: Kalskag Airport" : {
         "city" : "Kalskag",
         "name" : "Kalskag Airport",
         "state" : "Alaska",
         "code" : "PALG"
      },
      "PAUK: Alakanuk Airport" : {
         "city" : "Alakanuk",
         "name" : "Alakanuk Airport",
         "state" : "Alaska",
         "code" : "PAUK"
      },
      "PANO: Nondalton Airport" : {
         "city" : "Nondalton",
         "name" : "Nondalton Airport",
         "state" : "Alaska",
         "code" : "PANO"
      },
      "PADT: Duffy's Tavern Airport" : {
         "city" : "Slana",
         "name" : "Duffy's Tavern Airport",
         "state" : "Alaska",
         "code" : "PADT"
      },
      "PAPR: Prospect Creek Airport" : {
         "city" : "Prospect Creek",
         "name" : "Prospect Creek Airport",
         "state" : "Alaska",
         "code" : "PAPR"
      },
      "PAFV: Five Mile Airport" : {
         "city" : "Five Mile",
         "name" : "Five Mile Airport",
         "state" : "Alaska",
         "code" : "PAFV"
      },
      "PASK: Selawik Airport" : {
         "city" : "Selawik",
         "name" : "Selawik Airport",
         "state" : "Alaska",
         "code" : "PASK"
      },
      "PACX: Coldfoot Airport" : {
         "city" : "Coldfoot",
         "name" : "Coldfoot Airport",
         "state" : "Alaska",
         "code" : "PACX"
      },
      "PADK: Adak Airport" : {
         "city" : "– Adak Airport (Mitchell Field) – Adak Island",
         "name" : "Adak Airport",
         "state" : "Alaska",
         "code" : "PADK"
      },
      "PAKV: Kaltag Airport" : {
         "city" : "Kaltag",
         "name" : "Kaltag Airport",
         "state" : "Alaska",
         "code" : "PAKV"
      },
      "PAKT: Ketchikan International Airport" : {
         "city" : "Ketchikan",
         "name" : "Ketchikan International Airport",
         "state" : "Alaska",
         "code" : "PAKT"
      }
   },
   "Iceland" : {
      "KQNT: Naval Air Station Keflavik" : {
         "city" : "NAS Keflavik",
         "name" : "Naval Air Station Keflavik",
         "state" : "Iceland",
         "code" : "KQNT"
      }
   },
   "Idaho, United States" : {
      "KMYL: McCall Airport" : {
         "city" : "McCall",
         "name" : "McCall Airport",
         "state" : "Idaho",
         "code" : "KMYL"
      },
      "KPIH: Pocatello Regional Airport" : {
         "city" : "Pocatello",
         "name" : "Pocatello Regional Airport",
         "state" : "Idaho",
         "code" : "KPIH"
      },
      "KGNG: Gooding Municipal Airport" : {
         "city" : "Gooding",
         "name" : "Gooding Municipal Airport",
         "state" : "Idaho",
         "code" : "KGNG"
      },
      "KBOI: Boise Airport" : {
         "city" : "Boise",
         "name" : "Boise Airport",
         "state" : "Idaho",
         "code" : "KBOI"
      },
      "KSZT: Sandpoint Airport" : {
         "city" : "Sandpoint",
         "name" : "Sandpoint Airport",
         "state" : "Idaho",
         "code" : "KSZT"
      },
      "KSUN: Friedman Memorial Airport" : {
         "city" : "Hailey",
         "name" : "Friedman Memorial Airport",
         "state" : "Idaho",
         "code" : "KSUN"
      },
      "KBYI: Burley Municipal Airport" : {
         "city" : "Burley",
         "name" : "Burley Municipal Airport",
         "state" : "Idaho",
         "code" : "KBYI"
      },
      "KCOE: Coeur d'Alene Air Terminal" : {
         "city" : "Coeur d'Alene",
         "name" : "Coeur d'Alene Air Terminal",
         "state" : "Idaho",
         "code" : "KCOE"
      },
      "KRXE: Rexburg-Madison County Airport" : {
         "city" : "Rexburg",
         "name" : "Rexburg-Madison County Airport",
         "state" : "Idaho",
         "code" : "KRXE"
      },
      "KAOC: Arco Butte County Airport" : {
         "city" : "Arco",
         "name" : "Arco Butte County Airport",
         "state" : "Idaho",
         "code" : "KAOC"
      },
      "KMUO: Mountain Home Air Force Base" : {
         "city" : "Mountain Home",
         "name" : "Mountain Home Air Force Base",
         "state" : "Idaho",
         "code" : "KMUO"
      },
      "KTWF: Joslin Field - Magic Valley Regional Airport" : {
         "city" : "Twin Falls",
         "name" : "Joslin Field - Magic Valley Regional Airport",
         "state" : "Idaho",
         "code" : "KTWF"
      },
      "KLLJ: Challis Airport" : {
         "city" : "Challis",
         "name" : "Challis Airport",
         "state" : "Idaho",
         "code" : "KLLJ"
      },
      "KLWS: Lewiston-Nez Perce County Airport" : {
         "city" : "Lewiston",
         "name" : "Lewiston-Nez Perce County Airport",
         "state" : "Idaho",
         "code" : "KLWS"
      },
      "KIDA: Idaho Falls Regional Airport" : {
         "city" : "Idaho Falls",
         "name" : "Idaho Falls Regional Airport",
         "state" : "Idaho",
         "code" : "KIDA"
      },
      "KMLD: Malad City Airport" : {
         "city" : "Malad City",
         "name" : "Malad City Airport",
         "state" : "Idaho",
         "code" : "KMLD"
      },
      "KEUL: Caldwell Industrial Airport" : {
         "city" : "Caldwell",
         "name" : "Caldwell Industrial Airport",
         "state" : "Idaho",
         "code" : "KEUL"
      },
      "KJER: Jerome County Airport" : {
         "city" : "Jerome",
         "name" : "Jerome County Airport",
         "state" : "Idaho",
         "code" : "KJER"
      },
      "KSMN: Lemhi County Airport" : {
         "city" : "Salmon",
         "name" : "Lemhi County Airport",
         "state" : "Idaho",
         "code" : "KSMN"
      }
   },
   "West Virginia, United States" : {
      "KMGW: Morgantown Municipal Airport" : {
         "city" : "Morgantown",
         "name" : "Morgantown Municipal Airport",
         "state" : "West Virginia",
         "code" : "KMGW"
      },
      "KSXL: Summersville Airport" : {
         "city" : "Summersville",
         "name" : "Summersville Airport",
         "state" : "West Virginia",
         "code" : "KSXL"
      },
      "KHLG: Wheeling-Ohio County Airport" : {
         "city" : "Wheeling",
         "name" : "Wheeling-Ohio County Airport",
         "state" : "West Virginia",
         "code" : "KHLG"
      },
      "KBKW: Beckley Raleigh County Memorial Airport" : {
         "city" : "Beckley",
         "name" : "Beckley Raleigh County Memorial Airport",
         "state" : "West Virginia",
         "code" : "KBKW"
      },
      "KCKB: Harrison/Marion Regional Airport" : {
         "city" : "Clarksburg",
         "name" : "Harrison/Marion Regional Airport",
         "state" : "West Virginia",
         "code" : "KCKB"
      },
      "KPKB: Mid-Ohio Valley Regional Airport" : {
         "city" : "Parkersburg",
         "name" : "Mid-Ohio Valley Regional Airport",
         "state" : "West Virginia",
         "code" : "KPKB"
      },
      "KHTS: Tri-State Airport" : {
         "city" : "Huntington",
         "name" : "Tri-State Airport",
         "state" : "West Virginia",
         "code" : "KHTS"
      },
      "KMRB: Eastern West Virginia Regional Airport" : {
         "city" : "Martinsburg",
         "name" : "Eastern West Virginia Regional Airport",
         "state" : "West Virginia",
         "code" : "KMRB"
      },
      "KEKN: Elkins-Randolph County Airport" : {
         "city" : "Elkins",
         "name" : "Elkins-Randolph County Airport",
         "state" : "West Virginia",
         "code" : "KEKN"
      },
      "KCRW: Yeager Airport" : {
         "city" : "Charleston",
         "name" : "Yeager Airport",
         "state" : "West Virginia",
         "code" : "KCRW"
      },
      "KBLF: Mercer County Airport (West Virginia)" : {
         "city" : "Bluefield",
         "name" : "Mercer County Airport (West Virginia)",
         "state" : "West Virginia",
         "code" : "KBLF"
      },
      "KLWB: Greenbrier Valley Airport" : {
         "city" : "Lewisburg",
         "name" : "Greenbrier Valley Airport",
         "state" : "West Virginia",
         "code" : "KLWB"
      }
   },
   "South Carolina, United States" : {
      "KCEU: Oconee County Regional Airport" : {
         "city" : "Clemson",
         "name" : "Oconee County Regional Airport",
         "state" : "South Carolina",
         "code" : "KCEU"
      },
      "KHYW: Conway-Horry County Airport" : {
         "city" : "Conway",
         "name" : "Conway-Horry County Airport",
         "state" : "South Carolina",
         "code" : "KHYW"
      },
      "KCRE: Grand Strand Airport" : {
         "city" : "North Myrtle Beach",
         "name" : "Grand Strand Airport",
         "state" : "South Carolina",
         "code" : "KCRE"
      },
      "KMNI: Santee Cooper Regional Airport" : {
         "city" : "Manning",
         "name" : "Santee Cooper Regional Airport",
         "state" : "South Carolina",
         "code" : "KMNI"
      },
      "KMAO: Marion County Airport (South Carolina)" : {
         "city" : "Marion",
         "name" : "Marion County Airport (South Carolina)",
         "state" : "South Carolina",
         "code" : "KMAO"
      },
      "KAND: Anderson Regional Airport" : {
         "city" : "Anderson",
         "name" : "Anderson Regional Airport",
         "state" : "South Carolina",
         "code" : "KAND"
      },
      "KGGE: Georgetown County Airport" : {
         "city" : "Georgetown",
         "name" : "Georgetown County Airport",
         "state" : "South Carolina",
         "code" : "KGGE"
      },
      "KCQW: Cheraw Municipal Airport" : {
         "city" : "- Cheraw Municipal Airport - Chesterfield County",
         "name" : "Cheraw Municipal Airport",
         "state" : "South Carolina",
         "code" : "KCQW"
      },
      "KBBP: Marlboro County Jetport" : {
         "city" : "Bennettsville",
         "name" : "Marlboro County Jetport",
         "state" : "South Carolina",
         "code" : "KBBP"
      },
      "KRBW: Lowcountry Regional Airport" : {
         "city" : "Walterboro",
         "name" : "Lowcountry Regional Airport",
         "state" : "South Carolina",
         "code" : "KRBW"
      },
      "KCDN: Woodward Field (airport)" : {
         "city" : "Camden",
         "name" : "Woodward Field (airport)",
         "state" : "South Carolina",
         "code" : "KCDN"
      },
      "KPHH: Swinnie Airport" : {
         "city" : "Andrews",
         "name" : "Swinnie Airport",
         "state" : "South Carolina",
         "code" : "KPHH"
      },
      "KSPA: Spartanburg Downtown Memorial Airport" : {
         "city" : "Spartanburg",
         "name" : "Spartanburg Downtown Memorial Airport",
         "state" : "South Carolina",
         "code" : "KSPA"
      },
      "KJZI: Charleston Executive Airport" : {
         "city" : "Charleston",
         "name" : "Charleston Executive Airport",
         "state" : "South Carolina",
         "code" : "KJZI"
      },
      "KCAE: Columbia Metropolitan Airport" : {
         "city" : "West Columbia",
         "name" : "Columbia Metropolitan Airport",
         "state" : "South Carolina",
         "code" : "KCAE"
      },
      "KMYR: Myrtle Beach International Airport" : {
         "city" : "Myrtle Beach",
         "name" : "Myrtle Beach International Airport",
         "state" : "South Carolina",
         "code" : "KMYR"
      },
      "KSMS: Sumter Airport" : {
         "city" : "Sumter",
         "name" : "Sumter Airport",
         "state" : "South Carolina",
         "code" : "KSMS"
      },
      "KARW: Beaufort County Airport" : {
         "city" : "- Beaufort County Airport - Beaufort County",
         "name" : "Beaufort County Airport",
         "state" : "South Carolina",
         "code" : "KARW"
      },
      "KAIK: Aiken Municipal Airport" : {
         "city" : "Aiken",
         "name" : "Aiken Municipal Airport",
         "state" : "South Carolina",
         "code" : "KAIK"
      },
      "KHXD: Hilton Head Airport" : {
         "city" : "Hilton Head Island",
         "name" : "Hilton Head Airport",
         "state" : "South Carolina",
         "code" : "KHXD"
      },
      "KCKI: Williamsburg Regional Airport" : {
         "city" : "Kingstree",
         "name" : "Williamsburg Regional Airport",
         "state" : "South Carolina",
         "code" : "KCKI"
      },
      "KLKR: Lancaster County Airport" : {
         "city" : "Lancaster",
         "name" : "Lancaster County Airport",
         "state" : "South Carolina",
         "code" : "KLKR"
      },
      "KGRD: Greenwood County Airport" : {
         "city" : "Greenwood",
         "name" : "Greenwood County Airport",
         "state" : "South Carolina",
         "code" : "KGRD"
      },
      "KHVS: Hartsville Regional Airport" : {
         "city" : "Hartsville",
         "name" : "Hartsville Regional Airport",
         "state" : "South Carolina",
         "code" : "KHVS"
      },
      "KMMT: McEntire Joint National Guard Base" : {
         "city" : "Eastover",
         "name" : "McEntire Joint National Guard Base",
         "state" : "South Carolina",
         "code" : "KMMT"
      },
      "KSSC: Shaw AFB" : {
         "city" : "Sumter",
         "name" : "Shaw AFB",
         "state" : "South Carolina",
         "code" : "KSSC"
      },
      "KFDW: Fairfield County Airport (South Carolina)" : {
         "city" : "Winnsboro",
         "name" : "Fairfield County Airport (South Carolina)",
         "state" : "South Carolina",
         "code" : "KFDW"
      },
      "KFLO: Florence Regional Airport" : {
         "city" : "Florence",
         "name" : "Florence Regional Airport",
         "state" : "South Carolina",
         "code" : "KFLO"
      },
      "KLKQ: Pickens County Airport (South Carolina)" : {
         "city" : "Pickens",
         "name" : "Pickens County Airport (South Carolina)",
         "state" : "South Carolina",
         "code" : "KLKQ"
      },
      "KXNO: North Air Force Auxiliary Field" : {
         "city" : "North",
         "name" : "North Air Force Auxiliary Field",
         "state" : "South Carolina",
         "code" : "KXNO"
      },
      "KBNL: Barnwell County Airport" : {
         "city" : "Barnwell",
         "name" : "Barnwell County Airport",
         "state" : "South Carolina",
         "code" : "KBNL"
      },
      "KCHS: Charleston International Airport" : {
         "city" : "Charleston",
         "name" : "Charleston International Airport",
         "state" : "South Carolina",
         "code" : "KCHS"
      },
      "KUZA: Rock Hill/York County Airport" : {
         "city" : "Rock Hill",
         "name" : "Rock Hill/York County Airport",
         "state" : "South Carolina",
         "code" : "KUZA"
      },
      "KUDG: Darlington County Jetport" : {
         "city" : "Dalington",
         "name" : "Darlington County Jetport",
         "state" : "South Carolina",
         "code" : "KUDG"
      },
      "KNBC: Marine Corps Air Station Beaufort" : {
         "city" : "Beaufort",
         "name" : "Marine Corps Air Station Beaufort",
         "state" : "South Carolina",
         "code" : "KNBC"
      },
      "KGYH: Donaldson Center Airport" : {
         "city" : "Greenville",
         "name" : "Donaldson Center Airport",
         "state" : "South Carolina",
         "code" : "KGYH"
      },
      "KDYB: Summerville Airport" : {
         "city" : "Summerville",
         "name" : "Summerville Airport",
         "state" : "South Carolina",
         "code" : "KDYB"
      },
      "KCUB: Columbia Owens Downtown Airport" : {
         "city" : "Columbia",
         "name" : "Columbia Owens Downtown Airport",
         "state" : "South Carolina",
         "code" : "KCUB"
      },
      "KDLC: Dillon County Airport" : {
         "city" : "Dillon",
         "name" : "Dillon County Airport",
         "state" : "South Carolina",
         "code" : "KDLC"
      },
      "KOGB: Orangeburg Municipal Airport" : {
         "city" : "Orangeburg",
         "name" : "Orangeburg Municipal Airport",
         "state" : "South Carolina",
         "code" : "KOGB"
      },
      "KGMU: Greenville Downtown Airport" : {
         "city" : "Greenville",
         "name" : "Greenville Downtown Airport",
         "state" : "South Carolina",
         "code" : "KGMU"
      },
      "KPYG: Pageland Airport" : {
         "city" : "Pageland",
         "name" : "Pageland Airport",
         "state" : "South Carolina",
         "code" : "KPYG"
      },
      "KGSP: Greenville-Spartanburg International Airport" : {
         "city" : "Greer",
         "name" : "Greenville-Spartanburg International Airport",
         "state" : "South Carolina",
         "code" : "KGSP"
      }
   },
   "DC, United States" : {
      "KIAD: Washington Dulles International Airport" : {
         "city" : "Washington",
         "name" : "Washington Dulles International Airport",
         "state" : "DC",
         "code" : "KIAD"
      }
   },
   "New York, United States" : {
      "KIAG: Niagara Falls International Airport" : {
         "city" : "Niagara Falls",
         "name" : "Niagara Falls International Airport",
         "state" : "New York",
         "code" : "KIAG"
      },
      "KBUF: Buffalo Niagara International Airport" : {
         "city" : "Buffalo",
         "name" : "Buffalo Niagara International Airport",
         "state" : "New York",
         "code" : "KBUF"
      },
      "KROC: Greater Rochester International Airport" : {
         "city" : "Rochester",
         "name" : "Greater Rochester International Airport",
         "state" : "New York",
         "code" : "KROC"
      },
      "KPEO: Penn Yan Airport" : {
         "city" : "Penn Yan",
         "name" : "Penn Yan Airport",
         "state" : "New York",
         "code" : "KPEO"
      },
      "KART: Watertown International Airport" : {
         "city" : "Watertown",
         "name" : "Watertown International Airport",
         "state" : "New York",
         "code" : "KART"
      },
      "KJRA: West 30th Street Heliport" : {
         "city" : "West 30th Street Heliport - Manhattan",
         "name" : "West 30th Street Heliport",
         "state" : "New York",
         "code" : "KJRA"
      },
      "KHWV: Brookhaven Airport" : {
         "city" : "Shirley",
         "name" : "Brookhaven Airport",
         "state" : "New York",
         "code" : "KHWV"
      },
      "KLGA: LaGuardia International Airport" : {
         "city" : "New York",
         "name" : "LaGuardia International Airport",
         "state" : "New York",
         "code" : "KLGA"
      },
      "KBGM: Greater Binghamton Airport" : {
         "city" : "Binghamton",
         "name" : "Greater Binghamton Airport",
         "state" : "New York",
         "code" : "KBGM"
      },
      "KOIC: Norwich Lt. Warren Eaton Airport" : {
         "city" : "Norwich",
         "name" : "Norwich Lt. Warren Eaton Airport",
         "state" : "New York",
         "code" : "KOIC"
      },
      "KDSV: Dansville Municipal Airport" : {
         "city" : "Dansville",
         "name" : "Dansville Municipal Airport",
         "state" : "New York",
         "code" : "KDSV"
      },
      "KBQR: Buffalo-Lancaster Airport" : {
         "city" : "Lancaster",
         "name" : "Buffalo-Lancaster Airport",
         "state" : "New York",
         "code" : "KBQR"
      },
      "KSDC: Williamson-Sodus Airport" : {
         "city" : "Williamson",
         "name" : "Williamson-Sodus Airport",
         "state" : "New York",
         "code" : "KSDC"
      },
      "KPBG: Plattsburgh International Airport" : {
         "city" : "Plattsburg",
         "name" : "Plattsburgh International Airport",
         "state" : "New York",
         "code" : "KPBG"
      },
      "KOLE: Cattaraugus County-Olean Airport" : {
         "city" : "Olean",
         "name" : "Cattaraugus County-Olean Airport",
         "state" : "New York",
         "code" : "KOLE"
      },
      "KJHW: Chautauqua County-Jamestown Airport" : {
         "city" : "Jamestown",
         "name" : "Chautauqua County-Jamestown Airport",
         "state" : "New York",
         "code" : "KJHW"
      },
      "KSLK: Adirondack Regional Airport" : {
         "city" : "Saranac Lake",
         "name" : "Adirondack Regional Airport",
         "state" : "New York",
         "code" : "KSLK"
      },
      "KOGS: Ogdensburg International Airport" : {
         "city" : "Ogdensburg",
         "name" : "Ogdensburg International Airport",
         "state" : "New York",
         "code" : "KOGS"
      },
      "KGFL: Bennett Memorial Airport" : {
         "city" : "Glens Falls",
         "name" : "Bennett Memorial Airport",
         "state" : "New York",
         "code" : "KGFL"
      },
      "KHPN: Westchester County Airport" : {
         "city" : "White Plains",
         "name" : "Westchester County Airport",
         "state" : "New York",
         "code" : "KHPN"
      },
      "KMSS: Massena International Airport" : {
         "city" : "Massena",
         "name" : "Massena International Airport",
         "state" : "New York",
         "code" : "KMSS"
      },
      "KISP: Long Island MacArthur Airport" : {
         "city" : "Islip",
         "name" : "Long Island MacArthur Airport",
         "state" : "New York",
         "code" : "KISP"
      },
      "KJFK: John F. Kennedy International Airport" : {
         "city" : "New York",
         "name" : "John F. Kennedy International Airport",
         "state" : "New York",
         "code" : "KJFK"
      },
      "KFZY: Oswego County Airport" : {
         "city" : "Oswego County Airport – Fulton",
         "name" : "Oswego County Airport",
         "state" : "New York",
         "code" : "KFZY"
      },
      "KGVQ: Genesee County Airport" : {
         "city" : "Batavia",
         "name" : "Genesee County Airport",
         "state" : "New York",
         "code" : "KGVQ"
      },
      "KDKK: Chautauqua County/Dunkirk Airport" : {
         "city" : "Dunkirk",
         "name" : "Chautauqua County/Dunkirk Airport",
         "state" : "New York",
         "code" : "KDKK"
      },
      "KSSN: Seneca Army Airfield" : {
         "city" : "Romulus",
         "name" : "Seneca Army Airfield",
         "state" : "New York",
         "code" : "KSSN"
      },
      "KUCA: Oneida County Airport" : {
         "city" : "Utica",
         "name" : "Oneida County Airport",
         "state" : "New York",
         "code" : "KUCA"
      },
      "KGTB: Wheeler-Sack AAF" : {
         "city" : "Fort Drum",
         "name" : "Wheeler-Sack AAF",
         "state" : "New York",
         "code" : "KGTB"
      },
      "KMAL: Malone-Dufort Airport" : {
         "city" : "Malone",
         "name" : "Malone-Dufort Airport",
         "state" : "New York",
         "code" : "KMAL"
      },
      "KSWF: Stewart International Airport" : {
         "city" : "Newburgh",
         "name" : "Stewart International Airport",
         "state" : "New York",
         "code" : "KSWF"
      },
      "KPLB: Clinton County Airport" : {
         "city" : "Plattsburgh",
         "name" : "Clinton County Airport",
         "state" : "New York",
         "code" : "KPLB"
      },
      "KFOK: Francis S. Gabreski Airport" : {
         "city" : "Westhampton Beach",
         "name" : "Francis S. Gabreski Airport",
         "state" : "New York",
         "code" : "KFOK"
      },
      "KPTD: Potsdam Municipal Airport" : {
         "city" : "Potsdam (town)",
         "name" : "Potsdam Municipal Airport",
         "state" : "New York",
         "code" : "KPTD"
      },
      "KHTO: East Hampton Airport" : {
         "city" : "East Hampton (town)",
         "name" : "East Hampton Airport",
         "state" : "New York",
         "code" : "KHTO"
      },
      "KRME: Griffiss Airport" : {
         "city" : "Rome",
         "name" : "Griffiss Airport",
         "state" : "New York",
         "code" : "KRME"
      },
      "KMGJ: Orange County Airport (New York)" : {
         "city" : "Montgomery",
         "name" : "Orange County Airport (New York)",
         "state" : "New York",
         "code" : "KMGJ"
      },
      "KPOU: Dutchess County Airport" : {
         "city" : "Poughkeepsie",
         "name" : "Dutchess County Airport",
         "state" : "New York",
         "code" : "KPOU"
      },
      "KSYR: Syracuse Hancock International Airport" : {
         "city" : "Syracuse",
         "name" : "Syracuse Hancock International Airport",
         "state" : "New York",
         "code" : "KSYR"
      },
      "KELZ: Wellsville Municipal Airport" : {
         "city" : "Wellsville",
         "name" : "Wellsville Municipal Airport",
         "state" : "New York",
         "code" : "KELZ"
      },
      "KCZG: Tri-Cities Airport (New York)" : {
         "city" : "Endicott",
         "name" : "Tri-Cities Airport (New York)",
         "state" : "New York",
         "code" : "KCZG"
      },
      "KELM: Elmira/Corning Regional Airport" : {
         "city" : "Big Flats",
         "name" : "Elmira/Corning Regional Airport",
         "state" : "New York",
         "code" : "KELM"
      },
      "KALB: Albany International Airport" : {
         "city" : "Albany",
         "name" : "Albany International Airport",
         "state" : "New York",
         "code" : "KALB"
      },
      "KFRG: Republic Airport" : {
         "city" : "East Farmingdale",
         "name" : "Republic Airport",
         "state" : "New York",
         "code" : "KFRG"
      },
      "KLKP: Lake Placid Airport" : {
         "city" : "Lake Placid",
         "name" : "Lake Placid Airport",
         "state" : "New York",
         "code" : "KLKP"
      },
      "KMSV: Sullivan County International Airport" : {
         "city" : "Monticello",
         "name" : "Sullivan County International Airport",
         "state" : "New York",
         "code" : "KMSV"
      },
      "KMTP: Montauk Airport" : {
         "city" : "Montauk",
         "name" : "Montauk Airport",
         "state" : "New York",
         "code" : "KMTP"
      },
      "KITH: Ithaca Tompkins Regional Airport" : {
         "city" : "Ithaca",
         "name" : "Ithaca Tompkins Regional Airport",
         "state" : "New York",
         "code" : "KITH"
      },
      "KSCH: Schenectady County Airport" : {
         "city" : "Schenectady",
         "name" : "Schenectady County Airport",
         "state" : "New York",
         "code" : "KSCH"
      }
   },
   "Texas, United States" : {
      "KLFK: Angelina County Airport" : {
         "city" : "Lufkin",
         "name" : "Angelina County Airport",
         "state" : "Texas",
         "code" : "KLFK"
      },
      "KLBR: Clarksville/Red River County Airport" : {
         "city" : "Clarksville",
         "name" : "Clarksville/Red River County Airport",
         "state" : "Texas",
         "code" : "KLBR"
      },
      "KABI: Abilene Regional Airport" : {
         "city" : "Abilene",
         "name" : "Abilene Regional Airport",
         "state" : "Texas",
         "code" : "KABI"
      },
      "KGGG: East Texas Regional Airport" : {
         "city" : "Longview",
         "name" : "East Texas Regional Airport",
         "state" : "Texas",
         "code" : "KGGG"
      },
      "KLNC: Lancaster Airport (Texas)" : {
         "city" : "Lancaster",
         "name" : "Lancaster Airport (Texas)",
         "state" : "Texas",
         "code" : "KLNC"
      },
      "KGRK: Killeen-Fort Hood Regional Airport" : {
         "city" : "Killeen",
         "name" : "Killeen-Fort Hood Regional Airport",
         "state" : "Texas",
         "code" : "KGRK"
      },
      "KDTO: Denton Municipal Airport" : {
         "city" : "Denton",
         "name" : "Denton Municipal Airport",
         "state" : "Texas",
         "code" : "KDTO"
      },
      "KCXO: Lone Star Executive Airport" : {
         "city" : "Houston",
         "name" : "Lone Star Executive Airport",
         "state" : "Texas",
         "code" : "KCXO"
      },
      "KMFE: McAllen-Miller International Airport" : {
         "city" : "McAllen",
         "name" : "McAllen-Miller International Airport",
         "state" : "Texas",
         "code" : "KMFE"
      },
      "KFTW: Fort Worth Meacham International Airport" : {
         "city" : "Fort Worth",
         "name" : "Fort Worth Meacham International Airport",
         "state" : "Texas",
         "code" : "KFTW"
      },
      "KMNZ: Hamilton Municipal Airport (Texas)" : {
         "city" : "Hamilton",
         "name" : "Hamilton Municipal Airport (Texas)",
         "state" : "Texas",
         "code" : "KMNZ"
      },
      "KXBP: Bridgeport Municipal Airport" : {
         "city" : "Bridgeport",
         "name" : "Bridgeport Municipal Airport",
         "state" : "Texas",
         "code" : "KXBP"
      },
      "KEBG: South Texas International Airport at Edinburg" : {
         "city" : "Edinburg",
         "name" : "South Texas International Airport at Edinburg",
         "state" : "Texas",
         "code" : "KEBG"
      },
      "KAQO: Llano Municipal Airport" : {
         "city" : "Llano",
         "name" : "Llano Municipal Airport",
         "state" : "Texas",
         "code" : "KAQO"
      },
      "KLZZ: Lampasas Airport" : {
         "city" : "Lampasas",
         "name" : "Lampasas Airport",
         "state" : "Texas",
         "code" : "KLZZ"
      },
      "KORG: Orange County Airport (Texas)" : {
         "city" : "Orange",
         "name" : "Orange County Airport (Texas)",
         "state" : "Texas",
         "code" : "KORG"
      },
      "KBKS: Brooks County Airport" : {
         "city" : "Falfurrias",
         "name" : "Brooks County Airport",
         "state" : "Texas",
         "code" : "KBKS"
      },
      "KCNW: TSTC Waco Airport" : {
         "city" : "Waco",
         "name" : "TSTC Waco Airport",
         "state" : "Texas",
         "code" : "KCNW"
      },
      "KRYW: Rusty Allen Airport" : {
         "city" : "Lago Vista",
         "name" : "Rusty Allen Airport",
         "state" : "Texas",
         "code" : "KRYW"
      },
      "KBMT: Beaumont Municipal Airport" : {
         "city" : "Beaumont",
         "name" : "Beaumont Municipal Airport",
         "state" : "Texas",
         "code" : "KBMT"
      },
      "KATA: Hall-Miller Municipal Airport" : {
         "city" : "Atlanta",
         "name" : "Hall-Miller Municipal Airport",
         "state" : "Texas",
         "code" : "KATA"
      },
      "KSLR: Sulphur Springs Municipal Airport" : {
         "city" : "Sulphur Springs",
         "name" : "Sulphur Springs Municipal Airport",
         "state" : "Texas",
         "code" : "KSLR"
      },
      "KLHB: Hearne Municipal Airport" : {
         "city" : "Hearne",
         "name" : "Hearne Municipal Airport",
         "state" : "Texas",
         "code" : "KLHB"
      },
      "KTYR: Tyler Pounds Regional Airport" : {
         "city" : "Tyler",
         "name" : "Tyler Pounds Regional Airport",
         "state" : "Texas",
         "code" : "KTYR"
      },
      "KSWI: Sherman Municipal Airport" : {
         "city" : "Sherman",
         "name" : "Sherman Municipal Airport",
         "state" : "Texas",
         "code" : "KSWI"
      },
      "KSSF: Stinson Municipal Airport" : {
         "city" : "San Antonio",
         "name" : "Stinson Municipal Airport",
         "state" : "Texas",
         "code" : "KSSF"
      },
      "KRBD: Dallas Executive Airport" : {
         "city" : "Dallas",
         "name" : "Dallas Executive Airport",
         "state" : "Texas",
         "code" : "KRBD"
      },
      "KGVT: Majors Airport (Texas)" : {
         "city" : "Greenville",
         "name" : "Majors Airport (Texas)",
         "state" : "Texas",
         "code" : "KGVT"
      },
      "KGSW: Greater Southwest International Airport" : {
         "city" : "Fort Worth",
         "name" : "Greater Southwest International Airport",
         "state" : "Texas",
         "code" : "KGSW"
      },
      "KLBB: Lubbock Preston Smith International Airport" : {
         "city" : "Lubbock",
         "name" : "Lubbock Preston Smith International Airport",
         "state" : "Texas",
         "code" : "KLBB"
      },
      "KILE: Skylark Field" : {
         "city" : "Killeen",
         "name" : "Skylark Field",
         "state" : "Texas",
         "code" : "KILE"
      },
      "KDWH: David Wayne Hooks Memorial Airport" : {
         "city" : "Spring",
         "name" : "David Wayne Hooks Memorial Airport",
         "state" : "Texas",
         "code" : "KDWH"
      },
      "KAMA: Rick Husband Amarillo International Airport" : {
         "city" : "Amarillo",
         "name" : "Rick Husband Amarillo International Airport",
         "state" : "Texas",
         "code" : "KAMA"
      },
      "KUTS: Huntsville Municipal Airport (Texas)" : {
         "city" : "Huntsville",
         "name" : "Huntsville Municipal Airport (Texas)",
         "state" : "Texas",
         "code" : "KUTS"
      },
      "KALI: Alice International Airport" : {
         "city" : "Alice",
         "name" : "Alice International Airport",
         "state" : "Texas",
         "code" : "KALI"
      },
      "KCOT: Cotulla-La Salle County Airport" : {
         "city" : "Cotulla",
         "name" : "Cotulla-La Salle County Airport",
         "state" : "Texas",
         "code" : "KCOT"
      },
      "KDKR: Houston County Airport (Texas)" : {
         "city" : "Crockett",
         "name" : "Houston County Airport (Texas)",
         "state" : "Texas",
         "code" : "KDKR"
      },
      "KASL: Harrison County Airport" : {
         "city" : "Marshall",
         "name" : "Harrison County Airport",
         "state" : "Texas",
         "code" : "KASL"
      },
      "KBAZ: New Braunfels Municipal Airport" : {
         "city" : "New Braunfels",
         "name" : "New Braunfels Municipal Airport",
         "state" : "Texas",
         "code" : "KBAZ"
      },
      "KNGP: Naval Air Station Corpus Christi" : {
         "city" : "Corpus Christi",
         "name" : "Naval Air Station Corpus Christi",
         "state" : "Texas",
         "code" : "KNGP"
      },
      "KGLS: Scholes International Airport at Galveston" : {
         "city" : "Galveston",
         "name" : "Scholes International Airport at Galveston",
         "state" : "Texas",
         "code" : "KGLS"
      },
      "KADS: Addison Airport" : {
         "city" : "Addison",
         "name" : "Addison Airport",
         "state" : "Texas",
         "code" : "KADS"
      },
      "KRWV: Caldwell Municipal Airport (Texas)" : {
         "city" : "Caldwell",
         "name" : "Caldwell Municipal Airport (Texas)",
         "state" : "Texas",
         "code" : "KRWV"
      },
      "KELA: Eagle Lake Airport" : {
         "city" : "Eagle Lake",
         "name" : "Eagle Lake Airport",
         "state" : "Texas",
         "code" : "KELA"
      },
      "KECU: Edwards County Airport" : {
         "city" : "Rocksprings",
         "name" : "Edwards County Airport",
         "state" : "Texas",
         "code" : "KECU"
      },
      "KFWS: Fort Worth Spinks Airport" : {
         "city" : "Fort Worth",
         "name" : "Fort Worth Spinks Airport",
         "state" : "Texas",
         "code" : "KFWS"
      },
      "KCPT: Cleburne Municipal Airport" : {
         "city" : "Cleburne",
         "name" : "Cleburne Municipal Airport",
         "state" : "Texas",
         "code" : "KCPT"
      },
      "KCDS: Childress Municipal Airport" : {
         "city" : "Childress",
         "name" : "Childress Municipal Airport",
         "state" : "Texas",
         "code" : "KCDS"
      },
      "KBPG: Big Spring Mc Mahon-Wrinkle Airport" : {
         "city" : "Big Spring",
         "name" : "Big Spring Mc Mahon-Wrinkle Airport",
         "state" : "Texas",
         "code" : "KBPG"
      },
      "KDHT: Dalhart Municipal Airport" : {
         "city" : "Dalhart",
         "name" : "Dalhart Municipal Airport",
         "state" : "Texas",
         "code" : "KDHT"
      },
      "KEYQ: Weiser Air Park" : {
         "city" : "Houston",
         "name" : "Weiser Air Park",
         "state" : "Texas",
         "code" : "KEYQ"
      },
      "KBBD: Curtis Field" : {
         "city" : "Brady",
         "name" : "Curtis Field",
         "state" : "Texas",
         "code" : "KBBD"
      },
      "KHYI: San Marcos Municipal Airport" : {
         "city" : "San Marcos",
         "name" : "San Marcos Municipal Airport",
         "state" : "Texas",
         "code" : "KHYI"
      },
      "KACT: Waco Regional Airport" : {
         "city" : "Waco",
         "name" : "Waco Regional Airport",
         "state" : "Texas",
         "code" : "KACT"
      },
      "KSAT: San Antonio International Airport" : {
         "city" : "San Antonio",
         "name" : "San Antonio International Airport",
         "state" : "Texas",
         "code" : "KSAT"
      },
      "KDAL: Dallas Love Field" : {
         "city" : "Dallas",
         "name" : "Dallas Love Field",
         "state" : "Texas",
         "code" : "KDAL"
      },
      "KEFD: Ellington Field" : {
         "city" : "Houston",
         "name" : "Ellington Field",
         "state" : "Texas",
         "code" : "KEFD"
      },
      "KTME: Houston Executive Airport" : {
         "city" : "Waller County",
         "name" : "Houston Executive Airport",
         "state" : "Texas",
         "code" : "KTME"
      },
      "KPRX: Cox Field" : {
         "city" : "Paris",
         "name" : "Cox Field",
         "state" : "Texas",
         "code" : "KPRX"
      },
      "KGKY: Arlington Municipal Airport (Texas)" : {
         "city" : "Arlington",
         "name" : "Arlington Municipal Airport (Texas)",
         "state" : "Texas",
         "code" : "KGKY"
      },
      "KHQZ: Mesquite Metro Airport" : {
         "city" : "Mesquite",
         "name" : "Mesquite Metro Airport",
         "state" : "Texas",
         "code" : "KHQZ"
      },
      "KBGD: Hutchinson County Airport" : {
         "city" : "Borger",
         "name" : "Hutchinson County Airport",
         "state" : "Texas",
         "code" : "KBGD"
      },
      "KSPS: Wichita Falls Municipal Airport" : {
         "city" : "Wichita Falls",
         "name" : "Wichita Falls Municipal Airport",
         "state" : "Texas",
         "code" : "KSPS"
      },
      "KHDO: Hondo Municipal Airport" : {
         "city" : "Hondo",
         "name" : "Hondo Municipal Airport",
         "state" : "Texas",
         "code" : "KHDO"
      },
      "KIAH: George Bush Intercontinental Airport" : {
         "city" : "Houston",
         "name" : "George Bush Intercontinental Airport",
         "state" : "Texas",
         "code" : "KIAH"
      },
      "KCFD: Coulter Field" : {
         "city" : "Bryan",
         "name" : "Coulter Field",
         "state" : "Texas",
         "code" : "KCFD"
      },
      "KDLF: Laughlin Air Force Base" : {
         "city" : "Del Rio",
         "name" : "Laughlin Air Force Base",
         "state" : "Texas",
         "code" : "KDLF"
      },
      "KGDJ: Granbury Municipal Airport" : {
         "city" : "Granbury",
         "name" : "Granbury Municipal Airport",
         "state" : "Texas",
         "code" : "KGDJ"
      },
      "KBYY: Bay City Municipal Airport" : {
         "city" : "Bay City",
         "name" : "Bay City Municipal Airport",
         "state" : "Texas",
         "code" : "KBYY"
      },
      "KERV: Kerrville Municipal Airport" : {
         "city" : "Kerrville",
         "name" : "Kerrville Municipal Airport",
         "state" : "Texas",
         "code" : "KERV"
      },
      "KSEP: Clark Field Municipal Airport" : {
         "city" : "Stephenville",
         "name" : "Clark Field Municipal Airport",
         "state" : "Texas",
         "code" : "KSEP"
      },
      "KSPX: Houston Gulf Airport" : {
         "city" : "League City",
         "name" : "Houston Gulf Airport",
         "state" : "Texas",
         "code" : "KSPX"
      },
      "KBIF: Biggs Army Airfield" : {
         "city" : "El Paso",
         "name" : "Biggs Army Airfield",
         "state" : "Texas",
         "code" : "KBIF"
      },
      "KDYS: Dyess Air Force Base" : {
         "city" : "Abilene",
         "name" : "Dyess Air Force Base",
         "state" : "Texas",
         "code" : "KDYS"
      },
      "KETN: Eastland Municipal Airport" : {
         "city" : "Eastland",
         "name" : "Eastland Municipal Airport",
         "state" : "Texas",
         "code" : "KETN"
      },
      "KTKI: Collin County Regional Airport at McKinney" : {
         "city" : "McKinney",
         "name" : "Collin County Regional Airport at McKinney",
         "state" : "Texas",
         "code" : "KTKI"
      },
      "KHRL: Valley International Airport" : {
         "city" : "Harlingen",
         "name" : "Valley International Airport",
         "state" : "Texas",
         "code" : "KHRL"
      },
      "KBFE: Terry County Airport" : {
         "city" : "Brownfield",
         "name" : "Terry County Airport",
         "state" : "Texas",
         "code" : "KBFE"
      },
      "KGTU: Georgetown Municipal Airport" : {
         "city" : "Georgetown",
         "name" : "Georgetown Municipal Airport",
         "state" : "Texas",
         "code" : "KGTU"
      },
      "KSKF: Kelly Field Annex" : {
         "city" : "San Antonio",
         "name" : "Kelly Field Annex",
         "state" : "Texas",
         "code" : "KSKF"
      },
      "KARM: Wharton Regional Airport" : {
         "city" : "Wharton",
         "name" : "Wharton Regional Airport",
         "state" : "Texas",
         "code" : "KARM"
      },
      "KCRP: Corpus Christi International Airport" : {
         "city" : "Corpus Christi",
         "name" : "Corpus Christi International Airport",
         "state" : "Texas",
         "code" : "KCRP"
      },
      "KAFW: Fort Worth Alliance Airport" : {
         "city" : "Fort Worth",
         "name" : "Fort Worth Alliance Airport",
         "state" : "Texas",
         "code" : "KAFW"
      },
      "KGYI: Grayson County Airport" : {
         "city" : "Denison",
         "name" : "Grayson County Airport",
         "state" : "Texas",
         "code" : "KGYI"
      },
      "KBRO: Brownsville/South Padre Island International Airport" : {
         "city" : "Brownsville",
         "name" : "Brownsville/South Padre Island International Airport",
         "state" : "Texas",
         "code" : "KBRO"
      },
      "KVCT: Victoria Regional Airport" : {
         "city" : "Victoria",
         "name" : "Victoria Regional Airport",
         "state" : "Texas",
         "code" : "KVCT"
      },
      "KLRD: Laredo International Airport" : {
         "city" : "Laredo",
         "name" : "Laredo International Airport",
         "state" : "Texas",
         "code" : "KLRD"
      },
      "KNWL: Naval Outlying Field Waldron" : {
         "city" : "Corpus Christi",
         "name" : "Naval Outlying Field Waldron",
         "state" : "Texas",
         "code" : "KNWL"
      },
      "KAXH: Houston-Southwest Airport" : {
         "city" : "Houston",
         "name" : "Houston-Southwest Airport",
         "state" : "Texas",
         "code" : "KAXH"
      },
      "KBKD: Stephens County Airport" : {
         "city" : "Breckenridge",
         "name" : "Stephens County Airport",
         "state" : "Texas",
         "code" : "KBKD"
      },
      "KAUS: Austin-Bergstrom International Airport" : {
         "city" : "Austin",
         "name" : "Austin-Bergstrom International Airport",
         "state" : "Texas",
         "code" : "KAUS"
      },
      "KMAF: Midland International Airport" : {
         "city" : "Midland",
         "name" : "Midland International Airport",
         "state" : "Texas",
         "code" : "KMAF"
      },
      "KINJ: Hillsboro Municipal Airport (Texas)" : {
         "city" : "Hillsboro",
         "name" : "Hillsboro Municipal Airport (Texas)",
         "state" : "Texas",
         "code" : "KINJ"
      },
      "KDFW: Dallas-Fort Worth International Airport" : {
         "city" : "Fort Worth",
         "name" : "Dallas-Fort Worth International Airport",
         "state" : "Texas",
         "code" : "KDFW"
      },
      "KCRS: Corsicana Municipal Airport" : {
         "city" : "Corsicana",
         "name" : "Corsicana Municipal Airport",
         "state" : "Texas",
         "code" : "KCRS"
      },
      "KDRT: Del Rio International Airport" : {
         "city" : "Del Rio",
         "name" : "Del Rio International Airport",
         "state" : "Texas",
         "code" : "KDRT"
      },
      "KBMQ: Burnet Municipal Airport" : {
         "city" : "Burnet",
         "name" : "Burnet Municipal Airport",
         "state" : "Texas",
         "code" : "KBMQ"
      },
      "KCLL: Easterwood Airport" : {
         "city" : "College Station",
         "name" : "Easterwood Airport",
         "state" : "Texas",
         "code" : "KCLL"
      },
      "KELP: El Paso International Airport" : {
         "city" : "El Paso",
         "name" : "El Paso International Airport",
         "state" : "Texas",
         "code" : "KELP"
      },
      "KUVA: Garner Field" : {
         "city" : "Uvalde",
         "name" : "Garner Field",
         "state" : "Texas",
         "code" : "KUVA"
      },
      "KSJT: Mathis Field" : {
         "city" : "San Angelo",
         "name" : "Mathis Field",
         "state" : "Texas",
         "code" : "KSJT"
      },
      "KBWD: Brownwood Regional Airport" : {
         "city" : "Brownwood",
         "name" : "Brownwood Regional Airport",
         "state" : "Texas",
         "code" : "KBWD"
      },
      "KCOM: Coleman Municipal Airport" : {
         "city" : "Coleman",
         "name" : "Coleman Municipal Airport",
         "state" : "Texas",
         "code" : "KCOM"
      },
      "KDUX: Moore County Airport (Texas)" : {
         "city" : "Dumas",
         "name" : "Moore County Airport (Texas)",
         "state" : "Texas",
         "code" : "KDUX"
      },
      "KBPT: Southeast Texas Regional Airport" : {
         "city" : "Nederland",
         "name" : "Southeast Texas Regional Airport",
         "state" : "Texas",
         "code" : "KBPT"
      },
      "KGYB: Giddings-Lee County Airport" : {
         "city" : "Giddings",
         "name" : "Giddings-Lee County Airport",
         "state" : "Texas",
         "code" : "KGYB"
      },
      "KPKV: Calhoun County Airport (Texas)" : {
         "city" : "Port Lavaca",
         "name" : "Calhoun County Airport (Texas)",
         "state" : "Texas",
         "code" : "KPKV"
      },
      "KFST: Fort Stockton-Pecos County Airport" : {
         "city" : "Fort Stockton",
         "name" : "Fort Stockton-Pecos County Airport",
         "state" : "Texas",
         "code" : "KFST"
      },
      "KTPL: Draughon-Miller Central Texas Regional Airport" : {
         "city" : "Temple",
         "name" : "Draughon-Miller Central Texas Regional Airport",
         "state" : "Texas",
         "code" : "KTPL"
      },
      "KHOU: William P. Hobby Airport" : {
         "city" : "Houston",
         "name" : "William P. Hobby Airport",
         "state" : "Texas",
         "code" : "KHOU"
      },
      "KCZT: Dimmit County Airport" : {
         "city" : "Carrizo Springs",
         "name" : "Dimmit County Airport",
         "state" : "Texas",
         "code" : "KCZT"
      }
   },
   "Arizona, United States" : {
      "KDMA: Davis-Monthan Air Force Base" : {
         "city" : "Tucson",
         "name" : "Davis-Monthan Air Force Base",
         "state" : "Arizona",
         "code" : "KDMA"
      },
      "KSEZ: Sedona Airport" : {
         "city" : "Sedona",
         "name" : "Sedona Airport",
         "state" : "Arizona",
         "code" : "KSEZ"
      },
      "KPGA: Page Municipal Airport" : {
         "city" : "Page",
         "name" : "Page Municipal Airport",
         "state" : "Arizona",
         "code" : "KPGA"
      },
      "KCMR: H.A. Clark Memorial Field" : {
         "city" : "Williams",
         "name" : "H.A. Clark Memorial Field",
         "state" : "Arizona",
         "code" : "KCMR"
      },
      "KIWA: Phoenix-Mesa Gateway Airport" : {
         "city" : "Mesa",
         "name" : "Phoenix-Mesa Gateway Airport",
         "state" : "Arizona",
         "code" : "KIWA"
      },
      "KSDL: Scottsdale Municipal Airport" : {
         "city" : "Scottsdale",
         "name" : "Scottsdale Municipal Airport",
         "state" : "Arizona",
         "code" : "KSDL"
      },
      "KIFP: Laughlin/Bullhead International Airport" : {
         "city" : "Bullhead City",
         "name" : "Laughlin/Bullhead International Airport",
         "state" : "Arizona",
         "code" : "KIFP"
      },
      "KAZC: Colorado City Municipal Airport" : {
         "city" : "Colorado City",
         "name" : "Colorado City Municipal Airport",
         "state" : "Arizona",
         "code" : "KAZC"
      },
      "KTYL: Taylor Airport (Arizona)" : {
         "city" : "Taylor",
         "name" : "Taylor Airport (Arizona)",
         "state" : "Arizona",
         "code" : "KTYL"
      },
      "KINW: Winslow-Lindbergh Regional Airport" : {
         "city" : "Winslow",
         "name" : "Winslow-Lindbergh Regional Airport",
         "state" : "Arizona",
         "code" : "KINW"
      },
      "KDGL: Douglas Municipal Airport (Arizona)" : {
         "city" : "Douglas",
         "name" : "Douglas Municipal Airport (Arizona)",
         "state" : "Arizona",
         "code" : "KDGL"
      },
      "KRYN: Ryan Field (airport)" : {
         "city" : "Tucson",
         "name" : "Ryan Field (airport)",
         "state" : "Arizona",
         "code" : "KRYN"
      },
      "KCHD: Chandler Municipal Airport" : {
         "city" : "Chandler",
         "name" : "Chandler Municipal Airport",
         "state" : "Arizona",
         "code" : "KCHD"
      },
      "KLUF: Luke Air Force Base" : {
         "city" : "Glendale",
         "name" : "Luke Air Force Base",
         "state" : "Arizona",
         "code" : "KLUF"
      },
      "KGYR: Goodyear Municipal Airport" : {
         "city" : "Phoenix",
         "name" : "Goodyear Municipal Airport",
         "state" : "Arizona",
         "code" : "KGYR"
      },
      "KRQE: Window Rock Municipal Airport" : {
         "city" : "Window Rock",
         "name" : "Window Rock Municipal Airport",
         "state" : "Arizona",
         "code" : "KRQE"
      },
      "KCGZ: Casa Grande Municipal Airport" : {
         "city" : "Casa Grande",
         "name" : "Casa Grande Municipal Airport",
         "state" : "Arizona",
         "code" : "KCGZ"
      },
      "KPRC: Ernest A. Love Field" : {
         "city" : "Prescott",
         "name" : "Ernest A. Love Field",
         "state" : "Arizona",
         "code" : "KPRC"
      },
      "KIGM: Kingman Airport (Arizona)" : {
         "city" : "Kingman",
         "name" : "Kingman Airport (Arizona)",
         "state" : "Arizona",
         "code" : "KIGM"
      },
      "KSOW: Show Low Regional Airport" : {
         "city" : "Show Low",
         "name" : "Show Low Regional Airport",
         "state" : "Arizona",
         "code" : "KSOW"
      },
      "KDVT: Deer Valley Airport" : {
         "city" : "Phoenix",
         "name" : "Deer Valley Airport",
         "state" : "Arizona",
         "code" : "KDVT"
      },
      "KGCN: Grand Canyon National Park Airport" : {
         "city" : "Grand Canyon National Park Airport – Grand Canyon National Park",
         "name" : "Grand Canyon National Park Airport",
         "state" : "Arizona",
         "code" : "KGCN"
      },
      "KFFZ: Falcon Field (Arizona)" : {
         "city" : "Mesa",
         "name" : "Falcon Field (Arizona)",
         "state" : "Arizona",
         "code" : "KFFZ"
      },
      "KPAN: Payson Municipal Airport" : {
         "city" : "Payson",
         "name" : "Payson Municipal Airport",
         "state" : "Arizona",
         "code" : "KPAN"
      },
      "KGBN: Gila Bend Air Force Auxiliary Field" : {
         "city" : "Gila Bend",
         "name" : "Gila Bend Air Force Auxiliary Field",
         "state" : "Arizona",
         "code" : "KGBN"
      },
      "KOLS: Nogales International Airport" : {
         "city" : "Nogales",
         "name" : "Nogales International Airport",
         "state" : "Arizona",
         "code" : "KOLS"
      },
      "KMZJ: Pinal Airpark" : {
         "city" : "Pinal County",
         "name" : "Pinal Airpark",
         "state" : "Arizona",
         "code" : "KMZJ"
      },
      "KAVQ: Marana Regional Airport" : {
         "city" : "Marana",
         "name" : "Marana Regional Airport",
         "state" : "Arizona",
         "code" : "KAVQ"
      },
      "KGEU: Glendale Municipal Airport" : {
         "city" : "Glendale",
         "name" : "Glendale Municipal Airport",
         "state" : "Arizona",
         "code" : "KGEU"
      },
      "KTUS: Tucson International Airport" : {
         "city" : "Tucson",
         "name" : "Tucson International Airport",
         "state" : "Arizona",
         "code" : "KTUS"
      },
      "KSJN: St. Johns Industrial Air Park" : {
         "city" : "St. Johns",
         "name" : "St. Johns Industrial Air Park",
         "state" : "Arizona",
         "code" : "KSJN"
      },
      "KSAD: Safford Regional Airport" : {
         "city" : "Safford",
         "name" : "Safford Regional Airport",
         "state" : "Arizona",
         "code" : "KSAD"
      },
      "KNYL: Yuma International Airport" : {
         "city" : "Yuma",
         "name" : "Yuma International Airport",
         "state" : "Arizona",
         "code" : "KNYL"
      },
      "KHII: Lake Havasu City Airport" : {
         "city" : "Lake Havasu City",
         "name" : "Lake Havasu City Airport",
         "state" : "Arizona",
         "code" : "KHII"
      },
      "KFLG: Flagstaff Pulliam Airport" : {
         "city" : "Flagstaff",
         "name" : "Flagstaff Pulliam Airport",
         "state" : "Arizona",
         "code" : "KFLG"
      },
      "KPHX: Phoenix Sky Harbor International Airport" : {
         "city" : "Phoenix",
         "name" : "Phoenix Sky Harbor International Airport",
         "state" : "Arizona",
         "code" : "KPHX"
      },
      "KFHU: Sierra Vista Municipal Airport" : {
         "city" : "Sierra Vista",
         "name" : "Sierra Vista Municipal Airport",
         "state" : "Arizona",
         "code" : "KFHU"
      },
      "KBXK: Buckeye Municipal Airport" : {
         "city" : "Buckeye",
         "name" : "Buckeye Municipal Airport",
         "state" : "Arizona",
         "code" : "KBXK"
      },
      "KCFT: Greenlee County Airport" : {
         "city" : "Clifton",
         "name" : "Greenlee County Airport",
         "state" : "Arizona",
         "code" : "KCFT"
      },
      "KNYL: Marine Corps Air Station Yuma" : {
         "city" : "Yuma",
         "name" : "Marine Corps Air Station Yuma",
         "state" : "Arizona",
         "code" : "KNYL"
      },
      "KDUG: Bisbee-Douglas International Airport" : {
         "city" : "Bisbee",
         "name" : "Bisbee-Douglas International Airport",
         "state" : "Arizona",
         "code" : "KDUG"
      },
      "KLGF: Laguna Army Airfield" : {
         "city" : "Yuma",
         "name" : "Laguna Army Airfield",
         "state" : "Arizona",
         "code" : "KLGF"
      }
   },
   "Florida, United States" : {
      "KMTH: Florida Keys Marathon Airport" : {
         "city" : "Marathon",
         "name" : "Florida Keys Marathon Airport",
         "state" : "Florida",
         "code" : "KMTH"
      },
      "KCOF: Patrick Air Force Base" : {
         "city" : "Cocoa Beach",
         "name" : "Patrick Air Force Base",
         "state" : "Florida",
         "code" : "KCOF"
      },
      "KAGR: MacDill AFB Auxiliary Field" : {
         "city" : "Avon Park",
         "name" : "MacDill AFB Auxiliary Field",
         "state" : "Florida",
         "code" : "KAGR"
      },
      "KNEN: Naval Outlying Field Whitehouse" : {
         "city" : "Jacksonville",
         "name" : "Naval Outlying Field Whitehouse",
         "state" : "Florida",
         "code" : "KNEN"
      },
      "KNVI: Naval Outlying Field Pace" : {
         "city" : "Wallace",
         "name" : "Naval Outlying Field Pace",
         "state" : "Florida",
         "code" : "KNVI"
      },
      "KNPA: Naval Air Station Pensacola" : {
         "city" : "Pensacola",
         "name" : "Naval Air Station Pensacola",
         "state" : "Florida",
         "code" : "KNPA"
      },
      "KSFB: Orlando Sanford International Airport" : {
         "city" : "Sanford",
         "name" : "Orlando Sanford International Airport",
         "state" : "Florida",
         "code" : "KSFB"
      },
      "KOMN: Ormond Beach Municipal Airport" : {
         "city" : "Ormond Beach",
         "name" : "Ormond Beach Municipal Airport",
         "state" : "Florida",
         "code" : "KOMN"
      },
      "KFLL: Fort Lauderdale-Hollywood International Airport" : {
         "city" : "Fort Lauderdale",
         "name" : "Fort Lauderdale-Hollywood International Airport",
         "state" : "Florida",
         "code" : "KFLL"
      },
      "KCHN: Wauchula Municipal Airport" : {
         "city" : "Wauchula",
         "name" : "Wauchula Municipal Airport",
         "state" : "Florida",
         "code" : "KCHN"
      },
      "KTPF: Peter O. Knight Airport" : {
         "city" : "Tampa",
         "name" : "Peter O. Knight Airport",
         "state" : "Florida",
         "code" : "KTPF"
      },
      "KLCQ: Lake City Municipal Airport (Florida)" : {
         "city" : "Lake City",
         "name" : "Lake City Municipal Airport (Florida)",
         "state" : "Florida",
         "code" : "KLCQ"
      },
      "KDTS: Destin-Fort Walton Beach Airport" : {
         "city" : "Destin",
         "name" : "Destin-Fort Walton Beach Airport",
         "state" : "Florida",
         "code" : "KDTS"
      },
      "KOBE: Okeechobee County Airport" : {
         "city" : "Okeechobee",
         "name" : "Okeechobee County Airport",
         "state" : "Florida",
         "code" : "KOBE"
      },
      "KCGC: Crystal River Airport" : {
         "city" : "Crystal River",
         "name" : "Crystal River Airport",
         "state" : "Florida",
         "code" : "KCGC"
      },
      "KPCM: Plant City Airport" : {
         "city" : "Plant City",
         "name" : "Plant City Airport",
         "state" : "Florida",
         "code" : "KPCM"
      },
      "KORL: Orlando Executive Airport" : {
         "city" : "Orlando",
         "name" : "Orlando Executive Airport",
         "state" : "Florida",
         "code" : "KORL"
      },
      "KMCO: Orlando International Airport" : {
         "city" : "Orlando",
         "name" : "Orlando International Airport",
         "state" : "Florida",
         "code" : "KMCO"
      },
      "KDED: DeLand Municipal Airport" : {
         "city" : "Deland",
         "name" : "DeLand Municipal Airport",
         "state" : "Florida",
         "code" : "KDED"
      },
      "KAVO: Avon Park Executive Airport" : {
         "city" : "Avon Park",
         "name" : "Avon Park Executive Airport",
         "state" : "Florida",
         "code" : "KAVO"
      },
      "KPNS: Pensacola Regional Airport" : {
         "city" : "Pensacola",
         "name" : "Pensacola Regional Airport",
         "state" : "Florida",
         "code" : "KPNS"
      },
      "KEVB: New Smyrna Beach Municipal Airport" : {
         "city" : "New Smyrna Beach",
         "name" : "New Smyrna Beach Municipal Airport",
         "state" : "Florida",
         "code" : "KEVB"
      },
      "KNKL: Naval Outlying Field Holley" : {
         "city" : "Fort Walton Beach",
         "name" : "Naval Outlying Field Holley",
         "state" : "Florida",
         "code" : "KNKL"
      },
      "KPGD: Charlotte County Airport" : {
         "city" : "Punta Gorda",
         "name" : "Charlotte County Airport",
         "state" : "Florida",
         "code" : "KPGD"
      },
      "KMIA: Miami International Airport" : {
         "city" : "Miami",
         "name" : "Miami International Airport",
         "state" : "Florida",
         "code" : "KMIA"
      },
      "KBKV: Hernando County Airport" : {
         "city" : "Brooksville",
         "name" : "Hernando County Airport",
         "state" : "Florida",
         "code" : "KBKV"
      },
      "KFMY: Page Field" : {
         "city" : "Fort Myers",
         "name" : "Page Field",
         "state" : "Florida",
         "code" : "KFMY"
      },
      "KMCF: MacDill Air Force Base" : {
         "city" : "Tampa",
         "name" : "MacDill Air Force Base",
         "state" : "Florida",
         "code" : "KMCF"
      },
      "KGIF: Winter Haven's Gilbert Airport" : {
         "city" : "Winter Haven",
         "name" : "Winter Haven's Gilbert Airport",
         "state" : "Florida",
         "code" : "KGIF"
      },
      "KGNV: Gainesville Regional Airport" : {
         "city" : "Gainesville",
         "name" : "Gainesville Regional Airport",
         "state" : "Florida",
         "code" : "KGNV"
      },
      "KNQX: Naval Air Station Key West" : {
         "city" : "Boca Chica Key",
         "name" : "Naval Air Station Key West",
         "state" : "Florida",
         "code" : "KNQX"
      },
      "KJAX: Jacksonville International Airport" : {
         "city" : "Jacksonville",
         "name" : "Jacksonville International Airport",
         "state" : "Florida",
         "code" : "KJAX"
      },
      "KAAF: Apalachicola Municipal Airport" : {
         "city" : "Apalachicola",
         "name" : "Apalachicola Municipal Airport",
         "state" : "Florida",
         "code" : "KAAF"
      },
      "KAPF: Naples Municipal Airport" : {
         "city" : "Naples",
         "name" : "Naples Municipal Airport",
         "state" : "Florida",
         "code" : "KAPF"
      },
      "KVNC: Venice Municipal Airport" : {
         "city" : "Venice",
         "name" : "Venice Municipal Airport",
         "state" : "Florida",
         "code" : "KVNC"
      },
      "KLAL: Lakeland Linder Regional Airport" : {
         "city" : "Lakeland",
         "name" : "Lakeland Linder Regional Airport",
         "state" : "Florida",
         "code" : "KLAL"
      },
      "KBOW: Bartow Municipal Airport" : {
         "city" : "Bartow",
         "name" : "Bartow Municipal Airport",
         "state" : "Florida",
         "code" : "KBOW"
      },
      "KVDF: Tampa Executive Airport" : {
         "city" : "Tampa",
         "name" : "Tampa Executive Airport",
         "state" : "Florida",
         "code" : "KVDF"
      },
      "KTLH: Tallahassee Regional Airport" : {
         "city" : "Tallahassee",
         "name" : "Tallahassee Regional Airport",
         "state" : "Florida",
         "code" : "KTLH"
      },
      "KFPR: St. Lucie County International Airport" : {
         "city" : "Fort Pierce",
         "name" : "St. Lucie County International Airport",
         "state" : "Florida",
         "code" : "KFPR"
      },
      "KZPH: Zephyrhills Municipal Airport" : {
         "city" : "Zephyrhills",
         "name" : "Zephyrhills Municipal Airport",
         "state" : "Florida",
         "code" : "KZPH"
      },
      "KMAI: Marianna Municipal Airport" : {
         "city" : "Marianna",
         "name" : "Marianna Municipal Airport",
         "state" : "Florida",
         "code" : "KMAI"
      },
      "KTNT: Dade-Collier Training and Transition Airport" : {
         "city" : "Miami",
         "name" : "Dade-Collier Training and Transition Airport",
         "state" : "Florida",
         "code" : "KTNT"
      },
      "KOPF: Opa-locka Airport" : {
         "city" : "Opa-locka",
         "name" : "Opa-locka Airport",
         "state" : "Florida",
         "code" : "KOPF"
      },
      "KFXE: Fort Lauderdale Executive Airport" : {
         "city" : "Fort Lauderdale",
         "name" : "Fort Lauderdale Executive Airport",
         "state" : "Florida",
         "code" : "KFXE"
      },
      "KTMB: Kendall-Tamiami Executive Airport" : {
         "city" : "Miami",
         "name" : "Kendall-Tamiami Executive Airport",
         "state" : "Florida",
         "code" : "KTMB"
      },
      "KNPI: Naval Outlying Field Site 8" : {
         "city" : "Pensacola",
         "name" : "Naval Outlying Field Site 8",
         "state" : "Florida",
         "code" : "KNPI"
      },
      "KCDK: George T. Lewis Airport" : {
         "city" : "Cedar Key",
         "name" : "George T. Lewis Airport",
         "state" : "Florida",
         "code" : "KCDK"
      },
      "KVPS: Northwest Florida Regional Airport" : {
         "city" : "Valparaiso",
         "name" : "Northwest Florida Regional Airport",
         "state" : "Florida",
         "code" : "KVPS"
      },
      "KSPG: Albert Whitted Airport" : {
         "city" : "St. Petersburg",
         "name" : "Albert Whitted Airport",
         "state" : "Florida",
         "code" : "KSPG"
      },
      "KHEG: Herlong Airport" : {
         "city" : "Jacksonville",
         "name" : "Herlong Airport",
         "state" : "Florida",
         "code" : "KHEG"
      },
      "KMLB: Melbourne International Airport" : {
         "city" : "Melbourne",
         "name" : "Melbourne International Airport",
         "state" : "Florida",
         "code" : "KMLB"
      },
      "KFHB: Fernandina Beach Municipal Airport" : {
         "city" : "Fernandina Beach",
         "name" : "Fernandina Beach Municipal Airport",
         "state" : "Florida",
         "code" : "KFHB"
      },
      "KHST: Homestead Joint Air Reserve Base" : {
         "city" : "Homestead",
         "name" : "Homestead Joint Air Reserve Base",
         "state" : "Florida",
         "code" : "KHST"
      },
      "KXMR: Cape Canaveral Air Force Station Skid Strip" : {
         "city" : "Cocoa Beach",
         "name" : "Cape Canaveral Air Force Station Skid Strip",
         "state" : "Florida",
         "code" : "KXMR"
      },
      "KSUA: Witham Field" : {
         "city" : "Stuart",
         "name" : "Witham Field",
         "state" : "Florida",
         "code" : "KSUA"
      },
      "KPHK: Palm Beach County Glades Airport" : {
         "city" : "Pahokee",
         "name" : "Palm Beach County Glades Airport",
         "state" : "Florida",
         "code" : "KPHK"
      },
      "KLEE: Leesburg International Airport" : {
         "city" : "Leesburg",
         "name" : "Leesburg International Airport",
         "state" : "Florida",
         "code" : "KLEE"
      },
      "KPIE: St. Petersburg-Clearwater International Airport" : {
         "city" : "St. Petersburg",
         "name" : "St. Petersburg-Clearwater International Airport",
         "state" : "Florida",
         "code" : "KPIE"
      },
      "KXFL: Flagler County Airport" : {
         "city" : "Bunnell",
         "name" : "Flagler County Airport",
         "state" : "Florida",
         "code" : "KXFL"
      },
      "KSRQ: Sarasota-Bradenton International Airport" : {
         "city" : "Sarasota",
         "name" : "Sarasota-Bradenton International Airport",
         "state" : "Florida",
         "code" : "KSRQ"
      },
      "KVRB: Vero Beach Municipal Airport" : {
         "city" : "Vero Beach",
         "name" : "Vero Beach Municipal Airport",
         "state" : "Florida",
         "code" : "KVRB"
      },
      "KTIX: Space Coast Regional Airport" : {
         "city" : "Titusville",
         "name" : "Space Coast Regional Airport",
         "state" : "Florida",
         "code" : "KTIX"
      },
      "KSGJ: St. Augustine Airport" : {
         "city" : "St. Augustine",
         "name" : "St. Augustine Airport",
         "state" : "Florida",
         "code" : "KSGJ"
      },
      "KBCT: Boca Raton Airport" : {
         "city" : "Boca Raton",
         "name" : "Boca Raton Airport",
         "state" : "Florida",
         "code" : "KBCT"
      },
      "KEGI: Duke Field" : {
         "city" : "Crestview",
         "name" : "Duke Field",
         "state" : "Florida",
         "code" : "KEGI"
      },
      "KMKY: Marco Island Airport" : {
         "city" : "Naples",
         "name" : "Marco Island Airport",
         "state" : "Florida",
         "code" : "KMKY"
      },
      "KHWO: North Perry Airport" : {
         "city" : "Hollywood",
         "name" : "North Perry Airport",
         "state" : "Florida",
         "code" : "KHWO"
      },
      "KSEF: Sebring Regional Airport" : {
         "city" : "Sebring",
         "name" : "Sebring Regional Airport",
         "state" : "Florida",
         "code" : "KSEF"
      },
      "KRSW: Southwest Florida International Airport" : {
         "city" : "South Fort Myers",
         "name" : "Southwest Florida International Airport",
         "state" : "Florida",
         "code" : "KRSW"
      },
      "KNRQ: Naval Outlying Field Spencer" : {
         "city" : "Pace",
         "name" : "Naval Outlying Field Spencer",
         "state" : "Florida",
         "code" : "KNRQ"
      },
      "KNFJ: Naval Outlying Field Choctaw" : {
         "city" : "Milton",
         "name" : "Naval Outlying Field Choctaw",
         "state" : "Florida",
         "code" : "KNFJ"
      },
      "KNRB: Mayport Naval Station" : {
         "city" : "Jacksonville",
         "name" : "Mayport Naval Station",
         "state" : "Florida",
         "code" : "KNRB"
      },
      "KNUN: Naval Outlying Field Saufley" : {
         "city" : "Pensacola",
         "name" : "Naval Outlying Field Saufley",
         "state" : "Florida",
         "code" : "KNUN"
      },
      "KVQQ: Cecil Field" : {
         "city" : "Jacksonville",
         "name" : "Cecil Field",
         "state" : "Florida",
         "code" : "KVQQ"
      },
      "KLNA: Palm Beach County Park Airport" : {
         "city" : "West Palm Beach",
         "name" : "Palm Beach County Park Airport",
         "state" : "Florida",
         "code" : "KLNA"
      },
      "KOCF: Ocala International Airport" : {
         "city" : "Ocala",
         "name" : "Ocala International Airport",
         "state" : "Florida",
         "code" : "KOCF"
      },
      "KCEW: Bob Sikes Airport" : {
         "city" : "Crestview",
         "name" : "Bob Sikes Airport",
         "state" : "Florida",
         "code" : "KCEW"
      },
      "KEYW: Key West International Airport" : {
         "city" : "Key West",
         "name" : "Key West International Airport",
         "state" : "Florida",
         "code" : "KEYW"
      },
      "KCLW: Clearwater Air Park" : {
         "city" : "Clearwater",
         "name" : "Clearwater Air Park",
         "state" : "Florida",
         "code" : "KCLW"
      },
      "KIMM: Immokalee Airport" : {
         "city" : "Immokalee",
         "name" : "Immokalee Airport",
         "state" : "Florida",
         "code" : "KIMM"
      },
      "KCOI: Merritt Island Airport" : {
         "city" : "Merritt Island",
         "name" : "Merritt Island Airport",
         "state" : "Florida",
         "code" : "KCOI"
      },
      "KCRG: Craig Municipal Airport" : {
         "city" : "Jacksonville",
         "name" : "Craig Municipal Airport",
         "state" : "Florida",
         "code" : "KCRG"
      },
      "KCTY: Cross City Airport" : {
         "city" : "Cross City",
         "name" : "Cross City Airport",
         "state" : "Florida",
         "code" : "KCTY"
      },
      "KTPA: Tampa International Airport" : {
         "city" : "Tampa",
         "name" : "Tampa International Airport",
         "state" : "Florida",
         "code" : "KTPA"
      },
      "KISM: Kissimmee Gateway Airport" : {
         "city" : "Kissimmee",
         "name" : "Kissimmee Gateway Airport",
         "state" : "Florida",
         "code" : "KISM"
      },
      "KDAB: Daytona Beach International Airport" : {
         "city" : "Daytona Beach",
         "name" : "Daytona Beach International Airport",
         "state" : "Florida",
         "code" : "KDAB"
      },
      "KNGS: Naval Outlying Field Santa Rosa" : {
         "city" : "Milton",
         "name" : "Naval Outlying Field Santa Rosa",
         "state" : "Florida",
         "code" : "KNGS"
      },
      "KNIP: Naval Air Station Jacksonville" : {
         "city" : "Jacksonville",
         "name" : "Naval Air Station Jacksonville",
         "state" : "Florida",
         "code" : "KNIP"
      }
   },
   "Colorado, United States" : {
      "KLHX: La Junta Municipal Airport" : {
         "city" : "La Junta",
         "name" : "La Junta Municipal Airport",
         "state" : "Colorado",
         "code" : "KLHX"
      },
      "KGNB: Granby-Grand County Airport" : {
         "city" : "Granby",
         "name" : "Granby-Grand County Airport",
         "state" : "Colorado",
         "code" : "KGNB"
      },
      "KCOS: Colorado Springs Airport" : {
         "city" : "Colorado Springs",
         "name" : "Colorado Springs Airport",
         "state" : "Colorado",
         "code" : "KCOS"
      },
      "KHDN: Yampa Valley Airport" : {
         "city" : "Hayden",
         "name" : "Yampa Valley Airport",
         "state" : "Colorado",
         "code" : "KHDN"
      },
      "KSTK: Sterling Municipal Airport" : {
         "city" : "Sterling",
         "name" : "Sterling Municipal Airport",
         "state" : "Colorado",
         "code" : "KSTK"
      },
      "KGUC: Gunnison County Airport" : {
         "city" : "Gunnison",
         "name" : "Gunnison County Airport",
         "state" : "Colorado",
         "code" : "KGUC"
      },
      "KPUB: Pueblo Memorial Airport" : {
         "city" : "Pueblo",
         "name" : "Pueblo Memorial Airport",
         "state" : "Colorado",
         "code" : "KPUB"
      },
      "KFCS: Butts Army Airfield" : {
         "city" : "Fort Carson",
         "name" : "Butts Army Airfield",
         "state" : "Colorado",
         "code" : "KFCS"
      },
      "KMTJ: Montrose Regional Airport" : {
         "city" : "Montrose",
         "name" : "Montrose Regional Airport",
         "state" : "Colorado",
         "code" : "KMTJ"
      },
      "KLXV: Lake County Airport (Colorado)" : {
         "city" : "Leadville",
         "name" : "Lake County Airport (Colorado)",
         "state" : "Colorado",
         "code" : "KLXV"
      },
      "KPSO: Stevens Field" : {
         "city" : "Pagosa Springs",
         "name" : "Stevens Field",
         "state" : "Colorado",
         "code" : "KPSO"
      },
      "KTEX: Telluride Regional Airport" : {
         "city" : "Telluride",
         "name" : "Telluride Regional Airport",
         "state" : "Colorado",
         "code" : "KTEX"
      },
      "KITR: Kit Carson County Airport" : {
         "city" : "Burlington",
         "name" : "Kit Carson County Airport",
         "state" : "Colorado",
         "code" : "KITR"
      },
      "KBKF: Buckley Air Force Base" : {
         "city" : "Aurora",
         "name" : "Buckley Air Force Base",
         "state" : "Colorado",
         "code" : "KBKF"
      },
      "KCAG: Craig-Moffat Airport" : {
         "city" : "Craig",
         "name" : "Craig-Moffat Airport",
         "state" : "Colorado",
         "code" : "KCAG"
      },
      "KAPA: Centennial Airport" : {
         "city" : "Centennial",
         "name" : "Centennial Airport",
         "state" : "Colorado",
         "code" : "KAPA"
      },
      "KEEO: Meeker Airport" : {
         "city" : "Meeker",
         "name" : "Meeker Airport",
         "state" : "Colorado",
         "code" : "KEEO"
      },
      "KRIL: Garfield County Regional Airport" : {
         "city" : "Rifle",
         "name" : "Garfield County Regional Airport",
         "state" : "Colorado",
         "code" : "KRIL"
      },
      "KAFF: United States Air Force Academy" : {
         "city" : "Colorado Springs",
         "name" : "United States Air Force Academy",
         "state" : "Colorado",
         "code" : "KAFF"
      },
      "KAKO: Colorado Plains Regional Airport" : {
         "city" : "Akron",
         "name" : "Colorado Plains Regional Airport",
         "state" : "Colorado",
         "code" : "KAKO"
      },
      "KHEQ: Holyoke Municipal Airport" : {
         "city" : "Holyoke",
         "name" : "Holyoke Municipal Airport",
         "state" : "Colorado",
         "code" : "KHEQ"
      },
      "KLIC: Limon Municipal Airport" : {
         "city" : "Limon",
         "name" : "Limon Municipal Airport",
         "state" : "Colorado",
         "code" : "KLIC"
      },
      "KEGE: Eagle County Regional Airport" : {
         "city" : "Eagle",
         "name" : "Eagle County Regional Airport",
         "state" : "Colorado",
         "code" : "KEGE"
      },
      "KGWS: Glenwood Springs Municipal Airport" : {
         "city" : "Glenwood Springs",
         "name" : "Glenwood Springs Municipal Airport",
         "state" : "Colorado",
         "code" : "KGWS"
      },
      "KCEZ: Cortez Municipal Airport" : {
         "city" : "Cortez",
         "name" : "Cortez Municipal Airport",
         "state" : "Colorado",
         "code" : "KCEZ"
      },
      "KLAA: Lamar Municipal Airport (Colorado)" : {
         "city" : "Lamar",
         "name" : "Lamar Municipal Airport (Colorado)",
         "state" : "Colorado",
         "code" : "KLAA"
      },
      "KALS: San Luis Valley Regional Airport" : {
         "city" : "Alamosa",
         "name" : "San Luis Valley Regional Airport",
         "state" : "Colorado",
         "code" : "KALS"
      },
      "KFTG: Front Range Airport" : {
         "city" : "Aurora",
         "name" : "Front Range Airport",
         "state" : "Colorado",
         "code" : "KFTG"
      },
      "KGXY: Greeley-Weld County Airport" : {
         "city" : "Greeley",
         "name" : "Greeley-Weld County Airport",
         "state" : "Colorado",
         "code" : "KGXY"
      },
      "KDEN: Denver International Airport" : {
         "city" : "Denver",
         "name" : "Denver International Airport",
         "state" : "Colorado",
         "code" : "KDEN"
      },
      "KBJC: Rocky Mountain Metropolitan Airport" : {
         "city" : "Denver",
         "name" : "Rocky Mountain Metropolitan Airport",
         "state" : "Colorado",
         "code" : "KBJC"
      },
      "KASE: Aspen-Pitkin County Airport" : {
         "city" : "Aspen",
         "name" : "Aspen-Pitkin County Airport",
         "state" : "Colorado",
         "code" : "KASE"
      },
      "KGJT: Grand Junction Regional Airport" : {
         "city" : "Grand Junction",
         "name" : "Grand Junction Regional Airport",
         "state" : "Colorado",
         "code" : "KGJT"
      },
      "KTAD: Perry Stokes Airport" : {
         "city" : "Trinidad",
         "name" : "Perry Stokes Airport",
         "state" : "Colorado",
         "code" : "KTAD"
      },
      "KSBS: Steamboat Springs Airport" : {
         "city" : "Steamboat Springs",
         "name" : "Steamboat Springs Airport",
         "state" : "Colorado",
         "code" : "KSBS"
      },
      "KFNL: Fort Collins-Loveland Municipal Airport" : {
         "city" : "Fort Collins",
         "name" : "Fort Collins-Loveland Municipal Airport",
         "state" : "Colorado",
         "code" : "KFNL"
      },
      "KDRO: Durango-La Plata County Airport" : {
         "city" : "Durango",
         "name" : "Durango-La Plata County Airport",
         "state" : "Colorado",
         "code" : "KDRO"
      },
      "KMVI: Monte Vista Municipal Airport" : {
         "city" : "Monte Vista",
         "name" : "Monte Vista Municipal Airport",
         "state" : "Colorado",
         "code" : "KMVI"
      }
   },
   "Missouri, United States" : {
      "KTRX: Trenton Municipal Airport (Missouri)" : {
         "city" : "Trenton",
         "name" : "Trenton Municipal Airport (Missouri)",
         "state" : "Missouri",
         "code" : "KTRX"
      },
      "KMAW: Malden Municipal Airport" : {
         "city" : "Malden",
         "name" : "Malden Municipal Airport",
         "state" : "Missouri",
         "code" : "KMAW"
      },
      "KDXE: Dexter Municipal Airport" : {
         "city" : "Dexter",
         "name" : "Dexter Municipal Airport",
         "state" : "Missouri",
         "code" : "KDXE"
      },
      "KCHQ: Mississippi County Airport" : {
         "city" : "Charleston",
         "name" : "Mississippi County Airport",
         "state" : "Missouri",
         "code" : "KCHQ"
      },
      "KFAM: Farmington Regional Airport" : {
         "city" : "Farmington",
         "name" : "Farmington Regional Airport",
         "state" : "Missouri",
         "code" : "KFAM"
      },
      "KJEF: Jefferson City Memorial Airport" : {
         "city" : "Jefferson City",
         "name" : "Jefferson City Memorial Airport",
         "state" : "Missouri",
         "code" : "KJEF"
      },
      "KSUS: Spirit of St. Louis Airport" : {
         "city" : "St. Louis",
         "name" : "Spirit of St. Louis Airport",
         "state" : "Missouri",
         "code" : "KSUS"
      },
      "KSIK: Sikeston Memorial Municipal Airport" : {
         "city" : "Sikeston Memorial Municipal Airport - Sikeston",
         "name" : "Sikeston Memorial Municipal Airport",
         "state" : "Missouri",
         "code" : "KSIK"
      },
      "KRAW: Warsaw Municipal Airport (Missouri)" : {
         "city" : "Warsaw",
         "name" : "Warsaw Municipal Airport (Missouri)",
         "state" : "Missouri",
         "code" : "KRAW"
      },
      "KCOU: Columbia Regional Airport" : {
         "city" : "Columbia",
         "name" : "Columbia Regional Airport",
         "state" : "Missouri",
         "code" : "KCOU"
      },
      "KVER: Jesse Viertel Memorial Airport" : {
         "city" : "Boonville",
         "name" : "Jesse Viertel Memorial Airport",
         "state" : "Missouri",
         "code" : "KVER"
      },
      "KLLU: Lamar Municipal Airport (Missouri)" : {
         "city" : "Lamar",
         "name" : "Lamar Municipal Airport (Missouri)",
         "state" : "Missouri",
         "code" : "KLLU"
      },
      "KFTT: Elton Hensley Memorial Airport" : {
         "city" : "Fulton",
         "name" : "Elton Hensley Memorial Airport",
         "state" : "Missouri",
         "code" : "KFTT"
      },
      "KEVU: Northwest Missouri Regional Airport" : {
         "city" : "Maryville",
         "name" : "Northwest Missouri Regional Airport",
         "state" : "Missouri",
         "code" : "KEVU"
      },
      "KMCI: Kansas City International Airport" : {
         "city" : "Kansas City",
         "name" : "Kansas City International Airport",
         "state" : "Missouri",
         "code" : "KMCI"
      },
      "KPLK: M. Graham Clark Field, Taney County Airport" : {
         "city" : "Branson",
         "name" : "M. Graham Clark Field, Taney County Airport",
         "state" : "Missouri",
         "code" : "KPLK"
      },
      "KEIW: County Memorial Airport" : {
         "city" : "New Madrid",
         "name" : "County Memorial Airport",
         "state" : "Missouri",
         "code" : "KEIW"
      },
      "KFES: Festus Memorial Airport" : {
         "city" : "Festus",
         "name" : "Festus Memorial Airport",
         "state" : "Missouri",
         "code" : "KFES"
      },
      "KSGF: Springfield-Branson National Airport" : {
         "city" : "Springfield",
         "name" : "Springfield-Branson National Airport",
         "state" : "Missouri",
         "code" : "KSGF"
      },
      "KLXT: Lee's Summit Municipal Airport" : {
         "city" : "Lee's Summit",
         "name" : "Lee's Summit Municipal Airport",
         "state" : "Missouri",
         "code" : "KLXT"
      },
      "KRCM: Skyhaven Airport (Missouri)" : {
         "city" : "Warrensburg",
         "name" : "Skyhaven Airport (Missouri)",
         "state" : "Missouri",
         "code" : "KRCM"
      },
      "KAIZ: Lee C. Fine Memorial Airport" : {
         "city" : "Lake Ozark",
         "name" : "Lee C. Fine Memorial Airport",
         "state" : "Missouri",
         "code" : "KAIZ"
      },
      "KHAE: Hannibal Regional Airport" : {
         "city" : "Hannibal",
         "name" : "Hannibal Regional Airport",
         "state" : "Missouri",
         "code" : "KHAE"
      },
      "KSZL: Whiteman Air Force Base" : {
         "city" : "Johnson County",
         "name" : "Whiteman Air Force Base",
         "state" : "Missouri",
         "code" : "KSZL"
      },
      "KBUM: Butler Memorial Airport" : {
         "city" : "Butler",
         "name" : "Butler Memorial Airport",
         "state" : "Missouri",
         "code" : "KBUM"
      },
      "KEOS: Neosho Hugh Robinson Airport" : {
         "city" : "Neosho",
         "name" : "Neosho Hugh Robinson Airport",
         "state" : "Missouri",
         "code" : "KEOS"
      },
      "KCGI: Cape Girardeau Regional Airport" : {
         "city" : "Cape Girardeau",
         "name" : "Cape Girardeau Regional Airport",
         "state" : "Missouri",
         "code" : "KCGI"
      },
      "KMHL: Marshall Memorial Municipal Airport" : {
         "city" : "Marshall",
         "name" : "Marshall Memorial Municipal Airport",
         "state" : "Missouri",
         "code" : "KMHL"
      },
      "KSTL: Lambert Saint Louis International Airport" : {
         "city" : "St. Louis",
         "name" : "Lambert Saint Louis International Airport",
         "state" : "Missouri",
         "code" : "KSTL"
      },
      "KCHT: Chillicothe Municipal Airport" : {
         "city" : "Chillicothe",
         "name" : "Chillicothe Municipal Airport",
         "state" : "Missouri",
         "code" : "KCHT"
      },
      "KEZZ: Cameron Memorial Airport" : {
         "city" : "Cameron",
         "name" : "Cameron Memorial Airport",
         "state" : "Missouri",
         "code" : "KEZZ"
      },
      "KAOV: Ava Bill Martin Memorial Airport" : {
         "city" : "Ava",
         "name" : "Ava Bill Martin Memorial Airport",
         "state" : "Missouri",
         "code" : "KAOV"
      },
      "KDMO: Sedalia Memorial Airport" : {
         "city" : "Sedalia",
         "name" : "Sedalia Memorial Airport",
         "state" : "Missouri",
         "code" : "KDMO"
      },
      "KBBG: Branson Airport" : {
         "city" : "Hollister",
         "name" : "Branson Airport",
         "state" : "Missouri",
         "code" : "KBBG"
      }
   },
   "Arkansas, United States" : {
      "KAWM: West Memphis Municipal Airport" : {
         "city" : "West Memphis",
         "name" : "West Memphis Municipal Airport",
         "state" : "Arkansas",
         "code" : "KAWM"
      },
      "KVBT: Bentonville Municipal Airport" : {
         "city" : "Bentonville",
         "name" : "Bentonville Municipal Airport",
         "state" : "Arkansas",
         "code" : "KVBT"
      },
      "KFCY: Forrest City Municipal Airport" : {
         "city" : "Forrest City",
         "name" : "Forrest City Municipal Airport",
         "state" : "Arkansas",
         "code" : "KFCY"
      },
      "KCWS: Dennis F. Cantrell Field" : {
         "city" : "Conway",
         "name" : "Dennis F. Cantrell Field",
         "state" : "Arkansas",
         "code" : "KCWS"
      },
      "KBPK: Ozark Regional Airport" : {
         "city" : "Mountain Home",
         "name" : "Ozark Regional Airport",
         "state" : "Arkansas",
         "code" : "KBPK"
      },
      "KMPJ: Petit Jean Park Airport" : {
         "city" : "Morrilton",
         "name" : "Petit Jean Park Airport",
         "state" : "Arkansas",
         "code" : "KMPJ"
      },
      "KSRC: Searcy Municipal Airport" : {
         "city" : "Searcy",
         "name" : "Searcy Municipal Airport",
         "state" : "Arkansas",
         "code" : "KSRC"
      },
      "KBVX: Batesville Regional Airport" : {
         "city" : "Batesville",
         "name" : "Batesville Regional Airport",
         "state" : "Arkansas",
         "code" : "KBVX"
      },
      "KPGR: Kirk Airport" : {
         "city" : "Paragould",
         "name" : "Kirk Airport",
         "state" : "Arkansas",
         "code" : "KPGR"
      },
      "KASG: Springdale Municipal Airport" : {
         "city" : "Springdale",
         "name" : "Springdale Municipal Airport",
         "state" : "Arkansas",
         "code" : "KASG"
      },
      "KSGT: Stuttgart Municipal Airport" : {
         "city" : "Stuttgart",
         "name" : "Stuttgart Municipal Airport",
         "state" : "Arkansas",
         "code" : "KSGT"
      },
      "KXNA: Northwest Arkansas Regional Airport" : {
         "city" : "Fayetteville",
         "name" : "Northwest Arkansas Regional Airport",
         "state" : "Arkansas",
         "code" : "KXNA"
      },
      "KROG: Rogers Municipal Airport Carter Field" : {
         "city" : "Rogers",
         "name" : "Rogers Municipal Airport Carter Field",
         "state" : "Arkansas",
         "code" : "KROG"
      },
      "KJBR: Jonesboro Municipal Airport" : {
         "city" : "Jonesboro",
         "name" : "Jonesboro Municipal Airport",
         "state" : "Arkansas",
         "code" : "KJBR"
      },
      "KBYH: Arkansas International Airport" : {
         "city" : "Blytheville",
         "name" : "Arkansas International Airport",
         "state" : "Arkansas",
         "code" : "KBYH"
      },
      "KSLG: Smith Field (Arkansas)" : {
         "city" : "Siloam Springs",
         "name" : "Smith Field (Arkansas)",
         "state" : "Arkansas",
         "code" : "KSLG"
      },
      "KELD: South Arkansas Regional Airport at Goodwin Field" : {
         "city" : "El Dorado",
         "name" : "South Arkansas Regional Airport at Goodwin Field",
         "state" : "Arkansas",
         "code" : "KELD"
      },
      "KRBM: Robinson Army Airfield" : {
         "city" : "Camp Robinson",
         "name" : "Robinson Army Airfield",
         "state" : "Arkansas",
         "code" : "KRBM"
      },
      "KARG: Walnut Ridge Regional Airport" : {
         "city" : "Walnut Ridge",
         "name" : "Walnut Ridge Regional Airport",
         "state" : "Arkansas",
         "code" : "KARG"
      },
      "KFSM: Fort Smith Regional Airport" : {
         "city" : "Fort Smith",
         "name" : "Fort Smith Regional Airport",
         "state" : "Arkansas",
         "code" : "KFSM"
      },
      "KLLQ: Monticello Municipal Airport (Arkansas)" : {
         "city" : "Monticello",
         "name" : "Monticello Municipal Airport (Arkansas)",
         "state" : "Arkansas",
         "code" : "KLLQ"
      },
      "KHBZ: Heber Springs Municipal Airport" : {
         "city" : "Heber Springs",
         "name" : "Heber Springs Municipal Airport",
         "state" : "Arkansas",
         "code" : "KHBZ"
      },
      "KBDQ: Morrilton Municipal Airport" : {
         "city" : "Morrilton",
         "name" : "Morrilton Municipal Airport",
         "state" : "Arkansas",
         "code" : "KBDQ"
      },
      "KCDH: Harrell Field" : {
         "city" : "Camden",
         "name" : "Harrell Field",
         "state" : "Arkansas",
         "code" : "KCDH"
      },
      "KFYV: Drake Field" : {
         "city" : "Fayetteville",
         "name" : "Drake Field",
         "state" : "Arkansas",
         "code" : "KFYV"
      },
      "KHKA: Blytheville Municipal Airport" : {
         "city" : "Blytheville",
         "name" : "Blytheville Municipal Airport",
         "state" : "Arkansas",
         "code" : "KHKA"
      },
      "KCRT: Z.M. Jack Stell Field" : {
         "city" : "Crossett",
         "name" : "Z.M. Jack Stell Field",
         "state" : "Arkansas",
         "code" : "KCRT"
      },
      "KCVK: Sharp County Regional Airport" : {
         "city" : "Ash Flat",
         "name" : "Sharp County Regional Airport",
         "state" : "Arkansas",
         "code" : "KCVK"
      },
      "KHRO: Boone County Airport (Arkansas)" : {
         "city" : "Harrison",
         "name" : "Boone County Airport (Arkansas)",
         "state" : "Arkansas",
         "code" : "KHRO"
      },
      "KRUE: Russellville Regional Airport" : {
         "city" : "Russellville",
         "name" : "Russellville Regional Airport",
         "state" : "Arkansas",
         "code" : "KRUE"
      },
      "KCCA: Clinton Municipal Airport (Arkansas)" : {
         "city" : "Clinton",
         "name" : "Clinton Municipal Airport (Arkansas)",
         "state" : "Arkansas",
         "code" : "KCCA"
      },
      "KPBF: Grider Field" : {
         "city" : "Pine Bluff",
         "name" : "Grider Field",
         "state" : "Arkansas",
         "code" : "KPBF"
      },
      "KHEE: Thompson-Robbins Airport" : {
         "city" : "Helena",
         "name" : "Thompson-Robbins Airport",
         "state" : "Arkansas",
         "code" : "KHEE"
      },
      "KHOT: Hot Springs Memorial Field Airport" : {
         "city" : "Hot Springs",
         "name" : "Hot Springs Memorial Field Airport",
         "state" : "Arkansas",
         "code" : "KHOT"
      },
      "KTXK: Texarkana Regional Airport-Webb Field" : {
         "city" : "Texarkana",
         "name" : "Texarkana Regional Airport-Webb Field",
         "state" : "Arkansas",
         "code" : "KTXK"
      },
      "KFLP: Marion County Regional Airport" : {
         "city" : "Flippin",
         "name" : "Marion County Regional Airport",
         "state" : "Arkansas",
         "code" : "KFLP"
      },
      "KLRF: Little Rock Air Force Base" : {
         "city" : "Jacksonville",
         "name" : "Little Rock Air Force Base",
         "state" : "Arkansas",
         "code" : "KLRF"
      },
      "KMXA: Manila Municipal Airport" : {
         "city" : "Manila",
         "name" : "Manila Municipal Airport",
         "state" : "Arkansas",
         "code" : "KMXA"
      },
      "KAGO: Magnolia Municipal Airport" : {
         "city" : "Magnolia",
         "name" : "Magnolia Municipal Airport",
         "state" : "Arkansas",
         "code" : "KAGO"
      },
      "KQCU: Fort Chaffee" : {
         "city" : "Fort Chaffee",
         "name" : "Fort Chaffee",
         "state" : "Arkansas",
         "code" : "KQCU"
      },
      "KAZU: Arrowhead Assault Strip" : {
         "city" : "Fort Chaffee",
         "name" : "Arrowhead Assault Strip",
         "state" : "Arkansas",
         "code" : "KAZU"
      },
      "KSUZ: Saline County Regional Airport" : {
         "city" : "Benton",
         "name" : "Saline County Regional Airport",
         "state" : "Arkansas",
         "code" : "KSUZ"
      },
      "KLIT: Little Rock National Airport" : {
         "city" : "Little Rock",
         "name" : "Little Rock National Airport",
         "state" : "Arkansas",
         "code" : "KLIT"
      },
      "KDEQ: J. Lynn Helms Sevier County Airport" : {
         "city" : "De Queen",
         "name" : "J. Lynn Helms Sevier County Airport",
         "state" : "Arkansas",
         "code" : "KDEQ"
      }
   },
   "Japan" : {
      "KQSM: Misawa Air Base" : {
         "city" : "Misawa AB",
         "name" : "Misawa Air Base",
         "state" : "Japan",
         "code" : "KQSM"
      },
      "KQTA: Naval Air Facility Atsugi" : {
         "city" : "NAF Atsugi",
         "name" : "Naval Air Facility Atsugi",
         "state" : "Japan",
         "code" : "KQTA"
      }
   },
   "Rhode Island, United States" : {
      "KOQU: Quonset State Airport" : {
         "city" : "North Kingstown",
         "name" : "Quonset State Airport",
         "state" : "Rhode Island",
         "code" : "KOQU"
      },
      "KSFZ: North Central State Airport" : {
         "city" : "Pawtucket",
         "name" : "North Central State Airport",
         "state" : "Rhode Island",
         "code" : "KSFZ"
      },
      "KWST: Westerly State Airport" : {
         "city" : "Westerly",
         "name" : "Westerly State Airport",
         "state" : "Rhode Island",
         "code" : "KWST"
      },
      "KPVD: T.F. Green State Airport" : {
         "city" : "Providence",
         "name" : "T.F. Green State Airport",
         "state" : "Rhode Island",
         "code" : "KPVD"
      },
      "KBID: Block Island State Airport" : {
         "city" : "Block Island",
         "name" : "Block Island State Airport",
         "state" : "Rhode Island",
         "code" : "KBID"
      },
      "KUUU: Newport State Airport (Rhode Island)" : {
         "city" : "Newport",
         "name" : "Newport State Airport (Rhode Island)",
         "state" : "Rhode Island",
         "code" : "KUUU"
      }
   },
   "Virginia, United States" : {
      "KEZF: Shannon Airport (Virginia)" : {
         "city" : "Fredericksburg",
         "name" : "Shannon Airport (Virginia)",
         "state" : "Virginia",
         "code" : "KEZF"
      },
      "KFVX: Farmville Regional Airport" : {
         "city" : "Farmville",
         "name" : "Farmville Regional Airport",
         "state" : "Virginia",
         "code" : "KFVX"
      },
      "KNYG: Marine Corps Air Facility Quantico" : {
         "city" : "Quantico",
         "name" : "Marine Corps Air Facility Quantico",
         "state" : "Virginia",
         "code" : "KNYG"
      },
      "KLNP: Lonesome Pine Airport" : {
         "city" : "Wise",
         "name" : "Lonesome Pine Airport",
         "state" : "Virginia",
         "code" : "KLNP"
      },
      "KMFV: Accomack County Airport" : {
         "city" : "Melfa",
         "name" : "Accomack County Airport",
         "state" : "Virginia",
         "code" : "KMFV"
      },
      "KMTV: Blue Ridge Airport" : {
         "city" : "Martinsville",
         "name" : "Blue Ridge Airport",
         "state" : "Virginia",
         "code" : "KMTV"
      },
      "KLFI: Langley Air Force Base" : {
         "city" : "Hampton",
         "name" : "Langley Air Force Base",
         "state" : "Virginia",
         "code" : "KLFI"
      },
      "KBKT: Allen C. Perkinson Airport" : {
         "city" : "Blackstone",
         "name" : "Allen C. Perkinson Airport",
         "state" : "Virginia",
         "code" : "KBKT"
      },
      "KSFQ: Suffolk Executive Airport" : {
         "city" : "Suffolk",
         "name" : "Suffolk Executive Airport",
         "state" : "Virginia",
         "code" : "KSFQ"
      },
      "KHSP: Ingalls Field" : {
         "city" : "Hot Springs",
         "name" : "Ingalls Field",
         "state" : "Virginia",
         "code" : "KHSP"
      },
      "KCPK: Chesapeake Regional Airport" : {
         "city" : "Norfolk",
         "name" : "Chesapeake Regional Airport",
         "state" : "Virginia",
         "code" : "KCPK"
      },
      "KFRR: Front Royal-Warren County Airport" : {
         "city" : "Front Royal",
         "name" : "Front Royal-Warren County Airport",
         "state" : "Virginia",
         "code" : "KFRR"
      },
      "KJGG: Williamsbrug-Jamestown Airport" : {
         "city" : "Williamsburg",
         "name" : "Williamsbrug-Jamestown Airport",
         "state" : "Virginia",
         "code" : "KJGG"
      },
      "KNFE: Fentress NALF" : {
         "city" : "Fentress",
         "name" : "Fentress NALF",
         "state" : "Virginia",
         "code" : "KNFE"
      },
      "KFKN: Franklin Municipal Airport" : {
         "city" : "Franklin",
         "name" : "Franklin Municipal Airport",
         "state" : "Virginia",
         "code" : "KFKN"
      },
      "KVJI: Virginia Highlands Airport" : {
         "city" : "Abingdon",
         "name" : "Virginia Highlands Airport",
         "state" : "Virginia",
         "code" : "KVJI"
      },
      "KLKU: Louisa County Airport" : {
         "city" : "Louisa",
         "name" : "Louisa County Airport",
         "state" : "Virginia",
         "code" : "KLKU"
      },
      "KLVL: Lawrenceville/Brunswick Municipal Airport" : {
         "city" : "Lawrenceville",
         "name" : "Lawrenceville/Brunswick Municipal Airport",
         "state" : "Virginia",
         "code" : "KLVL"
      },
      "KCJR: Culpeper Regional Airport" : {
         "city" : "Culpeper",
         "name" : "Culpeper Regional Airport",
         "state" : "Virginia",
         "code" : "KCJR"
      },
      "KBCB: VirginiaTech/Montgomery Executive Airport" : {
         "city" : "Blacksburg",
         "name" : "VirginiaTech/Montgomery Executive Airport",
         "state" : "Virginia",
         "code" : "KBCB"
      },
      "KAPH: A.P. Hill Army Airfield" : {
         "city" : "– A.P. Hill Army Airfield – Fort A.P. Hill",
         "name" : "A.P. Hill Army Airfield",
         "state" : "Virginia",
         "code" : "KAPH"
      },
      "KPTB: Dinwiddie County Airport" : {
         "city" : "Petersburg",
         "name" : "Dinwiddie County Airport",
         "state" : "Virginia",
         "code" : "KPTB"
      },
      "KNGU: Norfolk Naval Station" : {
         "city" : "Norfolk",
         "name" : "Norfolk Naval Station",
         "state" : "Virginia",
         "code" : "KNGU"
      },
      "KMKJ: Mountain Empire Airport" : {
         "city" : "Marion-Wytheville",
         "name" : "Mountain Empire Airport",
         "state" : "Virginia",
         "code" : "KMKJ"
      },
      "KPVG: Hampton Roads Executive Airport" : {
         "city" : "Norfolk",
         "name" : "Hampton Roads Executive Airport",
         "state" : "Virginia",
         "code" : "KPVG"
      },
      "KHEF: Manassas Regional Airport" : {
         "city" : "Manassas",
         "name" : "Manassas Regional Airport",
         "state" : "Virginia",
         "code" : "KHEF"
      },
      "KSHD: Shenandoah Valley Regional Airport" : {
         "city" : "Staunton",
         "name" : "Shenandoah Valley Regional Airport",
         "state" : "Virginia",
         "code" : "KSHD"
      },
      "KRIC: Richmond International Airport" : {
         "city" : "Sandston",
         "name" : "Richmond International Airport",
         "state" : "Virginia",
         "code" : "KRIC"
      },
      "KFCI: Chesterfield County Airport" : {
         "city" : "Richmond",
         "name" : "Chesterfield County Airport",
         "state" : "Virginia",
         "code" : "KFCI"
      },
      "KOKV: Winchester Regional Airport" : {
         "city" : "Winchester",
         "name" : "Winchester Regional Airport",
         "state" : "Virginia",
         "code" : "KOKV"
      },
      "KFAF: Felker Army Airfield" : {
         "city" : "Fort Eustis",
         "name" : "Felker Army Airfield",
         "state" : "Virginia",
         "code" : "KFAF"
      },
      "KRMN: Stafford Regional Airport" : {
         "city" : "Stafford",
         "name" : "Stafford Regional Airport",
         "state" : "Virginia",
         "code" : "KRMN"
      },
      "KOFP: Hanover County Municipal Airport" : {
         "city" : "Richmond",
         "name" : "Hanover County Municipal Airport",
         "state" : "Virginia",
         "code" : "KOFP"
      },
      "KOMH: Orange County Airport (Virginia)" : {
         "city" : "Orange",
         "name" : "Orange County Airport (Virginia)",
         "state" : "Virginia",
         "code" : "KOMH"
      },
      "KHLX: Twin County Airport" : {
         "city" : "Galax-Hillsville",
         "name" : "Twin County Airport",
         "state" : "Virginia",
         "code" : "KHLX"
      },
      "KCXE: Chase City Municipal Airport" : {
         "city" : "Chase City",
         "name" : "Chase City Municipal Airport",
         "state" : "Virginia",
         "code" : "KCXE"
      },
      "KFYJ: Middle Peninsula Regional Airport" : {
         "city" : "West Point",
         "name" : "Middle Peninsula Regional Airport",
         "state" : "Virginia",
         "code" : "KFYJ"
      },
      "KDAN: Danville Regional Airport" : {
         "city" : "Danville",
         "name" : "Danville Regional Airport",
         "state" : "Virginia",
         "code" : "KDAN"
      },
      "KLYH: Lynchburg Regional Airport" : {
         "city" : "Lynchburg",
         "name" : "Lynchburg Regional Airport",
         "state" : "Virginia",
         "code" : "KLYH"
      },
      "KNDY: Dahlgren Naval Surface Warfare Center" : {
         "city" : "Dahlgren",
         "name" : "Dahlgren Naval Surface Warfare Center",
         "state" : "Virginia",
         "code" : "KNDY"
      },
      "KWAL: Wallops Flight Facility" : {
         "city" : "Wallops Flight Facility – Wallops Island",
         "name" : "Wallops Flight Facility",
         "state" : "Virginia",
         "code" : "KWAL"
      },
      "KAKQ: Wakefield Municipal Airport" : {
         "city" : "Wakefield",
         "name" : "Wakefield Municipal Airport",
         "state" : "Virginia",
         "code" : "KAKQ"
      },
      "KDAA: Davison Army Airfield" : {
         "city" : "Fort Belvoir",
         "name" : "Davison Army Airfield",
         "state" : "Virginia",
         "code" : "KDAA"
      },
      "KEMV: Emporia-Greensville Regional Airport" : {
         "city" : "Emporia",
         "name" : "Emporia-Greensville Regional Airport",
         "state" : "Virginia",
         "code" : "KEMV"
      },
      "KAVC: Mecklenburg-Brunswick Regional Airport" : {
         "city" : "South Hill",
         "name" : "Mecklenburg-Brunswick Regional Airport",
         "state" : "Virginia",
         "code" : "KAVC"
      },
      "KVBW: Bridgewater Airport" : {
         "city" : "Bridgewater",
         "name" : "Bridgewater Airport",
         "state" : "Virginia",
         "code" : "KVBW"
      },
      "KROA: Roanoke Regional Airport/Woodrum Field" : {
         "city" : "Roanoke",
         "name" : "Roanoke Regional Airport/Woodrum Field",
         "state" : "Virginia",
         "code" : "KROA"
      },
      "KDCA: Ronald Reagan Washington National Airport" : {
         "city" : "Arlington County",
         "name" : "Ronald Reagan Washington National Airport",
         "state" : "Virginia",
         "code" : "KDCA"
      },
      "KPHF: Newport News/Williamsburg International Airport" : {
         "city" : "Newport News",
         "name" : "Newport News/Williamsburg International Airport",
         "state" : "Virginia",
         "code" : "KPHF"
      },
      "KGDY: Grundy Municipal Airport" : {
         "city" : "Grundy",
         "name" : "Grundy Municipal Airport",
         "state" : "Virginia",
         "code" : "KGDY"
      },
      "KNTU: Naval Air Station Oceana" : {
         "city" : "Virginia Beach",
         "name" : "Naval Air Station Oceana",
         "state" : "Virginia",
         "code" : "KNTU"
      },
      "KPSK: New River Valley Airport" : {
         "city" : "Dublin",
         "name" : "New River Valley Airport",
         "state" : "Virginia",
         "code" : "KPSK"
      },
      "KORF: Norfolk International Airport" : {
         "city" : "Norfolk",
         "name" : "Norfolk International Airport",
         "state" : "Virginia",
         "code" : "KORF"
      },
      "KTGI: Tangier Island Airport" : {
         "city" : "Tangier",
         "name" : "Tangier Island Airport",
         "state" : "Virginia",
         "code" : "KTGI"
      },
      "KCHO: Charlottesville-Albemarle Airport" : {
         "city" : "Charlottesville",
         "name" : "Charlottesville-Albemarle Airport",
         "state" : "Virginia",
         "code" : "KCHO"
      },
      "KGVE: Gordonsville Municipal Airport" : {
         "city" : "Gordonsville",
         "name" : "Gordonsville Municipal Airport",
         "state" : "Virginia",
         "code" : "KGVE"
      },
      "KHWY: Warrenton-Fauquier Airport" : {
         "city" : "Warrenton",
         "name" : "Warrenton-Fauquier Airport",
         "state" : "Virginia",
         "code" : "KHWY"
      },
      "KJYO: Leesburg Executive Airport" : {
         "city" : "Leesburg",
         "name" : "Leesburg Executive Airport",
         "state" : "Virginia",
         "code" : "KJYO"
      }
   },
   "North Carolina, United States" : {
      "KZEF: Elkin Municipal Airport" : {
         "city" : "Elkin",
         "name" : "Elkin Municipal Airport",
         "state" : "North Carolina",
         "code" : "KZEF"
      },
      "KPMZ: Plymouth Municipal Airport (North Carolina)" : {
         "city" : "Plymouth",
         "name" : "Plymouth Municipal Airport (North Carolina)",
         "state" : "North Carolina",
         "code" : "KPMZ"
      },
      "KGSB: Seymour Johnson AFB" : {
         "city" : "Goldsboro",
         "name" : "Seymour Johnson AFB",
         "state" : "North Carolina",
         "code" : "KGSB"
      },
      "KAFP: Anson County Airport" : {
         "city" : "Wadesboro",
         "name" : "Anson County Airport",
         "state" : "North Carolina",
         "code" : "KAFP"
      },
      "KNJM: Marine Corps Auxiliary Landing Field Bogue" : {
         "city" : "Swansboro",
         "name" : "Marine Corps Auxiliary Landing Field Bogue",
         "state" : "North Carolina",
         "code" : "KNJM"
      },
      "KAKH: Gastonia Municipal Airport" : {
         "city" : "Gastonia",
         "name" : "Gastonia Municipal Airport",
         "state" : "North Carolina",
         "code" : "KAKH"
      },
      "KMCZ: Martin County Airport" : {
         "city" : "Williamston",
         "name" : "Martin County Airport",
         "state" : "North Carolina",
         "code" : "KMCZ"
      },
      "KJNX: Johnston County Airport" : {
         "city" : "Smithfield",
         "name" : "Johnston County Airport",
         "state" : "North Carolina",
         "code" : "KJNX"
      },
      "KTDF: Person County Airport" : {
         "city" : "Roxboro",
         "name" : "Person County Airport",
         "state" : "North Carolina",
         "code" : "KTDF"
      },
      "KRWI: Rocky Mount Regional Airport" : {
         "city" : "Rocky Mount",
         "name" : "Rocky Mount Regional Airport",
         "state" : "North Carolina",
         "code" : "KRWI"
      },
      "KFQD: Rutherford County Airport" : {
         "city" : "Rutherfordton",
         "name" : "Rutherford County Airport",
         "state" : "North Carolina",
         "code" : "KFQD"
      },
      "KVUJ: Stanly County Airport" : {
         "city" : "Albemarle",
         "name" : "Stanly County Airport",
         "state" : "North Carolina",
         "code" : "KVUJ"
      },
      "KBUY: Burlington-Alamance Regional Airport" : {
         "city" : "Burlington",
         "name" : "Burlington-Alamance Regional Airport",
         "state" : "North Carolina",
         "code" : "KBUY"
      },
      "KPGV: Pitt-Greenville Airport" : {
         "city" : "Greenville",
         "name" : "Pitt-Greenville Airport",
         "state" : "North Carolina",
         "code" : "KPGV"
      },
      "KEQY: Monroe Regional Airport (North Carolina)" : {
         "city" : "Monroe",
         "name" : "Monroe Regional Airport (North Carolina)",
         "state" : "North Carolina",
         "code" : "KEQY"
      },
      "KISO: Kinston Regional Jetport" : {
         "city" : "Kinston",
         "name" : "Kinston Regional Jetport",
         "state" : "North Carolina",
         "code" : "KISO"
      },
      "KAVL: Asheville Regional Airport" : {
         "city" : "Fletcher",
         "name" : "Asheville Regional Airport",
         "state" : "North Carolina",
         "code" : "KAVL"
      },
      "KRZZ: Halifax County Airport" : {
         "city" : "Roanoke Rapids",
         "name" : "Halifax County Airport",
         "state" : "North Carolina",
         "code" : "KRZZ"
      },
      "KHFF: Mackall Army Airfield" : {
         "city" : "Camp Mackall",
         "name" : "Mackall Army Airfield",
         "state" : "North Carolina",
         "code" : "KHFF"
      },
      "KEHO: Shelby Municipal Airport" : {
         "city" : "Shelby",
         "name" : "Shelby Municipal Airport",
         "state" : "North Carolina",
         "code" : "KEHO"
      },
      "KHBI: Asheboro Regional Airport" : {
         "city" : "Asheboro",
         "name" : "Asheboro Regional Airport",
         "state" : "North Carolina",
         "code" : "KHBI"
      },
      "KNCA: Marine Corps Air Station New River" : {
         "city" : "Jacksonville",
         "name" : "Marine Corps Air Station New River",
         "state" : "North Carolina",
         "code" : "KNCA"
      },
      "KEYF: Curtis L. Brown Jr. Field" : {
         "city" : "Elizabethtown",
         "name" : "Curtis L. Brown Jr. Field",
         "state" : "North Carolina",
         "code" : "KEYF"
      },
      "KETC: Tarboro-Edgecombe Airport" : {
         "city" : "Tarboro",
         "name" : "Tarboro-Edgecombe Airport",
         "state" : "North Carolina",
         "code" : "KETC"
      },
      "KHSE: Billy Mitchell Airport" : {
         "city" : "Hatteras",
         "name" : "Billy Mitchell Airport",
         "state" : "North Carolina",
         "code" : "KHSE"
      },
      "KOCW: Warren Airport" : {
         "city" : "Washington",
         "name" : "Warren Airport",
         "state" : "North Carolina",
         "code" : "KOCW"
      },
      "KJQF: Concord Regional Airport" : {
         "city" : "Concord",
         "name" : "Concord Regional Airport",
         "state" : "North Carolina",
         "code" : "KJQF"
      },
      "KFFA: First Flight Airport" : {
         "city" : "Kill Devil Hills",
         "name" : "First Flight Airport",
         "state" : "North Carolina",
         "code" : "KFFA"
      },
      "KIPJ: Lincoln County Regional Airport" : {
         "city" : "Lincolnton",
         "name" : "Lincoln County Regional Airport",
         "state" : "North Carolina",
         "code" : "KIPJ"
      },
      "KIGX: Horace Williams Airport" : {
         "city" : "Chapel Hill",
         "name" : "Horace Williams Airport",
         "state" : "North Carolina",
         "code" : "KIGX"
      },
      "KINT: Smith Reynolds Airport" : {
         "city" : "Winston-Salem",
         "name" : "Smith Reynolds Airport",
         "state" : "North Carolina",
         "code" : "KINT"
      },
      "KEWN: Craven County Regional Airport" : {
         "city" : "New Bern",
         "name" : "Craven County Regional Airport",
         "state" : "North Carolina",
         "code" : "KEWN"
      },
      "KLHZ: Franklin County Airport (North Carolina)" : {
         "city" : "Louisburg",
         "name" : "Franklin County Airport (North Carolina)",
         "state" : "North Carolina",
         "code" : "KLHZ"
      },
      "KFBG: Simmons Army Airfield" : {
         "city" : "Fort Bragg",
         "name" : "Simmons Army Airfield",
         "state" : "North Carolina",
         "code" : "KFBG"
      },
      "KSOP: Moore County Airport (North Carolina)" : {
         "city" : "Southern Pines",
         "name" : "Moore County Airport (North Carolina)",
         "state" : "North Carolina",
         "code" : "KSOP"
      },
      "KNKT: Marine Corps Air Station Cherry Point" : {
         "city" : "Cherry Point",
         "name" : "Marine Corps Air Station Cherry Point",
         "state" : "North Carolina",
         "code" : "KNKT"
      },
      "KRUQ: Rowan County Airport" : {
         "city" : "Salisbury",
         "name" : "Rowan County Airport",
         "state" : "North Carolina",
         "code" : "KRUQ"
      },
      "KSUT: Brunswick County Airport" : {
         "city" : "Oak Island",
         "name" : "Brunswick County Airport",
         "state" : "North Carolina",
         "code" : "KSUT"
      },
      "KASJ: Tri-County Airport (North Carolina)" : {
         "city" : "Ahoskie",
         "name" : "Tri-County Airport (North Carolina)",
         "state" : "North Carolina",
         "code" : "KASJ"
      },
      "KECG: Elizabeth City-Pasquotank County Regional Airport" : {
         "city" : "Elizabeth City",
         "name" : "Elizabeth City-Pasquotank County Regional Airport",
         "state" : "North Carolina",
         "code" : "KECG"
      },
      "KMRN: Morganton-Lenoir Airport" : {
         "city" : "Morganton",
         "name" : "Morganton-Lenoir Airport",
         "state" : "North Carolina",
         "code" : "KMRN"
      },
      "KMRH: Michael J. Smith Field" : {
         "city" : "Beaufort",
         "name" : "Michael J. Smith Field",
         "state" : "North Carolina",
         "code" : "KMRH"
      },
      "KMEB: Laurinburg-Maxton Airport" : {
         "city" : "Maxton",
         "name" : "Laurinburg-Maxton Airport",
         "state" : "North Carolina",
         "code" : "KMEB"
      },
      "KSVH: Statesville Regional Airport" : {
         "city" : "Statesville",
         "name" : "Statesville Regional Airport",
         "state" : "North Carolina",
         "code" : "KSVH"
      },
      "KEDE: Northeastern Regional Airport" : {
         "city" : "Edenton",
         "name" : "Northeastern Regional Airport",
         "state" : "North Carolina",
         "code" : "KEDE"
      },
      "KTTA: Sanford-Lee County Regional Airport" : {
         "city" : "Sanford",
         "name" : "Sanford-Lee County Regional Airport",
         "state" : "North Carolina",
         "code" : "KTTA"
      },
      "KLBT: Lumberton Municipal Airport" : {
         "city" : "Lumberton",
         "name" : "Lumberton Municipal Airport",
         "state" : "North Carolina",
         "code" : "KLBT"
      },
      "KRDU: Raleigh-Durham International Airport" : {
         "city" : "Raleigh",
         "name" : "Raleigh-Durham International Airport",
         "state" : "North Carolina",
         "code" : "KRDU"
      },
      "KMWK: Mount Airy/Surry County Airport" : {
         "city" : "Mount Airy",
         "name" : "Mount Airy/Surry County Airport",
         "state" : "North Carolina",
         "code" : "KMWK"
      },
      "KGSO: Piedmont Triad International Airport" : {
         "city" : "Greensboro",
         "name" : "Piedmont Triad International Airport",
         "state" : "North Carolina",
         "code" : "KGSO"
      },
      "KEXX: Davidson County Airport" : {
         "city" : "Lexington",
         "name" : "Davidson County Airport",
         "state" : "North Carolina",
         "code" : "KEXX"
      },
      "KCPC: Columbus County Municipal Airport" : {
         "city" : "Whiteville",
         "name" : "Columbus County Municipal Airport",
         "state" : "North Carolina",
         "code" : "KCPC"
      },
      "KILM: Wilmington International Airport" : {
         "city" : "Wilmington",
         "name" : "Wilmington International Airport",
         "state" : "North Carolina",
         "code" : "KILM"
      },
      "KHKY: Hickory Regional Airport" : {
         "city" : "Hickory",
         "name" : "Hickory Regional Airport",
         "state" : "North Carolina",
         "code" : "KHKY"
      },
      "KCTZ: Sampson County Airport" : {
         "city" : "Clinton",
         "name" : "Sampson County Airport",
         "state" : "North Carolina",
         "code" : "KCTZ"
      },
      "KDPL: Duplin County Airport" : {
         "city" : "Kenansville",
         "name" : "Duplin County Airport",
         "state" : "North Carolina",
         "code" : "KDPL"
      },
      "KACZ: Henderson Field (North Carolina)" : {
         "city" : "Wallace",
         "name" : "Henderson Field (North Carolina)",
         "state" : "North Carolina",
         "code" : "KACZ"
      },
      "KGWW: Goldsboro-Wayne Municipal Airport" : {
         "city" : "Goldsboro",
         "name" : "Goldsboro-Wayne Municipal Airport",
         "state" : "North Carolina",
         "code" : "KGWW"
      },
      "KPOB: Pope Airforce Base" : {
         "city" : "Fayetteville",
         "name" : "Pope Airforce Base",
         "state" : "North Carolina",
         "code" : "KPOB"
      },
      "KHNZ: Oxford-Henderson Airport" : {
         "city" : "Oxford",
         "name" : "Oxford-Henderson Airport",
         "state" : "North Carolina",
         "code" : "KHNZ"
      },
      "KCLT: Charlotte/Douglas International Airport" : {
         "city" : "Charlotte",
         "name" : "Charlotte/Douglas International Airport",
         "state" : "North Carolina",
         "code" : "KCLT"
      },
      "KHRJ: Harnett County Airport" : {
         "city" : "Erwin",
         "name" : "Harnett County Airport",
         "state" : "North Carolina",
         "code" : "KHRJ"
      },
      "KONX: Currituck County Airport" : {
         "city" : "Currituck",
         "name" : "Currituck County Airport",
         "state" : "North Carolina",
         "code" : "KONX"
      },
      "KUKF: Wilkes County Airport" : {
         "city" : "North Wilkesboro",
         "name" : "Wilkes County Airport",
         "state" : "North Carolina",
         "code" : "KUKF"
      },
      "KOAJ: Ellis Airport" : {
         "city" : "Jacksonville",
         "name" : "Ellis Airport",
         "state" : "North Carolina",
         "code" : "KOAJ"
      },
      "KRHP: Andrews-Murphy Airport" : {
         "city" : "Andrews",
         "name" : "Andrews-Murphy Airport",
         "state" : "North Carolina",
         "code" : "KRHP"
      },
      "KMQI: Dare County Regional Airport" : {
         "city" : "Manteo",
         "name" : "Dare County Regional Airport",
         "state" : "North Carolina",
         "code" : "KMQI"
      },
      "KGEV: Ashe County Airport" : {
         "city" : "Jefferson",
         "name" : "Ashe County Airport",
         "state" : "North Carolina",
         "code" : "KGEV"
      },
      "KFAY: Fayetteville Regional Airport" : {
         "city" : "Fayetteville",
         "name" : "Fayetteville Regional Airport",
         "state" : "North Carolina",
         "code" : "KFAY"
      }
   },
   "Mississippi, United States" : {
      "KTUP: Tupelo Regional Airport" : {
         "city" : "Tupelo",
         "name" : "Tupelo Regional Airport",
         "state" : "Mississippi",
         "code" : "KTUP"
      },
      "KGTR: Golden Triangle Regional Airport" : {
         "city" : "Columbus",
         "name" : "Golden Triangle Regional Airport",
         "state" : "Mississippi",
         "code" : "KGTR"
      },
      "KCBM: Columbus Air Force Base" : {
         "city" : "Columbus",
         "name" : "Columbus Air Force Base",
         "state" : "Mississippi",
         "code" : "KCBM"
      },
      "KRNV: Cleveland Municipal Airport (Mississippi)" : {
         "city" : "Cleveland",
         "name" : "Cleveland Municipal Airport (Mississippi)",
         "state" : "Mississippi",
         "code" : "KRNV"
      },
      "KGWO: Greenwood-Leflore Airport" : {
         "city" : "Greenwood",
         "name" : "Greenwood-Leflore Airport",
         "state" : "Mississippi",
         "code" : "KGWO"
      },
      "KGPT: Gulfport-Biloxi International Airport" : {
         "city" : "Gulfport",
         "name" : "Gulfport-Biloxi International Airport",
         "state" : "Mississippi",
         "code" : "KGPT"
      },
      "KNJW: Williams NOLF" : {
         "city" : "Meridian",
         "name" : "Williams NOLF",
         "state" : "Mississippi",
         "code" : "KNJW"
      },
      "KMBO: Campbell Airport" : {
         "city" : "Madison",
         "name" : "Campbell Airport",
         "state" : "Mississippi",
         "code" : "KMBO"
      },
      "KNMM: Naval Air Station Meridian" : {
         "city" : "Meridian",
         "name" : "Naval Air Station Meridian",
         "state" : "Mississippi",
         "code" : "KNMM"
      },
      "KVKS: Vicksburg Municipal Airport" : {
         "city" : "Vicksburg",
         "name" : "Vicksburg Municipal Airport",
         "state" : "Mississippi",
         "code" : "KVKS"
      },
      "KIDL: Indianola Municipal Airport" : {
         "city" : "Indianola",
         "name" : "Indianola Municipal Airport",
         "state" : "Mississippi",
         "code" : "KIDL"
      },
      "KMMS: Selfs Airport" : {
         "city" : "Marks",
         "name" : "Selfs Airport",
         "state" : "Mississippi",
         "code" : "KMMS"
      },
      "KCKM: Fletcher Municipal Airport" : {
         "city" : "Clarksdale",
         "name" : "Fletcher Municipal Airport",
         "state" : "Mississippi",
         "code" : "KCKM"
      },
      "KHEZ: Natchez-Adams County Airport" : {
         "city" : "Natchez",
         "name" : "Natchez-Adams County Airport",
         "state" : "Mississippi",
         "code" : "KHEZ"
      },
      "KMPE: Philadelphia Municipal Airport" : {
         "city" : "Philadelphia",
         "name" : "Philadelphia Municipal Airport",
         "state" : "Mississippi",
         "code" : "KMPE"
      },
      "KPCU: Picayune-Pearl River County Airport" : {
         "city" : "Picayune",
         "name" : "Picayune-Pearl River County Airport",
         "state" : "Mississippi",
         "code" : "KPCU"
      },
      "KPIB: Hattiesburg-Laurel Regional Airport" : {
         "city" : "Hattiesburg",
         "name" : "Hattiesburg-Laurel Regional Airport",
         "state" : "Mississippi",
         "code" : "KPIB"
      },
      "KHKS: Hawkins Field (airport)" : {
         "city" : "Jackson",
         "name" : "Hawkins Field (airport)",
         "state" : "Mississippi",
         "code" : "KHKS"
      },
      "KPQL: Trent Lott International Airport" : {
         "city" : "Pascagoula",
         "name" : "Trent Lott International Airport",
         "state" : "Mississippi",
         "code" : "KPQL"
      },
      "KGNF: Grenada Municipal Airport" : {
         "city" : "Grenada",
         "name" : "Grenada Municipal Airport",
         "state" : "Mississippi",
         "code" : "KGNF"
      },
      "KCRX: Roscoe Turner Airport" : {
         "city" : "Corinth",
         "name" : "Roscoe Turner Airport",
         "state" : "Mississippi",
         "code" : "KCRX"
      },
      "KOLV: Olive Branch Airport" : {
         "city" : "Olive Branch",
         "name" : "Olive Branch Airport",
         "state" : "Mississippi",
         "code" : "KOLV"
      },
      "KMCB: McComb-Pike County Airport" : {
         "city" : "McComb",
         "name" : "McComb-Pike County Airport",
         "state" : "Mississippi",
         "code" : "KMCB"
      },
      "KSTF: George M. Bryan Airport" : {
         "city" : "Starkville",
         "name" : "George M. Bryan Airport",
         "state" : "Mississippi",
         "code" : "KSTF"
      },
      "KMEI: Key Airport" : {
         "city" : "Meridian",
         "name" : "Key Airport",
         "state" : "Mississippi",
         "code" : "KMEI"
      },
      "KLMS: Louisville Winston County Airport" : {
         "city" : "Louisville",
         "name" : "Louisville Winston County Airport",
         "state" : "Mississippi",
         "code" : "KLMS"
      },
      "KBIX: Keesler Air Force Base" : {
         "city" : "Biloxi",
         "name" : "Keesler Air Force Base",
         "state" : "Mississippi",
         "code" : "KBIX"
      },
      "KLUL: Hesler-Noble Field" : {
         "city" : "Laurel",
         "name" : "Hesler-Noble Field",
         "state" : "Mississippi",
         "code" : "KLUL"
      },
      "KOSX: Kosciusko Attala County Airport" : {
         "city" : "Kosciusko",
         "name" : "Kosciusko Attala County Airport",
         "state" : "Mississippi",
         "code" : "KOSX"
      },
      "KHSA: Stennis International Airport" : {
         "city" : "Bay St. Louis",
         "name" : "Stennis International Airport",
         "state" : "Mississippi",
         "code" : "KHSA"
      },
      "KGLH: Mid Delta Regional Airport" : {
         "city" : "Greenville",
         "name" : "Mid Delta Regional Airport",
         "state" : "Mississippi",
         "code" : "KGLH"
      },
      "KJAN: Jackson International Airport" : {
         "city" : "Jackson",
         "name" : "Jackson International Airport",
         "state" : "Mississippi",
         "code" : "KJAN"
      },
      "KHBG: Chain Municipal Airport" : {
         "city" : "Hattiesburg",
         "name" : "Chain Municipal Airport",
         "state" : "Mississippi",
         "code" : "KHBG"
      },
      "KUOX: University-Oxford Airport" : {
         "city" : "Oxford",
         "name" : "University-Oxford Airport",
         "state" : "Mississippi",
         "code" : "KUOX"
      },
      "KUBS: Columbus-Lowndes County Airport" : {
         "city" : "Columbus",
         "name" : "Columbus-Lowndes County Airport",
         "state" : "Mississippi",
         "code" : "KUBS"
      }
   },
   "New Jersey, United States" : {
      "KACY: Atlantic City International Airport" : {
         "city" : "Atlantic City",
         "name" : "Atlantic City International Airport",
         "state" : "New Jersey",
         "code" : "KACY"
      },
      "KMIV: Millville Municipal Airport" : {
         "city" : "Millville",
         "name" : "Millville Municipal Airport",
         "state" : "New Jersey",
         "code" : "KMIV"
      },
      "KSMQ: Somerset Airport (New Jersey)" : {
         "city" : "Somerville",
         "name" : "Somerset Airport (New Jersey)",
         "state" : "New Jersey",
         "code" : "KSMQ"
      },
      "KFWN: Sussex Airport (New Jersey)" : {
         "city" : "Sussex",
         "name" : "Sussex Airport (New Jersey)",
         "state" : "New Jersey",
         "code" : "KFWN"
      },
      "KWRI: McGuire Air Force Base" : {
         "city" : "Wrightstown",
         "name" : "McGuire Air Force Base",
         "state" : "New Jersey",
         "code" : "KWRI"
      },
      "KVAY: South Jersey Regional Airport" : {
         "city" : "Mount Holly",
         "name" : "South Jersey Regional Airport",
         "state" : "New Jersey",
         "code" : "KVAY"
      },
      "KMMU: Morristown Municipal Airport" : {
         "city" : "Morristown",
         "name" : "Morristown Municipal Airport",
         "state" : "New Jersey",
         "code" : "KMMU"
      },
      "KAIY: Atlantic City Municipal Airport" : {
         "city" : "Atlantic City",
         "name" : "Atlantic City Municipal Airport",
         "state" : "New Jersey",
         "code" : "KAIY"
      },
      "KBLM: Monmouth Executive Airport" : {
         "city" : "Belmar",
         "name" : "Monmouth Executive Airport",
         "state" : "New Jersey",
         "code" : "KBLM"
      },
      "KLDJ: Linden Airport" : {
         "city" : "Linden",
         "name" : "Linden Airport",
         "state" : "New Jersey",
         "code" : "KLDJ"
      },
      "KCDW: Essex County Airport" : {
         "city" : "Caldwell",
         "name" : "Essex County Airport",
         "state" : "New Jersey",
         "code" : "KCDW"
      },
      "KWWD: Cape May Airport" : {
         "city" : "Wildwood",
         "name" : "Cape May Airport",
         "state" : "New Jersey",
         "code" : "KWWD"
      },
      "KTEB: Teterboro Airport" : {
         "city" : "Teterboro",
         "name" : "Teterboro Airport",
         "state" : "New Jersey",
         "code" : "KTEB"
      },
      "KEWR: Newark Liberty International Airport" : {
         "city" : "Newark",
         "name" : "Newark Liberty International Airport",
         "state" : "New Jersey",
         "code" : "KEWR"
      },
      "KNEL: Naval Air Engineering Station Lakehurst" : {
         "city" : "Lakehurst",
         "name" : "Naval Air Engineering Station Lakehurst",
         "state" : "New Jersey",
         "code" : "KNEL"
      },
      "KTTN: Trenton-Mercer Airport" : {
         "city" : "Trenton",
         "name" : "Trenton-Mercer Airport",
         "state" : "New Jersey",
         "code" : "KTTN"
      },
      "KMJX: Robert J. Miller Air Park" : {
         "city" : "Toms River",
         "name" : "Robert J. Miller Air Park",
         "state" : "New Jersey",
         "code" : "KMJX"
      },
      "KOBI: Woodbine Municipal Airport (New Jersey)" : {
         "city" : "Woodbine",
         "name" : "Woodbine Municipal Airport (New Jersey)",
         "state" : "New Jersey",
         "code" : "KOBI"
      }
   },
   "Kentucky, United States" : {
      "KLEX: Blue Grass Airport" : {
         "city" : "Lexington",
         "name" : "Blue Grass Airport",
         "state" : "Kentucky",
         "code" : "KLEX"
      },
      "KDWU: Ashland-Boyd County Airport" : {
         "city" : "Ashland",
         "name" : "Ashland-Boyd County Airport",
         "state" : "Kentucky",
         "code" : "KDWU"
      },
      "KPBX: Pike County Airport (Kentucky)" : {
         "city" : "Pikeville",
         "name" : "Pike County Airport (Kentucky)",
         "state" : "Kentucky",
         "code" : "KPBX"
      },
      "KDVK: Stuart Powell Field" : {
         "city" : "Danville",
         "name" : "Stuart Powell Field",
         "state" : "Kentucky",
         "code" : "KDVK"
      },
      "KSME: Lake Cumberland Regional Airport" : {
         "city" : "Somerset",
         "name" : "Lake Cumberland Regional Airport",
         "state" : "Kentucky",
         "code" : "KSME"
      },
      "KAAS: Taylor County Airport (Kentucky)" : {
         "city" : "Campbellsville",
         "name" : "Taylor County Airport (Kentucky)",
         "state" : "Kentucky",
         "code" : "KAAS"
      },
      "KSDF: Louisville International Airport" : {
         "city" : "Louisville",
         "name" : "Louisville International Airport",
         "state" : "Kentucky",
         "code" : "KSDF"
      },
      "KOWB: Owensboro-Daviess County Regional Airport" : {
         "city" : "Owensboro",
         "name" : "Owensboro-Daviess County Regional Airport",
         "state" : "Kentucky",
         "code" : "KOWB"
      },
      "KGLW: Glasgow Municipal Airport" : {
         "city" : "Glasgow",
         "name" : "Glasgow Municipal Airport",
         "state" : "Kentucky",
         "code" : "KGLW"
      },
      "KFTK: Godman Army Airfield" : {
         "city" : "Godman Army Airfield – Fort Knox",
         "name" : "Godman Army Airfield",
         "state" : "Kentucky",
         "code" : "KFTK"
      },
      "KEKX: Addington Field" : {
         "city" : "Elizabethtown",
         "name" : "Addington Field",
         "state" : "Kentucky",
         "code" : "KEKX"
      },
      "KHOP: Campbell Army Airfield" : {
         "city" : "Fort Campbell",
         "name" : "Campbell Army Airfield",
         "state" : "Kentucky",
         "code" : "KHOP"
      },
      "KFFT: Capital City Airport (Kentucky)" : {
         "city" : "Frankfort",
         "name" : "Capital City Airport (Kentucky)",
         "state" : "Kentucky",
         "code" : "KFFT"
      },
      "KCEY: Murray-Calloway County Airport" : {
         "city" : "Murray",
         "name" : "Murray-Calloway County Airport",
         "state" : "Kentucky",
         "code" : "KCEY"
      },
      "KBRY: Samuels Field" : {
         "city" : "Bardstown",
         "name" : "Samuels Field",
         "state" : "Kentucky",
         "code" : "KBRY"
      },
      "KCVG: Cincinnati/Northern Kentucky International Airport" : {
         "city" : "Hebron",
         "name" : "Cincinnati/Northern Kentucky International Airport",
         "state" : "Kentucky",
         "code" : "KCVG"
      },
      "KHVC: Hopkinsville-Christian County Airport" : {
         "city" : "Hopkinsville",
         "name" : "Hopkinsville-Christian County Airport",
         "state" : "Kentucky",
         "code" : "KHVC"
      },
      "KEHR: Henderson City-County Airport" : {
         "city" : "Henderson",
         "name" : "Henderson City-County Airport",
         "state" : "Kentucky",
         "code" : "KEHR"
      },
      "KLOZ: London-Corbin Airport" : {
         "city" : "London",
         "name" : "London-Corbin Airport",
         "state" : "Kentucky",
         "code" : "KLOZ"
      },
      "KTWT: Sturgis Municipal Airport (Kentucky)" : {
         "city" : "Sturgis",
         "name" : "Sturgis Municipal Airport (Kentucky)",
         "state" : "Kentucky",
         "code" : "KTWT"
      },
      "KEKQ: Wayne County Airport (Kentucky)" : {
         "city" : "Monticello",
         "name" : "Wayne County Airport (Kentucky)",
         "state" : "Kentucky",
         "code" : "KEKQ"
      },
      "KFGX: Fleming-Mason Airport" : {
         "city" : "Flemingsburg",
         "name" : "Fleming-Mason Airport",
         "state" : "Kentucky",
         "code" : "KFGX"
      },
      "KLOU: Bowman Field (airport)" : {
         "city" : "Louisville",
         "name" : "Bowman Field (airport)",
         "state" : "Kentucky",
         "code" : "KLOU"
      },
      "KBWG: Bowling Green-Warren County Regional Airport" : {
         "city" : "Bowling Green",
         "name" : "Bowling Green-Warren County Regional Airport",
         "state" : "Kentucky",
         "code" : "KBWG"
      }
   },
   "Iowa, United States" : {
      "KAXA: Algona Municipal Airport" : {
         "city" : "Algona",
         "name" : "Algona Municipal Airport",
         "state" : "Iowa",
         "code" : "KAXA"
      },
      "KEST: Estherville Municipal Airport" : {
         "city" : "Estherville",
         "name" : "Estherville Municipal Airport",
         "state" : "Iowa",
         "code" : "KEST"
      },
      "KIIB: Independence Municipal Airport (Iowa)" : {
         "city" : "Independence",
         "name" : "Independence Municipal Airport (Iowa)",
         "state" : "Iowa",
         "code" : "KIIB"
      },
      "KCBF: Council Bluffs Municipal Airport" : {
         "city" : "Council Bluffs",
         "name" : "Council Bluffs Municipal Airport",
         "state" : "Iowa",
         "code" : "KCBF"
      },
      "KCWI: Clinton Municipal Airport (Iowa)" : {
         "city" : "Clinton",
         "name" : "Clinton Municipal Airport (Iowa)",
         "state" : "Iowa",
         "code" : "KCWI"
      },
      "KMUT: Muscatine Municipal Airport" : {
         "city" : "Muscatine",
         "name" : "Muscatine Municipal Airport",
         "state" : "Iowa",
         "code" : "KMUT"
      },
      "KFXY: Forest City Municipal Airport" : {
         "city" : "Forest City",
         "name" : "Forest City Municipal Airport",
         "state" : "Iowa",
         "code" : "KFXY"
      },
      "KEAG: Eagle Grove Municipal Airport" : {
         "city" : "Eagle Grove",
         "name" : "Eagle Grove Municipal Airport",
         "state" : "Iowa",
         "code" : "KEAG"
      },
      "KDSM: Des Moines International Airport" : {
         "city" : "Des Moines",
         "name" : "Des Moines International Airport",
         "state" : "Iowa",
         "code" : "KDSM"
      },
      "KIOW: Iowa City Municipal Airport" : {
         "city" : "Iowa City",
         "name" : "Iowa City Municipal Airport",
         "state" : "Iowa",
         "code" : "KIOW"
      },
      "KBRL: Southeast Iowa Regional Airport" : {
         "city" : "Burlington",
         "name" : "Southeast Iowa Regional Airport",
         "state" : "Iowa",
         "code" : "KBRL"
      },
      "KDVN: Davenport Municipal Airport (Iowa)" : {
         "city" : "Davenport",
         "name" : "Davenport Municipal Airport (Iowa)",
         "state" : "Iowa",
         "code" : "KDVN"
      },
      "KSUX: Sioux Gateway Airport" : {
         "city" : "Sioux City",
         "name" : "Sioux Gateway Airport",
         "state" : "Iowa",
         "code" : "KSUX"
      },
      "KADU: Audubon County Airport" : {
         "city" : "Audubon",
         "name" : "Audubon County Airport",
         "state" : "Iowa",
         "code" : "KADU"
      },
      "KCAV: Clarion Municipal Airport" : {
         "city" : "Clarion",
         "name" : "Clarion Municipal Airport",
         "state" : "Iowa",
         "code" : "KCAV"
      },
      "KDBQ: Dubuque Regional Airport" : {
         "city" : "Dubuque",
         "name" : "Dubuque Regional Airport",
         "state" : "Iowa",
         "code" : "KDBQ"
      },
      "KOTM: Ottumwa Industrial Airport" : {
         "city" : "Ottumwa",
         "name" : "Ottumwa Industrial Airport",
         "state" : "Iowa",
         "code" : "KOTM"
      },
      "KDNS: Denison Municipal Airport" : {
         "city" : "Denison",
         "name" : "Denison Municipal Airport",
         "state" : "Iowa",
         "code" : "KDNS"
      },
      "KCKP: Cherokee Municipal Airport" : {
         "city" : "Cherokee",
         "name" : "Cherokee Municipal Airport",
         "state" : "Iowa",
         "code" : "KCKP"
      },
      "KCNC: Chariton Municipal Airport" : {
         "city" : "Chariton",
         "name" : "Chariton Municipal Airport",
         "state" : "Iowa",
         "code" : "KCNC"
      },
      "KCID: The Eastern Iowa Airport" : {
         "city" : "Cedar Rapids",
         "name" : "The Eastern Iowa Airport",
         "state" : "Iowa",
         "code" : "KCID"
      },
      "KFFL: Fairfield Municipal Airport (Iowa)" : {
         "city" : "Fairfield",
         "name" : "Fairfield Municipal Airport (Iowa)",
         "state" : "Iowa",
         "code" : "KFFL"
      },
      "KGGI: Grinnell Regional Airport" : {
         "city" : "Grinnell",
         "name" : "Grinnell Regional Airport",
         "state" : "Iowa",
         "code" : "KGGI"
      },
      "KEFW: Jefferson Municipal Airport" : {
         "city" : "Jefferson",
         "name" : "Jefferson Municipal Airport",
         "state" : "Iowa",
         "code" : "KEFW"
      },
      "KMCW: Mason City Municipal Airport" : {
         "city" : "Mason City",
         "name" : "Mason City Municipal Airport",
         "state" : "Iowa",
         "code" : "KMCW"
      },
      "KFOD: Fort Dodge Regional Airport" : {
         "city" : "Fort Dodge",
         "name" : "Fort Dodge Regional Airport",
         "state" : "Iowa",
         "code" : "KFOD"
      },
      "KMXO: Monticello Regional Airport" : {
         "city" : "Monticello",
         "name" : "Monticello Regional Airport",
         "state" : "Iowa",
         "code" : "KMXO"
      },
      "KDEH: Decorah Municipal Airport" : {
         "city" : "Decorah",
         "name" : "Decorah Municipal Airport",
         "state" : "Iowa",
         "code" : "KDEH"
      },
      "KBNW: Boone Municipal Airport" : {
         "city" : "Boone",
         "name" : "Boone Municipal Airport",
         "state" : "Iowa",
         "code" : "KBNW"
      },
      "KCIN: Arthur N. Neu Airport" : {
         "city" : "Carroll",
         "name" : "Arthur N. Neu Airport",
         "state" : "Iowa",
         "code" : "KCIN"
      },
      "KEGQ: Emmetsburg Municipal Airport" : {
         "city" : "Emmetsburg",
         "name" : "Emmetsburg Municipal Airport",
         "state" : "Iowa",
         "code" : "KEGQ"
      },
      "KEBS: Webster City Municipal Airport" : {
         "city" : "Webster City",
         "name" : "Webster City Municipal Airport",
         "state" : "Iowa",
         "code" : "KEBS"
      },
      "KAIO: Atlantic Municipal Airport" : {
         "city" : "Atlantic",
         "name" : "Atlantic Municipal Airport",
         "state" : "Iowa",
         "code" : "KAIO"
      },
      "KEOK: Keokuk Municipal Airport" : {
         "city" : "Keokuk",
         "name" : "Keokuk Municipal Airport",
         "state" : "Iowa",
         "code" : "KEOK"
      },
      "KCRZ: Corning Municipal Airport (Iowa)" : {
         "city" : "Corning",
         "name" : "Corning Municipal Airport (Iowa)",
         "state" : "Iowa",
         "code" : "KCRZ"
      },
      "KCSQ: Creston Municipal Airport" : {
         "city" : "Creston",
         "name" : "Creston Municipal Airport",
         "state" : "Iowa",
         "code" : "KCSQ"
      },
      "KSPW: Spencer Municipal Airport" : {
         "city" : "Spencer",
         "name" : "Spencer Municipal Airport",
         "state" : "Iowa",
         "code" : "KSPW"
      },
      "KALO: Waterloo Regional Airport" : {
         "city" : "Waterloo",
         "name" : "Waterloo Regional Airport",
         "state" : "Iowa",
         "code" : "KALO"
      },
      "KOLZ: Oelwein Municipal Airport" : {
         "city" : "Oelwein",
         "name" : "Oelwein Municipal Airport",
         "state" : "Iowa",
         "code" : "KOLZ"
      },
      "KAMW: Ames Municipal Airport" : {
         "city" : "Ames",
         "name" : "Ames Municipal Airport",
         "state" : "Iowa",
         "code" : "KAMW"
      },
      "KTNU: Newton Municipal Airport (Iowa)" : {
         "city" : "Newton",
         "name" : "Newton Municipal Airport (Iowa)",
         "state" : "Iowa",
         "code" : "KTNU"
      },
      "KTZT: Belle Plaine Municipal Airport" : {
         "city" : "Belle Plaine",
         "name" : "Belle Plaine Municipal Airport",
         "state" : "Iowa",
         "code" : "KTZT"
      },
      "KAWG: Washington Municipal Airport" : {
         "city" : "Washington",
         "name" : "Washington Municipal Airport",
         "state" : "Iowa",
         "code" : "KAWG"
      },
      "KCJJ: Ellen Church Field" : {
         "city" : "Cresco",
         "name" : "Ellen Church Field",
         "state" : "Iowa",
         "code" : "KCJJ"
      },
      "KFSW: Fort Madison Municipal Airport" : {
         "city" : "Fort Madison",
         "name" : "Fort Madison Municipal Airport",
         "state" : "Iowa",
         "code" : "KFSW"
      },
      "KCCY: Northeast Iowa Regional Airport" : {
         "city" : "Charles City",
         "name" : "Northeast Iowa Regional Airport",
         "state" : "Iowa",
         "code" : "KCCY"
      }
   },
   "Massachusetts, United States" : {
      "KGHG: Marshfield Municipal Airport (Massachusetts)" : {
         "city" : "Marshfield",
         "name" : "Marshfield Municipal Airport (Massachusetts)",
         "state" : "Massachusetts",
         "code" : "KGHG"
      },
      "KBVY: Beverly Municipal Airport" : {
         "city" : "Beverly",
         "name" : "Beverly Municipal Airport",
         "state" : "Massachusetts",
         "code" : "KBVY"
      },
      "KBAF: Barnes Municipal Airport" : {
         "city" : "Westfield",
         "name" : "Barnes Municipal Airport",
         "state" : "Massachusetts",
         "code" : "KBAF"
      },
      "KPVC: Provincetown Municipal Airport" : {
         "city" : "Provincetown",
         "name" : "Provincetown Municipal Airport",
         "state" : "Massachusetts",
         "code" : "KPVC"
      },
      "KGDM: Gardner Municipal Airport" : {
         "city" : "Gardner",
         "name" : "Gardner Municipal Airport",
         "state" : "Massachusetts",
         "code" : "KGDM"
      },
      "KPYM: Plymouth Municipal Airport (Massachusetts)" : {
         "city" : "Plymouth",
         "name" : "Plymouth Municipal Airport (Massachusetts)",
         "state" : "Massachusetts",
         "code" : "KPYM"
      },
      "KORH: Worcester Regional Airport" : {
         "city" : "Worcester",
         "name" : "Worcester Regional Airport",
         "state" : "Massachusetts",
         "code" : "KORH"
      },
      "KEWB: New Bedford Regional Airport" : {
         "city" : "New Bedford",
         "name" : "New Bedford Regional Airport",
         "state" : "Massachusetts",
         "code" : "KEWB"
      },
      "KACK: Nantucket Memorial Airport" : {
         "city" : "Nantucket",
         "name" : "Nantucket Memorial Airport",
         "state" : "Massachusetts",
         "code" : "KACK"
      },
      "KORE: Orange Municipal Airport" : {
         "city" : "Orange",
         "name" : "Orange Municipal Airport",
         "state" : "Massachusetts",
         "code" : "KORE"
      },
      "KHYA: Barnstable Municipal Airport" : {
         "city" : "Hyannis",
         "name" : "Barnstable Municipal Airport",
         "state" : "Massachusetts",
         "code" : "KHYA"
      },
      "KFIT: Fitchburg Municipal Airport" : {
         "city" : "Fitchburg",
         "name" : "Fitchburg Municipal Airport",
         "state" : "Massachusetts",
         "code" : "KFIT"
      },
      "KOWD: Norwood Memorial Airport" : {
         "city" : "Norwood",
         "name" : "Norwood Memorial Airport",
         "state" : "Massachusetts",
         "code" : "KOWD"
      },
      "KCQX: Chatham Municipal Airport" : {
         "city" : "Chatham",
         "name" : "Chatham Municipal Airport",
         "state" : "Massachusetts",
         "code" : "KCQX"
      },
      "KBOS: Logan International Airport" : {
         "city" : "Boston",
         "name" : "Logan International Airport",
         "state" : "Massachusetts",
         "code" : "KBOS"
      },
      "KGBR: Walter J. Koladza Airport" : {
         "city" : "Great Barrington",
         "name" : "Walter J. Koladza Airport",
         "state" : "Massachusetts",
         "code" : "KGBR"
      },
      "KFMH: Otis Air National Guard Base" : {
         "city" : "Falmouth",
         "name" : "Otis Air National Guard Base",
         "state" : "Massachusetts",
         "code" : "KFMH"
      },
      "KCEF: Westover Metropolitan Airport" : {
         "city" : "Springfield",
         "name" : "Westover Metropolitan Airport",
         "state" : "Massachusetts",
         "code" : "KCEF"
      },
      "KTAN: Taunton Municipal Airport" : {
         "city" : "Taunton",
         "name" : "Taunton Municipal Airport",
         "state" : "Massachusetts",
         "code" : "KTAN"
      },
      "KPSF: Pittsfield Municipal Airport" : {
         "city" : "Pittsfield",
         "name" : "Pittsfield Municipal Airport",
         "state" : "Massachusetts",
         "code" : "KPSF"
      },
      "KMVY: Martha's Vineyard Airport" : {
         "city" : "Vineyard Haven",
         "name" : "Martha's Vineyard Airport",
         "state" : "Massachusetts",
         "code" : "KMVY"
      },
      "KLWM: Lawrence Municipal Airport (Massachusetts)" : {
         "city" : "Lawrence",
         "name" : "Lawrence Municipal Airport (Massachusetts)",
         "state" : "Massachusetts",
         "code" : "KLWM"
      },
      "KAQW: Harriman-and-West Airport" : {
         "city" : "North Adams",
         "name" : "Harriman-and-West Airport",
         "state" : "Massachusetts",
         "code" : "KAQW"
      },
      "KBED: Hanscom Field" : {
         "city" : "Bedford",
         "name" : "Hanscom Field",
         "state" : "Massachusetts",
         "code" : "KBED"
      }
   },
   "Maryland, United States" : {
      "KADW: Andrews Air Force Base" : {
         "city" : "Camp Springs",
         "name" : "Andrews Air Force Base",
         "state" : "Maryland",
         "code" : "KADW"
      },
      "KMTN: Martin State Airport" : {
         "city" : "Baltimore",
         "name" : "Martin State Airport",
         "state" : "Maryland",
         "code" : "KMTN"
      },
      "KNHK: Naval Air Station Patuxent River" : {
         "city" : "Patuxent River",
         "name" : "Naval Air Station Patuxent River",
         "state" : "Maryland",
         "code" : "KNHK"
      },
      "KSBY: Wicomico Regional Airport" : {
         "city" : "Salisbury",
         "name" : "Wicomico Regional Airport",
         "state" : "Maryland",
         "code" : "KSBY"
      },
      "KHGR: Hagerstown Regional Airport" : {
         "city" : "Hagerstown",
         "name" : "Hagerstown Regional Airport",
         "state" : "Maryland",
         "code" : "KHGR"
      },
      "KRJD: Ridgely Airpark" : {
         "city" : "Ridgely",
         "name" : "Ridgely Airpark",
         "state" : "Maryland",
         "code" : "KRJD"
      },
      "KGAI: Montgomery County Airpark" : {
         "city" : "Gaithersburg",
         "name" : "Montgomery County Airpark",
         "state" : "Maryland",
         "code" : "KGAI"
      },
      "KCGS: College Park Airport" : {
         "city" : "College Park",
         "name" : "College Park Airport",
         "state" : "Maryland",
         "code" : "KCGS"
      },
      "KNUI: Webster NOLF" : {
         "city" : "Saint Inigoes",
         "name" : "Webster NOLF",
         "state" : "Maryland",
         "code" : "KNUI"
      },
      "KCBE: Greater Cumberland Regional Airport" : {
         "city" : "Cumberland",
         "name" : "Greater Cumberland Regional Airport",
         "state" : "Maryland",
         "code" : "KCBE"
      },
      "KEDG: Weide Army Airfield" : {
         "city" : "Aberdeen Proving Ground",
         "name" : "Weide Army Airfield",
         "state" : "Maryland",
         "code" : "KEDG"
      },
      "KOXB: Ocean City Municipal Airport (Maryland)" : {
         "city" : "Ocean City",
         "name" : "Ocean City Municipal Airport (Maryland)",
         "state" : "Maryland",
         "code" : "KOXB"
      },
      "KFDK: Frederick Municipal Airport (Maryland)" : {
         "city" : "Frederick",
         "name" : "Frederick Municipal Airport (Maryland)",
         "state" : "Maryland",
         "code" : "KFDK"
      },
      "KVKX: Potomac Airfield" : {
         "city" : "Friendly",
         "name" : "Potomac Airfield",
         "state" : "Maryland",
         "code" : "KVKX"
      },
      "KDMW: Carroll County Regional Airport" : {
         "city" : "Westminster",
         "name" : "Carroll County Regional Airport",
         "state" : "Maryland",
         "code" : "KDMW"
      },
      "KANP: Lee Airport" : {
         "city" : "Annapolis",
         "name" : "Lee Airport",
         "state" : "Maryland",
         "code" : "KANP"
      },
      "KESN: Easton/Newnam Field" : {
         "city" : "Easton",
         "name" : "Easton/Newnam Field",
         "state" : "Maryland",
         "code" : "KESN"
      },
      "KFME: Tipton Airport" : {
         "city" : "Fort Meade",
         "name" : "Tipton Airport",
         "state" : "Maryland",
         "code" : "KFME"
      },
      "KAPG: Phillips Army Airfield" : {
         "city" : "– Phillips Army Airfield – Aberdeen Proving Ground",
         "name" : "Phillips Army Airfield",
         "state" : "Maryland",
         "code" : "KAPG"
      },
      "KCGE: Cambridge-Dorchester Airport" : {
         "city" : "Cambridge",
         "name" : "Cambridge-Dorchester Airport",
         "state" : "Maryland",
         "code" : "KCGE"
      },
      "KBWI: Baltimore-Washington International Thurgood Marshall Airport" : {
         "city" : "Baltimore",
         "name" : "Baltimore-Washington International Thurgood Marshall Airport",
         "state" : "Maryland",
         "code" : "KBWI"
      }
   },
   "Nebraska, United States" : {
      "KLCG: Wayne Municipal Airport" : {
         "city" : "Wayne",
         "name" : "Wayne Municipal Airport",
         "state" : "Nebraska",
         "code" : "KLCG"
      },
      "KSWT: Seward Municipal Airport" : {
         "city" : "Seward",
         "name" : "Seward Municipal Airport",
         "state" : "Nebraska",
         "code" : "KSWT"
      },
      "KOLU: Columbus Municipal Airport (Nebraska)" : {
         "city" : "Columbus",
         "name" : "Columbus Municipal Airport (Nebraska)",
         "state" : "Nebraska",
         "code" : "KOLU"
      },
      "KFMZ: Fairmont State Airfield" : {
         "city" : "Fairmont",
         "name" : "Fairmont State Airfield",
         "state" : "Nebraska",
         "code" : "KFMZ"
      },
      "KBIE: Beatrice Municipal Airport" : {
         "city" : "Beatrice",
         "name" : "Beatrice Municipal Airport",
         "state" : "Nebraska",
         "code" : "KBIE"
      },
      "KMCK: McCook Municipal Airport" : {
         "city" : "McCook",
         "name" : "McCook Municipal Airport",
         "state" : "Nebraska",
         "code" : "KMCK"
      },
      "KOMA: Eppley Field" : {
         "city" : "Omaha",
         "name" : "Eppley Field",
         "state" : "Nebraska",
         "code" : "KOMA"
      },
      "KGTE: Quinn Airport" : {
         "city" : "Gothenburg",
         "name" : "Quinn Airport",
         "state" : "Nebraska",
         "code" : "KGTE"
      },
      "KBFF: Western Nebraska Regional Airport" : {
         "city" : "Scottsbluff",
         "name" : "Western Nebraska Regional Airport",
         "state" : "Nebraska",
         "code" : "KBFF"
      },
      "KFET: Fremont Municipal Airport (Nebraska)" : {
         "city" : "Fremont",
         "name" : "Fremont Municipal Airport (Nebraska)",
         "state" : "Nebraska",
         "code" : "KFET"
      },
      "KAUH: Aurora Municipal Airport (Nebraska)" : {
         "city" : "Aurora",
         "name" : "Aurora Municipal Airport (Nebraska)",
         "state" : "Nebraska",
         "code" : "KAUH"
      },
      "KONL: O'Neill Municipal Airport" : {
         "city" : "O'Neill",
         "name" : "O'Neill Municipal Airport",
         "state" : "Nebraska",
         "code" : "KONL"
      },
      "KODX: Sharp Airport" : {
         "city" : "Ord",
         "name" : "Sharp Airport",
         "state" : "Nebraska",
         "code" : "KODX"
      },
      "KSCB: Scribner State Airport" : {
         "city" : "Scribner",
         "name" : "Scribner State Airport",
         "state" : "Nebraska",
         "code" : "KSCB"
      },
      "KHJH: Hebron Municipal Airport" : {
         "city" : "Hebron",
         "name" : "Hebron Municipal Airport",
         "state" : "Nebraska",
         "code" : "KHJH"
      },
      "KTQE: Tekamah Airport" : {
         "city" : "Tekamah",
         "name" : "Tekamah Airport",
         "state" : "Nebraska",
         "code" : "KTQE"
      },
      "KAHQ: Wahoo Municipal Airport" : {
         "city" : "Wahoo",
         "name" : "Wahoo Municipal Airport",
         "state" : "Nebraska",
         "code" : "KAHQ"
      },
      "KLNK: Lincoln Airport (Nebraska)" : {
         "city" : "Lincoln",
         "name" : "Lincoln Airport (Nebraska)",
         "state" : "Nebraska",
         "code" : "KLNK"
      },
      "KGGF: Grant Municipal Airport" : {
         "city" : "Grant",
         "name" : "Grant Municipal Airport",
         "state" : "Nebraska",
         "code" : "KGGF"
      },
      "KCDR: Chadron Municipal Airport" : {
         "city" : "Chadron",
         "name" : "Chadron Municipal Airport",
         "state" : "Nebraska",
         "code" : "KCDR"
      },
      "KEPG: Browns Airport" : {
         "city" : "Weeping Water",
         "name" : "Browns Airport",
         "state" : "Nebraska",
         "code" : "KEPG"
      },
      "KAFK: Nebraska City Municipal Airport" : {
         "city" : "Nebraska City",
         "name" : "Nebraska City Municipal Airport",
         "state" : "Nebraska",
         "code" : "KAFK"
      },
      "KOGA: Searle Airport" : {
         "city" : "Ogallala",
         "name" : "Searle Airport",
         "state" : "Nebraska",
         "code" : "KOGA"
      },
      "KVTN: Miller Field (airport)" : {
         "city" : "Valentine",
         "name" : "Miller Field (airport)",
         "state" : "Nebraska",
         "code" : "KVTN"
      },
      "KANW: Ainsworth Municipal Airport" : {
         "city" : "Ainsworth",
         "name" : "Ainsworth Municipal Airport",
         "state" : "Nebraska",
         "code" : "KANW"
      },
      "KSNY: Sidney Municipal Airport (Nebraska)" : {
         "city" : "Sidney",
         "name" : "Sidney Municipal Airport (Nebraska)",
         "state" : "Nebraska",
         "code" : "KSNY"
      },
      "KLXN: Jim Kelly Field" : {
         "city" : "Lexington",
         "name" : "Jim Kelly Field",
         "state" : "Nebraska",
         "code" : "KLXN"
      },
      "KHDE: Brewster Airport" : {
         "city" : "Holdrege",
         "name" : "Brewster Airport",
         "state" : "Nebraska",
         "code" : "KHDE"
      },
      "KRBE: Rock County Airport (Nebraska)" : {
         "city" : "Bassett",
         "name" : "Rock County Airport (Nebraska)",
         "state" : "Nebraska",
         "code" : "KRBE"
      },
      "KMHN: Hooker County Airport" : {
         "city" : "Mullen",
         "name" : "Hooker County Airport",
         "state" : "Nebraska",
         "code" : "KMHN"
      },
      "KOFF: Offutt Air Force Base" : {
         "city" : "Omaha",
         "name" : "Offutt Air Force Base",
         "state" : "Nebraska",
         "code" : "KOFF"
      },
      "KGRI: Central Nebraska Regional Airport" : {
         "city" : "Grand Island",
         "name" : "Central Nebraska Regional Airport",
         "state" : "Nebraska",
         "code" : "KGRI"
      },
      "KEAR: Kearney Municipal Airport" : {
         "city" : "Kearney",
         "name" : "Kearney Municipal Airport",
         "state" : "Nebraska",
         "code" : "KEAR"
      },
      "KIBM: Kimball Municipal Airport" : {
         "city" : "Kimball",
         "name" : "Kimball Municipal Airport",
         "state" : "Nebraska",
         "code" : "KIBM"
      },
      "KLBF: North Platte Regional Airport" : {
         "city" : "North Platte",
         "name" : "North Platte Regional Airport",
         "state" : "Nebraska",
         "code" : "KLBF"
      },
      "KCEK: Crete Municipal Airport" : {
         "city" : "Crete",
         "name" : "Crete Municipal Airport",
         "state" : "Nebraska",
         "code" : "KCEK"
      },
      "KCNP: Billy G. Ray Field" : {
         "city" : "Chappell",
         "name" : "Billy G. Ray Field",
         "state" : "Nebraska",
         "code" : "KCNP"
      },
      "KTQK: Scott City Municipal Airport" : {
         "city" : "Scott City",
         "name" : "Scott City Municipal Airport",
         "state" : "Nebraska",
         "code" : "KTQK"
      },
      "KOKS: Garden County Airport" : {
         "city" : "Oshkosh",
         "name" : "Garden County Airport",
         "state" : "Nebraska",
         "code" : "KOKS"
      },
      "KBVN: Albion Municipal Airport" : {
         "city" : "Albion",
         "name" : "Albion Municipal Airport",
         "state" : "Nebraska",
         "code" : "KBVN"
      },
      "KBBW: Broken Bow Municipal Airport" : {
         "city" : "Broken Bow",
         "name" : "Broken Bow Municipal Airport",
         "state" : "Nebraska",
         "code" : "KBBW"
      },
      "KFBY: Fairbury Municipal Airport" : {
         "city" : "Fairbury",
         "name" : "Fairbury Municipal Airport",
         "state" : "Nebraska",
         "code" : "KFBY"
      },
      "KAIA: Alliance Municipal Airport" : {
         "city" : "Alliance",
         "name" : "Alliance Municipal Airport",
         "state" : "Nebraska",
         "code" : "KAIA"
      },
      "KGRN: Gordon Municipal Airport" : {
         "city" : "Gordon",
         "name" : "Gordon Municipal Airport",
         "state" : "Nebraska",
         "code" : "KGRN"
      },
      "KFNB: Brenner Field" : {
         "city" : "Falls City",
         "name" : "Brenner Field",
         "state" : "Nebraska",
         "code" : "KFNB"
      },
      "KJYR: York Municipal Airport" : {
         "city" : "York",
         "name" : "York Municipal Airport",
         "state" : "Nebraska",
         "code" : "KJYR"
      },
      "KOFK: Stefan Memorial Airport" : {
         "city" : "Norfolk",
         "name" : "Stefan Memorial Airport",
         "state" : "Nebraska",
         "code" : "KOFK"
      },
      "KCSB: Cambridge Municipal Airport (Nebraska)" : {
         "city" : "Cambridge",
         "name" : "Cambridge Municipal Airport (Nebraska)",
         "state" : "Nebraska",
         "code" : "KCSB"
      },
      "KHSI: Hastings Municipal Airport" : {
         "city" : "Hastings",
         "name" : "Hastings Municipal Airport",
         "state" : "Nebraska",
         "code" : "KHSI"
      },
      "KCZD: Cozad Municipal Airport" : {
         "city" : "Cozad",
         "name" : "Cozad Municipal Airport",
         "state" : "Nebraska",
         "code" : "KCZD"
      },
      "KBUB: Cram Field" : {
         "city" : "Burwell",
         "name" : "Cram Field",
         "state" : "Nebraska",
         "code" : "KBUB"
      },
      "KIML: Imperial Municipal Airport" : {
         "city" : "Imperial",
         "name" : "Imperial Municipal Airport",
         "state" : "Nebraska",
         "code" : "KIML"
      },
      "KPMV: Plattsmouth Municipal Airport" : {
         "city" : "Plattsmouth",
         "name" : "Plattsmouth Municipal Airport",
         "state" : "Nebraska",
         "code" : "KPMV"
      }
   },
   "Delaware, United States" : {
      "KILG: New Castle County Airport" : {
         "city" : "Wilmington",
         "name" : "New Castle County Airport",
         "state" : "Delaware",
         "code" : "KILG"
      },
      "KGED: Sussex County Airport" : {
         "city" : "Georgetown",
         "name" : "Sussex County Airport",
         "state" : "Delaware",
         "code" : "KGED"
      },
      "KDOV: Dover Air Force Base" : {
         "city" : "Dover",
         "name" : "Dover Air Force Base",
         "state" : "Delaware",
         "code" : "KDOV"
      },
      "KEVY: Summit Airport (Delaware)" : {
         "city" : "Middletown",
         "name" : "Summit Airport (Delaware)",
         "state" : "Delaware",
         "code" : "KEVY"
      }
   },
   "Maine, United States" : {
      "KSFM: Sanford Regional Airport" : {
         "city" : "Sanford",
         "name" : "Sanford Regional Airport",
         "state" : "Maine",
         "code" : "KSFM"
      },
      "KMVM: Machias-Valley Airport" : {
         "city" : "Machias",
         "name" : "Machias-Valley Airport",
         "state" : "Maine",
         "code" : "KMVM"
      },
      "KNHZ: Naval Air Station Brunswick" : {
         "city" : "Brunswick",
         "name" : "Naval Air Station Brunswick",
         "state" : "Maine",
         "code" : "KNHZ"
      },
      "KIZG: Eastern Slopes Regional Airport" : {
         "city" : "Fryeburg",
         "name" : "Eastern Slopes Regional Airport",
         "state" : "Maine",
         "code" : "KIZG"
      },
      "KBST: Belfast Municipal Airport" : {
         "city" : "Belfast",
         "name" : "Belfast Municipal Airport",
         "state" : "Maine",
         "code" : "KBST"
      },
      "KOLD: Old Town Municipal Airport and Seaplane Base" : {
         "city" : "Old Town",
         "name" : "Old Town Municipal Airport and Seaplane Base",
         "state" : "Maine",
         "code" : "KOLD"
      },
      "KCAR: Caribou Municipal Airport" : {
         "city" : "Caribou",
         "name" : "Caribou Municipal Airport",
         "state" : "Maine",
         "code" : "KCAR"
      },
      "KBGR: Bangor International Airport" : {
         "city" : "Bangor",
         "name" : "Bangor International Airport",
         "state" : "Maine",
         "code" : "KBGR"
      },
      "KIWI: Wiscasset Airport" : {
         "city" : "Wiscasset",
         "name" : "Wiscasset Airport",
         "state" : "Maine",
         "code" : "KIWI"
      },
      "KEPM: Eastport Municipal Airport" : {
         "city" : "Eastport",
         "name" : "Eastport Municipal Airport",
         "state" : "Maine",
         "code" : "KEPM"
      },
      "KRKD: Knox County Regional Airport" : {
         "city" : "Rockland",
         "name" : "Knox County Regional Airport",
         "state" : "Maine",
         "code" : "KRKD"
      },
      "KPWM: Portland International Jetport" : {
         "city" : "Portland",
         "name" : "Portland International Jetport",
         "state" : "Maine",
         "code" : "KPWM"
      },
      "KAUG: Augusta State Airport" : {
         "city" : "Augusta",
         "name" : "Augusta State Airport",
         "state" : "Maine",
         "code" : "KAUG"
      },
      "KBHB: Hancock County-Bar Harbor Airport" : {
         "city" : "Bar Harbor",
         "name" : "Hancock County-Bar Harbor Airport",
         "state" : "Maine",
         "code" : "KBHB"
      },
      "KHUL: Houlton International Airport" : {
         "city" : "Houlton",
         "name" : "Houlton International Airport",
         "state" : "Maine",
         "code" : "KHUL"
      },
      "KLRG: Lincoln Regional Airport (Maine)" : {
         "city" : "Lincoln",
         "name" : "Lincoln Regional Airport (Maine)",
         "state" : "Maine",
         "code" : "KLRG"
      },
      "KPQI: Northern Maine Regional At Presque Airport" : {
         "city" : "Presque Isle",
         "name" : "Northern Maine Regional At Presque Airport",
         "state" : "Maine",
         "code" : "KPQI"
      },
      "KPNN: Princeton Municipal Airport (Maine)" : {
         "city" : "Princeton",
         "name" : "Princeton Municipal Airport (Maine)",
         "state" : "Maine",
         "code" : "KPNN"
      },
      "KMLT: Millinocket Municipal Airport" : {
         "city" : "Millinocket",
         "name" : "Millinocket Municipal Airport",
         "state" : "Maine",
         "code" : "KMLT"
      },
      "KWVL: Waterville Robert LaFleur Airport" : {
         "city" : "Waterville",
         "name" : "Waterville Robert LaFleur Airport",
         "state" : "Maine",
         "code" : "KWVL"
      },
      "KLEW: Auburn/Lewiston Municipal Airport" : {
         "city" : "Auburn",
         "name" : "Auburn/Lewiston Municipal Airport",
         "state" : "Maine",
         "code" : "KLEW"
      },
      "KOWK: Central Maine Airport of Norridgewock" : {
         "city" : "Norridgewock",
         "name" : "Central Maine Airport of Norridgewock",
         "state" : "Maine",
         "code" : "KOWK"
      },
      "KFVE: Northern Aroostook Regional Airport" : {
         "city" : "Frenchville",
         "name" : "Northern Aroostook Regional Airport",
         "state" : "Maine",
         "code" : "KFVE"
      }
   },
   "North Dakota, United States" : {
      "KMOT: Minot International Airport" : {
         "city" : "Minot",
         "name" : "Minot International Airport",
         "state" : "North Dakota",
         "code" : "KMOT"
      },
      "KRUG: Rugby Municipal Airport" : {
         "city" : "Rugby",
         "name" : "Rugby Municipal Airport",
         "state" : "North Dakota",
         "code" : "KRUG"
      },
      "KHZE: Mercer County Regional Airport" : {
         "city" : "Hazen",
         "name" : "Mercer County Regional Airport",
         "state" : "North Dakota",
         "code" : "KHZE"
      },
      "KJMS: Jamestown Regional Airport" : {
         "city" : "Jamestown",
         "name" : "Jamestown Regional Airport",
         "state" : "North Dakota",
         "code" : "KJMS"
      },
      "KASY: Ashley Municipal Airport" : {
         "city" : "Ashley",
         "name" : "Ashley Municipal Airport",
         "state" : "North Dakota",
         "code" : "KASY"
      },
      "KHEI: Hettinger Municipal Airport" : {
         "city" : "Hettinger",
         "name" : "Hettinger Municipal Airport",
         "state" : "North Dakota",
         "code" : "KHEI"
      },
      "KDIK: Theodore Roosevelt Regional Airport" : {
         "city" : "Dickinson",
         "name" : "Theodore Roosevelt Regional Airport",
         "state" : "North Dakota",
         "code" : "KDIK"
      },
      "KMIB: Minot Air Force Base" : {
         "city" : "Minot",
         "name" : "Minot Air Force Base",
         "state" : "North Dakota",
         "code" : "KMIB"
      },
      "KGWR: Gwinner-Roger Melroe Field" : {
         "city" : "Gwinner",
         "name" : "Gwinner-Roger Melroe Field",
         "state" : "North Dakota",
         "code" : "KGWR"
      },
      "KDVL: Devils Lake Municipal Airport" : {
         "city" : "Devils Lake",
         "name" : "Devils Lake Municipal Airport",
         "state" : "North Dakota",
         "code" : "KDVL"
      },
      "KBWP: Harry Stern Airport" : {
         "city" : "Wahpeton",
         "name" : "Harry Stern Airport",
         "state" : "North Dakota",
         "code" : "KBWP"
      },
      "KGAF: Grafton Municipal Airport" : {
         "city" : "Grafton",
         "name" : "Grafton Municipal Airport",
         "state" : "North Dakota",
         "code" : "KGAF"
      },
      "KISN: Sloulin Field International Airport" : {
         "city" : "Williston",
         "name" : "Sloulin Field International Airport",
         "state" : "North Dakota",
         "code" : "KISN"
      },
      "KBIS: Bismarck Municipal Airport" : {
         "city" : "Bismarck",
         "name" : "Bismarck Municipal Airport",
         "state" : "North Dakota",
         "code" : "KBIS"
      },
      "KFAR: Hector International Airport" : {
         "city" : "Fargo",
         "name" : "Hector International Airport",
         "state" : "North Dakota",
         "code" : "KFAR"
      },
      "KPMB: Pembina Municipal Airport" : {
         "city" : "Pembina",
         "name" : "Pembina Municipal Airport",
         "state" : "North Dakota",
         "code" : "KPMB"
      },
      "KRDR: Grand Forks Air Force Base" : {
         "city" : "Grand Forks",
         "name" : "Grand Forks Air Force Base",
         "state" : "North Dakota",
         "code" : "KRDR"
      },
      "KBPP: Bowman Municipal Airport" : {
         "city" : "Bowman",
         "name" : "Bowman Municipal Airport",
         "state" : "North Dakota",
         "code" : "KBPP"
      },
      "KGFK: Grand Forks International Airport" : {
         "city" : "Grand Forks",
         "name" : "Grand Forks International Airport",
         "state" : "North Dakota",
         "code" : "KGFK"
      },
      "KHBC: Mohall Municipal Airport" : {
         "city" : "Mohall",
         "name" : "Mohall Municipal Airport",
         "state" : "North Dakota",
         "code" : "KHBC"
      }
   },
   "Wisconsin, United States" : {
      "KPCZ: Waupaca Municipal Airport" : {
         "city" : "Waupaca",
         "name" : "Waupaca Municipal Airport",
         "state" : "Wisconsin",
         "code" : "KPCZ"
      },
      "KCWA: Central Wisconsin Airport" : {
         "city" : "Mosinee",
         "name" : "Central Wisconsin Airport",
         "state" : "Wisconsin",
         "code" : "KCWA"
      },
      "KPDC: Prairie du Chien Municipal Airport" : {
         "city" : "Prairie du Chien",
         "name" : "Prairie du Chien Municipal Airport",
         "state" : "Wisconsin",
         "code" : "KPDC"
      },
      "KGRB: Austin Straubel International Airport" : {
         "city" : "Green Bay",
         "name" : "Austin Straubel International Airport",
         "state" : "Wisconsin",
         "code" : "KGRB"
      },
      "KBDJ: Boulder Junction Airport" : {
         "city" : "Boulder Junction",
         "name" : "Boulder Junction Airport",
         "state" : "Wisconsin",
         "code" : "KBDJ"
      },
      "KMTW: Manitowoc County Airport" : {
         "city" : "Manitowoc",
         "name" : "Manitowoc County Airport",
         "state" : "Wisconsin",
         "code" : "KMTW"
      },
      "KMDZ: Taylor County Airport (Wisconsin)" : {
         "city" : "Medford",
         "name" : "Taylor County Airport (Wisconsin)",
         "state" : "Wisconsin",
         "code" : "KMDZ"
      },
      "KSSQ: Shell Lake Municipal Airport" : {
         "city" : "Shell Lake",
         "name" : "Shell Lake Municipal Airport",
         "state" : "Wisconsin",
         "code" : "KSSQ"
      },
      "KETB: West Bend Municipal Airport" : {
         "city" : "West Bend",
         "name" : "West Bend Municipal Airport",
         "state" : "Wisconsin",
         "code" : "KETB"
      },
      "KFLD: Fond du Lac County Airport" : {
         "city" : "Fond du Lac",
         "name" : "Fond du Lac County Airport",
         "state" : "Wisconsin",
         "code" : "KFLD"
      },
      "KBCK: Black River Falls Area Airport" : {
         "city" : "Black River Falls",
         "name" : "Black River Falls Area Airport",
         "state" : "Wisconsin",
         "code" : "KBCK"
      },
      "KMSN: Dane County Regional Airport" : {
         "city" : "Madison",
         "name" : "Dane County Regional Airport",
         "state" : "Wisconsin",
         "code" : "KMSN"
      },
      "KCMY: Sparta/Fort McCoy Airport" : {
         "city" : "Sparta",
         "name" : "Sparta/Fort McCoy Airport",
         "state" : "Wisconsin",
         "code" : "KCMY"
      },
      "KEZS: Shawano Municipal Airport" : {
         "city" : "Shawano",
         "name" : "Shawano Municipal Airport",
         "state" : "Wisconsin",
         "code" : "KEZS"
      },
      "KSBM: Sheboygan County Memorial Airport" : {
         "city" : "Sheboygan",
         "name" : "Sheboygan County Memorial Airport",
         "state" : "Wisconsin",
         "code" : "KSBM"
      },
      "KMKE: General Mitchell International Airport" : {
         "city" : "Milwaukee",
         "name" : "General Mitchell International Airport",
         "state" : "Wisconsin",
         "code" : "KMKE"
      },
      "KDLL: Baraboo-Wisconsin Dells Airport" : {
         "city" : "Baraboo",
         "name" : "Baraboo-Wisconsin Dells Airport",
         "state" : "Wisconsin",
         "code" : "KDLL"
      },
      "KHXF: Hartford Municipal Airport" : {
         "city" : "Hartford",
         "name" : "Hartford Municipal Airport",
         "state" : "Wisconsin",
         "code" : "KHXF"
      },
      "KISW: South Wood County Airport" : {
         "city" : "Wisconsin Rapids",
         "name" : "South Wood County Airport",
         "state" : "Wisconsin",
         "code" : "KISW"
      },
      "KAHH: Amery Municipal Airport" : {
         "city" : "Amery",
         "name" : "Amery Municipal Airport",
         "state" : "Wisconsin",
         "code" : "KAHH"
      },
      "KDAF: Necedah Airport" : {
         "city" : "Necedah",
         "name" : "Necedah Airport",
         "state" : "Wisconsin",
         "code" : "KDAF"
      },
      "KEGV: Eagle River Union Airport" : {
         "city" : "Eagle River",
         "name" : "Eagle River Union Airport",
         "state" : "Wisconsin",
         "code" : "KEGV"
      },
      "KRYV: Watertown Municipal Airport (Wisconsin)" : {
         "city" : "Watertown",
         "name" : "Watertown Municipal Airport (Wisconsin)",
         "state" : "Wisconsin",
         "code" : "KRYV"
      },
      "KRPD: Rice Lake Regional Airport" : {
         "city" : "Rice Lake",
         "name" : "Rice Lake Regional Airport",
         "state" : "Wisconsin",
         "code" : "KRPD"
      },
      "KPVB: Platteville Municipal Airport" : {
         "city" : "Platteville",
         "name" : "Platteville Municipal Airport",
         "state" : "Wisconsin",
         "code" : "KPVB"
      },
      "KATW: Outagamie County Regional Airport" : {
         "city" : "Greenville",
         "name" : "Outagamie County Regional Airport",
         "state" : "Wisconsin",
         "code" : "KATW"
      },
      "KCLI: Clintonville Municipal Airport" : {
         "city" : "Clintonville",
         "name" : "Clintonville Municipal Airport",
         "state" : "Wisconsin",
         "code" : "KCLI"
      },
      "KAIG: Langlade County Airport" : {
         "city" : "Antigo",
         "name" : "Langlade County Airport",
         "state" : "Wisconsin",
         "code" : "KAIG"
      },
      "KRAC: Batten International Airport" : {
         "city" : "Racine",
         "name" : "Batten International Airport",
         "state" : "Wisconsin",
         "code" : "KRAC"
      },
      "KRZN: Burnett County Airport" : {
         "city" : "Siren",
         "name" : "Burnett County Airport",
         "state" : "Wisconsin",
         "code" : "KRZN"
      },
      "KEFT: Monroe Municipal Airport" : {
         "city" : "Monroe",
         "name" : "Monroe Municipal Airport",
         "state" : "Wisconsin",
         "code" : "KEFT"
      },
      "KSUW: Richard I. Bong Airport" : {
         "city" : "Superior",
         "name" : "Richard I. Bong Airport",
         "state" : "Wisconsin",
         "code" : "KSUW"
      },
      "KGTG: Grantsburg Municipal Airport" : {
         "city" : "Grantsburg",
         "name" : "Grantsburg Municipal Airport",
         "state" : "Wisconsin",
         "code" : "KGTG"
      },
      "KBUU: Burlington Municipal Airport (Wisconsin)" : {
         "city" : "Burlington",
         "name" : "Burlington Municipal Airport (Wisconsin)",
         "state" : "Wisconsin",
         "code" : "KBUU"
      },
      "KENW: Kenosha Regional Airport" : {
         "city" : "Kenosha",
         "name" : "Kenosha Regional Airport",
         "state" : "Wisconsin",
         "code" : "KENW"
      },
      "KOSH: Whittman Regional Airport" : {
         "city" : "Oshkosh",
         "name" : "Whittman Regional Airport",
         "state" : "Wisconsin",
         "code" : "KOSH"
      },
      "KOVS: Boscobel Airport" : {
         "city" : "Boscobel",
         "name" : "Boscobel Airport",
         "state" : "Wisconsin",
         "code" : "KOVS"
      },
      "KUBE: Cumberland Municipal Airport" : {
         "city" : "Cumberland",
         "name" : "Cumberland Municipal Airport",
         "state" : "Wisconsin",
         "code" : "KUBE"
      },
      "KAUW: Wausau Downtown Airport" : {
         "city" : "Wausau",
         "name" : "Wausau Downtown Airport",
         "state" : "Wisconsin",
         "code" : "KAUW"
      },
      "KARV: Lakeland Airport" : {
         "city" : "Minocqua",
         "name" : "Lakeland Airport",
         "state" : "Wisconsin",
         "code" : "KARV"
      },
      "KUES: Waukesha County Airport" : {
         "city" : "Waukesha",
         "name" : "Waukesha County Airport",
         "state" : "Wisconsin",
         "code" : "KUES"
      },
      "KMFI: Marshfield Municipal Airport (Wisconsin)" : {
         "city" : "Marshfield",
         "name" : "Marshfield Municipal Airport (Wisconsin)",
         "state" : "Wisconsin",
         "code" : "KMFI"
      },
      "KUNU: Dodge County Airport" : {
         "city" : "Juneau",
         "name" : "Dodge County Airport",
         "state" : "Wisconsin",
         "code" : "KUNU"
      },
      "KASX: John F. Kennedy Memorial Airport" : {
         "city" : "Ashland",
         "name" : "John F. Kennedy Memorial Airport",
         "state" : "Wisconsin",
         "code" : "KASX"
      },
      "KHYR: Sawyer County Airport" : {
         "city" : "Hayward",
         "name" : "Sawyer County Airport",
         "state" : "Wisconsin",
         "code" : "KHYR"
      },
      "KEAU: Chippewa Valley Regional Airport" : {
         "city" : "Eau Claire",
         "name" : "Chippewa Valley Regional Airport",
         "state" : "Wisconsin",
         "code" : "KEAU"
      },
      "KLSE: La Crosse Municipal Airport" : {
         "city" : "La Crosse",
         "name" : "La Crosse Municipal Airport",
         "state" : "Wisconsin",
         "code" : "KLSE"
      }
   },
   "Vermont, United States" : {
      "KDDH: William H. Morse State Airport" : {
         "city" : "Bennington",
         "name" : "William H. Morse State Airport",
         "state" : "Vermont",
         "code" : "KDDH"
      },
      "KVSF: Hartness State Airport" : {
         "city" : "Springfield",
         "name" : "Hartness State Airport",
         "state" : "Vermont",
         "code" : "KVSF"
      },
      "KEFK: Newport State Airport (Vermont)" : {
         "city" : "Newport",
         "name" : "Newport State Airport (Vermont)",
         "state" : "Vermont",
         "code" : "KEFK"
      },
      "KFSO: Franklin County State Airport" : {
         "city" : "Highgate",
         "name" : "Franklin County State Airport",
         "state" : "Vermont",
         "code" : "KFSO"
      },
      "KCDA: Caledonia County Airport" : {
         "city" : "Lyndonville",
         "name" : "Caledonia County Airport",
         "state" : "Vermont",
         "code" : "KCDA"
      },
      "KBTV: Burlington International Airport" : {
         "city" : "Burlington",
         "name" : "Burlington International Airport",
         "state" : "Vermont",
         "code" : "KBTV"
      },
      "KMPV: Edward F. Knapp State Airport" : {
         "city" : "Barre",
         "name" : "Edward F. Knapp State Airport",
         "state" : "Vermont",
         "code" : "KMPV"
      },
      "KRUT: Rutland State Airport" : {
         "city" : "Rutland",
         "name" : "Rutland State Airport",
         "state" : "Vermont",
         "code" : "KRUT"
      },
      "KMVL: Morrisville Stowe State Airport" : {
         "city" : "Morrisville",
         "name" : "Morrisville Stowe State Airport",
         "state" : "Vermont",
         "code" : "KMVL"
      }
   }
}
;